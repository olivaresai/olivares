...
## Files
`go.mod` and `go.sum` hold the only Go and cedar-go pins. `supervisor.py` holds the only limits. `MATRIX.json` holds the only inputs and expectations, and `schema/` describes both JSON formats.
