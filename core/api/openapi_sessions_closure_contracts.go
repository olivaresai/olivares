...
func sessionsClosureRequestBodyDeclarationFor(r moduleRoute) (sessionsClosureRequestBodyDeclaration, bool) {
	...
	case http.MethodPost + " /provider-profiles/{ref}/retire",
		...
		// D19. The connection TEST is bodyless on purpose: everything it needs —
...
func sessionsCreateProviderProfileSchema() map[string]any {
	return sessionsClosureClosedObject(oaObj(
		...
		"provider_record_ref", sessionsClosureOptionalString("D19. Optional: the registered provider whose credential this profile's managed launches use. Empty leaves the host's own credential variables in charge, which is every profile's behaviour before D19. The engine refuses a credential whose kind this driver cannot read."),
...
// sessionsPatchProviderSchema mirrors patchProviderRecordRequest (D19): rename,
