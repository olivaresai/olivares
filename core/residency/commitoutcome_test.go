...
// TestGuardStorePreservesCommitOutcome is HS-W for the residency decorator.
//
// The commit-outcome sentinel is only worth having if it survives the layers
// between the store and the caller. This decorator already returns the inner
// error untouched; the control is here so that a future rewrite which "tidies"
// the return into %v — losing the chain and with it every errors.Is a consumer
// depends on — fails instead of shipping.
//
...
func TestGuardStorePreservesCommitOutcome(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("build the enforcing registry: %v", err)
	}
	guard := residency.Guard(inner, reg, nil)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !errors.Is(got, store.ErrCommitOutcomeUnknown) {
				t.Errorf("the residency guard dropped the commit-outcome sentinel: %v", got)
			}
			if !errors.Is(got, cause) {
				t.Errorf("the residency guard dropped the original cause: %v", got)
			}
		})
	}
	...
	t.Run("nil_is_still_nil", func(t *testing.T) {
		t.Parallel()
		clean := residency.Guard(commitOutcomeFakeStore{}, reg, nil)
