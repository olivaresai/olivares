...
| Peer identity / throttling | Client IP always via `SplitHostPort(RemoteAddr)`; API rate-limit keys on principal/tenant, not IP; `X-Forwarded-For` deliberately not trusted. Open residual: the login throttle keys on the WHOLE address, with no /64 aggregation, so a client holding a routed IPv6 prefix presents a fresh key per address — the account key is what bounds it meanwhile | `core/api/handlers_auth.go`, `core/auth/throttle.go`, `core/api/middleware_ratelimit.go` |
