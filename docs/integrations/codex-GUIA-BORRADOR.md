...
# Integrar Codex con Olivares AI

> Borrador editorial para la documentación pública. Las capturas pendientes se producirán
> desde el estate de demostración; no representan maquetas.
...
[CAPTURA: alta de `codex-enterprise` en Control console > Connectors, con `workspace_id`, credencial sellada, intervalo por lotes y estado habilitado; variantes clara y oscura, sin revelar identificadores ni secretos reales.]
...
[CAPTURA: configuración de Codex con comparación entre `requirements.toml` impuesto, `managed_config.toml` como defaults y estado del hook administrado; variantes clara y oscura del estate sembrado.]
...
[CAPTURA: panel de Codex con la fuente sana, registros de ingestión, gasto en FinOps, hallazgos de Security y una sesión procedente del hook; variantes clara y oscura del estate sembrado.]
