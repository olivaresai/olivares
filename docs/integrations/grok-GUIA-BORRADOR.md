...
# Integrar Grok Build con Olivares AI

> Borrador editorial para la documentación pública. Las capturas señaladas se producirán
> desde el estate de demostración y no deben sustituirse por maquetas.
...
[CAPTURA: alta de `grok-demo` en Control console > Connectors, con tipo `grok`, intervalo 60, rutas locales y estado habilitado; variantes clara y oscura del estate sembrado.]
...
[CAPTURA: configuración de Grok Build con `requirements.toml` presente, perfil `strict`, inventario MCP, estado de `disabled-hooks` y receptor OTLP apagado o en loopback; variantes clara y oscura.]
...
[CAPTURA: panel de Grok Build con hallazgos de sandbox/MCP/hooks en Security, actividad de la fuente en Ingestion y una sesión del hook con postura visible; variantes clara y oscura del estate sembrado.]
