...
//
// THE FIRST SCREENS, from a keyboard, in both themes and in all seven console
// languages, against a live `--seed-demo` engine. Nothing here is intercepted.
//
// ⛔ IT CARRIES THE DEMO GUARD, AND FOR A MEASURED REASON. `scripts/web-e2e.sh` puts
...
test.describe('the first screens', () => {
  test.beforeEach(() => {
    test.skip(
      !demoTenant,
  ...
  for (const theme of ['dark', 'light'] as const) {
    test(`home (${theme}): one h1, the next action, and the work`, async ({
      ...
      // at:gate holds every route to exactly one h1; the statement blocks on the
      // signed-out screens are text and section headings, never a second page title.
