...
const ORIGIN = new URL(
  process.env.PLAYWRIGHT_BASE_URL ?? 'http://127.0.0.1:8456',
).origin
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () => reject(new Inability(`${what}: deadline exhausted`)),
          milliseconds,
        )
      }),
    ])
...
const isRun = (v: unknown): v is RunRow => {
  ...
  return (
    typeof r.run_ref === 'string' &&
    r.run_ref.length > 0 &&
    typeof r.state === 'string' &&
    (r.name === undefined || typeof r.name === 'string') &&
    (r.provider_profile_ref === undefined ||
      typeof r.provider_profile_ref === 'string')
  )
}
...
const matchesFixture = (r: RunRow) =>
  r.name === RUN_NAME && r.provider_profile_ref === PROFILE_REF
const matching = (rows: RunRow[]) =>
  rows
    .filter(matchesFixture)
    .map((r) => r.run_ref)
    .sort()
async function checkedBody<T>(
  response: APIResponse,
  shape: (v: unknown) => v is T,
  what: string,
) {
  ...
  try {
    ...
    throw new Inability(`${what}: unreadable JSON`)
  }
...
async function readChecked<T>(
  ...
  shape: (v: unknown) => v is T,
  what: string,
  deadline = deadlineAfter(CALL_TIMEOUT_MS),
): Promise<T> {
  try {
    return await checkedBody(
      await api.get(`${ORIGIN}${path}`, {
...
async function exactOwnedRun(
  api: APIRequestContext,
  ref: string,
  deadline: number,
) {
  const row = await readChecked(
    api,
    `${RUNS_PATH}/${encodeURIComponent(ref)}`,
    isRun,
    'owned run',
    deadline,
  )
  if (row.run_ref !== ref || !matchesFixture(row))
    throw new Inability('owned run: reference or fixture identity differs')
  return row
}
...
class OwnedLaunch {
  calls = 0
  private observation?: Promise<
    { value: CreateObservation } | { error: unknown }
  >
  ...
  private arm() {
    if (this.closed || this.observation)
      throw new Error('admission is closed or already armed')
    ...
    this.observation = (async () => {
      ...
      const body: unknown = await within(
        response.json(),
        deadline,
        'create response body',
      )
      if (!isRun(body))
        throw new Inability('create response: invalid run shape')
      ...
      return {
        ...
        body,
        sentName: sent?.name,
        sentProfile: sent?.provider_profile_ref,
      }
    })().then(
      (value) => ({ value }),
      (error) => ({ error }),
  ...
  private async admitted() {
    ...
    if ('error' in observed)
      throw new Inability(`create response: ${observed.error}`)
    ...
    const row = await exactOwnedRun(
      this.page.request,
      value.body.run_ref,
      deadlineAfter(CALL_TIMEOUT_MS),
    )
  ...
  async settle() {
    ...
    if (this.observation) {
      const result = await within(
        this.observation,
        deadline,
        'settle create observer',
      )
      notes.push(
        'error' in result
          ? 'create_response=unreadable'
          : `create_response=HTTP${result.value.status}`,
      )
    }
    if (!this.possibleEffect) return
    const api = this.page.request
    const observe = async <T>(
      read: (until: number) => Promise<T>,
      until: number,
    ): Promise<T> => {
      let last: unknown
    ...
    try {
      ...
      while (!ref) {
        const candidates = matching(
          (
            await observe(
              (until) =>
                readChecked(api, RUNS_PATH, isRunList, 'resolve effect', until),
              deadline,
            )
          ).items,
        )
        if (candidates.length > 1)
          throw new Inability('effect UNKNOWN: multiple matching rows')
        if (candidates.length === 1) ref = candidates[0]
        else
          await new Promise((resolve) =>
            setTimeout(
              resolve,
              Math.min(250, timeLeft(deadline, 'resolve effect')),
            ),
          )
      }
      let row = await observe(
        (until) => exactOwnedRun(api, ref, until),
        deadline,
      )
      if (!TERMINAL.includes(row.state)) {
        try {
          const stop = await api.post(
            `${ORIGIN}${RUNS_PATH}/${encodeURIComponent(ref)}/stop`,
            {
        ...
        while (
          ...
          try {
            ...
            notes.push(`terminal_read=unavailable ${error}`)
          }
          if (
            !TERMINAL.includes(row.state) &&
            performance.now() < pollingDeadline
            ...
            await new Promise((resolve) =>
              setTimeout(
                ...
                Math.min(
                  250,
                  timeLeft(pollingDeadline, 'terminal observation'),
                ),
              ),
            )
          }
      ...
      try {
        ...
        notes.push(`before_cleanup=unavailable ${error}`)
      }
      ...
      try {
        if (!corroborated)
          throw new Inability(
            'cleanup not authorized by an exact checked identity',
          )
        cleaned = await checkedBody(
          await api.post(
            `${ORIGIN}${RUNS_PATH}/${encodeURIComponent(ref)}/cleanup`,
            {
      ...
      notes.push(
        `cleanup=${cleaned?.state ?? 'UNREAD'} readback=${readback.state}`,
      )
      if (
        !TERMINAL.includes(row.state) ||
        cleanupError ||
        !cleaned ||
        cleaned.run_ref !== ref ||
        !matchesFixture(cleaned) ||
        cleaned.state !== 'cleaned' ||
        readback.state !== 'cleaned'
      ) {
        throw new Inability(
          `cleanup not proved: ${cleanupError ?? 'result/readback mismatch'}`,
        )
      }
    } finally {
...
const test = base.extend<{ launch: OwnedLaunch }>({
  launch: [
    async ({ page }, provideLaunch) => {
      const launch = new OwnedLaunch(page)
      page.on('request', launch.onRequest)
      try {
        await provideLaunch(launch)
      } finally {
        try {
          await launch.settle()
        } finally {
          page.off('request', launch.onRequest)
        }
      }
    },
    { timeout: 35_000 },
...
async function selectProfileAndAwaitReadiness(page: Page, dialog: Locator) {
    ...
    .waitForResponse(
      (r) => {
        ...
        return (
          ...
          url.origin === ORIGIN &&
          url.pathname ===
            `/v1/m/sessions/provider-profiles/${PROFILE_REF}/launch-readiness` &&
          url.searchParams.get('transport') === 'stream-json' &&
          url.searchParams.get('isolation') === 'native'
        )
      },
      { timeout: 15_000 },
    )
  ...
  if ('error' in result)
    throw new Inability(`readiness response: ${result.error}`)
  ...
  expect(
    ['ready', 'unknown'],
    `observed readiness=${body.configuration_state}`,
  ).toContain(body.configuration_state)
  expect(
  ...
  await expect
    .poll(async () =>
      rows.evaluateAll((elements) =>
        elements.map((el) => ({
          check: el.getAttribute('data-check'),
...
test('New session is pointer-reachable and admits exactly one settled launch', async ({
  ...
  await signIn(page)
  expect(new URL(page.url()).origin, 'sign-in kept the configured origin').toBe(
    ORIGIN,
  )
  ...
  await test.step('missing profile keeps the control disabled and admits nothing', async () => {
    ...
    expect(stillDisabled, 'the SAME attached element is still disabled').toBe(
      true,
    )
    expect(launch.calls, 'no POST to the exact create endpoint').toBe(0)
    const after = matching(
      (await readChecked(api, RUNS_PATH, isRunList, 'negative-proof run list'))
        .items,
    )
    expect(after, 'no new run row matching this fixture identity').toEqual(
      before,
    )
  ...
  for (const vp of VIEWPORTS) {
    await test.step(vp.name, async () => {
      ...
      expect(region.documentScrolls, 'the document is not the scroller').toBe(
        false,
      )
      ...
      if (region.active > 0) {
        ...
        await owner.dispose()
        const controls = {
          cancel: await hitsSelf(
            page,
            dialog.getByRole('button', { name: /^cancel$/i }),
          ),
          close: await hitsSelf(
            page,
            dialog.getByRole('button', { name: /^close$/i }),
          ),
        ...
        console.log(
          `[after-wheel] ${vp.name} ${JSON.stringify({ ...after, sameOwner, controls })}`,
        )
        ...
        for (const [name, hit] of Object.entries(controls)) {
          expect(
            hit.isSelf,
            `${name} remains pointer-reachable after wheel`,
          ).toBe(true)
      ...
      for (const [label, hit] of [
        ...
        expect(
          ...
          `${label} bottom edge inside the viewport`,
        ).toBeLessThanOrEqual(vp.height)
        expect(
          hit.box!.y,
          `${label} top edge inside the viewport`,
        ).toBeGreaterThanOrEqual(0)
        expect(hit.isSelf, `elementFromPoint lands on ${label} itself`).toBe(
          true,
        )
      }
      ...
      const visible = await focusable.evaluateAll((elements) =>
        elements
          .filter(
            (el) =>
      ...
      expect(
        visible.length,
        'dialog has multiple focus targets',
      ).toBeGreaterThan(1)
      ...
      for (const key of ['Tab', 'Shift+Tab']) {
        for (let i = 0; i <= visible.length; i++) {
          ...
          const observation = await dialog.evaluate(
            (el, prior) => ({
              inside: el.contains(document.activeElement),
              moved: document.activeElement !== prior,
            }),
            previous,
          )
          ...
          expect(
            observation.inside,
            `${key} stays trapped across a full cycle`,
          ).toBe(true)
