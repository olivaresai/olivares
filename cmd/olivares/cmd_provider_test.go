...
// D19 CLI acceptance. These drive the real Cobra tree against a controlled
...
// Without --provider the profile is registered and NOT authorised for injection: the
// host's own variables decide, which is exactly the pre-D19 behaviour.
func TestAgentDeployWithoutProviderDoesNotAuthoriseInjection(t *testing.T) {
	...
	if _, ok := body["auth_source"]; ok {
		t.Fatalf("deploy must not authorise a source nobody asked for: %v", body)
	}
