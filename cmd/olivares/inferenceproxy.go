...
func (d *inferenceProxyDecider) runSizingAndBudget(ctx context.Context, req claudeapi.MessageRequest, id resolvedIdentity, sess *proxySession) (gateResult, bool) {
	...
	if pol.GateBudget {
		...
		// A PER-CALL key, because this caller is handed none. An idempotency key
		// makes a retry of one call cheap; it cannot make two calls one. The key
		// used to be the session reference plus a digest of the request, which is
		// the same string every time the same bytes arrive — so a second, genuinely
		// separate call with the same prompt presented itself as a retry of the
		// first and was admitted on the first call's verdict. The client sends no
		// idempotency header and the proxy cannot tell a retry from a repeat, so it
		// claims neither: every call asks the ledger its own question. The digest
		// is not lost — the row's payload hash still binds the key to the request
		// it admitted, and a key reused with different bytes is still a conflict.
