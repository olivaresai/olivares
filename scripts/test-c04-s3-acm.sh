...
#
# Battery for check-c04-s3-acm.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-s3-acm.sh"
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-s3-acm.sh"
	cp "$ROOT/design/c04-s3-acm.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-S3-ACM-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-s3-acm.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live S3+ACM pin is CLEAN"
else
...
import sys
p = Path(sys.argv[1])
p.write_text(p.read_text().replace('status = "Enabled"', 'status = "Suspended"', 1))
PY
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: versioning Suspended is FAIL"
else
	bad "suspended versioning should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: public ACLs allowed is FAIL"
else
	bad "public ACLs should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
i = 0
while i < len(lines):
    if 'resource "aws_acm_certificate" "alb"' in lines[i]:
        depth = 0
        started = False
        while i < len(lines):
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped ACM is FAIL"
else
	bad "dropped ACM should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/c04-s3-acm.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-S3-ACM-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/C04-S3-ACM-2026-08-20.md"
run
...
echo "check-c04-s3-acm selftest: $pass passed, $fail failed"
