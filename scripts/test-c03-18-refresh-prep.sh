...
CHECK="$ROOT/scripts/check-c03-18-refresh-prep.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c0318prep.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0; fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
bad() { printf 'FAIL %s\n' "$1" >&2; fail=$((fail + 1)); }
...
stage() {
  ...
  cp "$ROOT/design/c03-18-refresh-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C03-18-REFRESH-PREP-2026-08-20.md" "$TMP/tree/design/"
  cp "$ROOT/commercial/license-worker/src/index.ts" \
    "$TMP/tree/commercial/license-worker/src/"
  cp "$CHECK" "$TMP/tree/scripts/"
  chmod +x "$TMP/tree/scripts/check-c03-18-refresh-prep.sh"
}
run() {
  local rc=0
  unset OLIVARES_ENT_DIR || true
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-c03-18-refresh-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe refresh pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c03-18-refresh-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
stage
printf 'export {}\n' > "$TMP/tree/commercial/license-worker/src/license/refresh.ts"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (refresh.ts landed) is killed"
else bad "refresh.ts stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
...
stage
rm -f "$TMP/tree/design/c03-18-refresh-prep-2026-08-20.json"
run
...
echo "check-c03-18-refresh-prep selftest: $pass passed, $fail failed"
