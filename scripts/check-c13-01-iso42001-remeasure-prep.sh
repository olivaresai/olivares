...
# check-c13-01-iso42001-remeasure.sh: hub-safe HOLD so lint:addon-sets
# does not LOOK 2 without OLIVARES_ENT_DIR.
# Three answers: 0 CLEAN · 1 finding · 2 could not look.

set -euo pipefail
say() { printf '%s\n' "$*"; }
fail() { say "check-c13-01-iso42001-remeasure-prep: FAIL — $*" >&2; exit 1; }
cannot() { say "check-c13-01-iso42001-remeasure-prep: COULD NOT LOOK — $*" >&2; exit 2; }

ROOT="${OLIVARES_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
cd "$ROOT" || cannot "cannot enter $ROOT"

JSON="${OLIVARES_C1301RP_JSON:-design/c13-01-iso42001-remeasure-prep-2026-08-20.json}"
DOC="${OLIVARES_C1301RP_DOC:-design/C13-01-ISO42001-REMEASURE-PREP-2026-08-20.md}"
SOLD="${OLIVARES_C1301RP_SOLD:-commercial/module-slug-package.json}"
...
grep -F -q 'Unique leftover unique vs `check-c13-01-iso42001-remeasure.sh`' "$DOC" \
  || fail "prepare doc lost uniqueness vs overlay-gated remasure check"
...
def fail(msg):
    print(f"check-c13-01-iso42001-remeasure-prep: FAIL — {msg}", file=sys.stderr)
    sys.exit(1)

def cannot(msg):
    print(f"check-c13-01-iso42001-remeasure-prep: COULD NOT LOOK — {msg}", file=sys.stderr)
    sys.exit(2)
...
if data.get("schema") != "c13-01-iso42001-remeasure-prep/v1":
...
say "check-c13-01-iso42001-remeasure-prep: CLEAN — remasure HOLD; hub-safe; overlay not remasured."
exit 0
