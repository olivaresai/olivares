...
CHECK="$ROOT/scripts/check-cfg-04-business-fence-prep.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/cfg04prep.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0; fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
bad() { printf 'FAIL %s\n' "$1" >&2; fail=$((fail + 1)); }
...
stage() {
  ...
  cp "$ROOT/design/cfg-04-business-fence-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/CFG-04-BUSINESS-FENCE-PREP-2026-08-20.md" "$TMP/tree/design/"
  ...
  chmod +x "$TMP/tree/scripts/check-cfg-04-business-fence-prep.sh"
}
run() {
  local rc=0
  unset OLIVARES_ENT_DIR || true
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-cfg-04-business-fence-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe business-fence pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/cfg-04-business-fence-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
stage
printf '\nexport function expectedDodoBusinessId() { return null }\n' >> \
  "$TMP/tree/commercial/license-worker/src/env.ts"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (expectedDodoBusinessId landed) is killed"
else bad "expectedDodoBusinessId stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
...
stage
rm -f "$TMP/tree/design/cfg-04-business-fence-prep-2026-08-20.json"
run
...
echo "check-cfg-04-business-fence-prep selftest: $pass passed, $fail failed"
