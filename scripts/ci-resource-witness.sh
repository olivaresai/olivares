...
#
# WHAT IT GUARANTEES. Between `start` and `report` the sampler keeps three running maxima
# in a state file: the most memory the MACHINE had in use, the most processes it carried,
# and the least free space the measured filesystem had. `report` prints them whatever
# killed the step, because it reads the file and never the sampler — a sampler cut down
# with the rest of the process group still leaves its last sample behind.
#
# WHAT IT IS NOT. It measures the WHOLE machine, not this job's process tree: on a hosted
# runner the job is the machine, and on a shared runner the number is honest about the
# neighbours instead of hiding them. It does not decide anything and never fails a step:
# a witness that can fail a job is a witness nobody dares to install.
#
