...
PY
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (overlay_matches_sold true) is killed"
else bad "matches true stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
printf '\ncatalog matches sold\n' >> \
  "$TMP/tree/design/C13-05-CATALOG-DIVERGE-PREP-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (doc claims match) is killed"
else bad "doc match stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
