...
# real binary and assert the evidence rows. It is the D17 reproducibility
...
printf '%s\n' "$ALLOW_EV" | grep -q 'hook.tool.allow' || fail "no hook.tool.allow evidence row"
printf '%s\n' "$DENY_EV" | grep -q 'hook.tool.deny' || fail "no hook.tool.deny evidence row"
printf '%s\n' "$ALLOW_EV" | grep -q 'count 0' && fail "allow evidence count is 0"
printf '%s\n' "$DENY_EV" | grep -q 'count 0' && fail "deny evidence count is 0"
