...
FAST_LINTS=(
	...
	lint:ci-timeout-arithmetic lint:ci-timeout-arithmetic:selftest lint:ci-step-guard lint:ci-step-guard:selftest lint:ci-a11y-gate-wrap:selftest lint:ci-inner-clocks lint:ci-inner-clocks:selftest lint:ci-sibling-legs lint:ci-sibling-legs:selftest lint:pr-suite-shards lint:pr-suite-shards:selftest lint:pr-ci-regime lint:pr-ci-regime:selftest
	...
	lint:openapi-op-descriptions lint:runbook-coverage lint:reserve-numbers:selftest lint:release-diagrams lint:claim-safety:selftest lint:classify-paths-parity lint:classify-paths-parity:selftest lint:publisher-contract lint:publisher-contract:selftest lint:roles-oneshot lint:aws-iam-phase2:selftest lint:export-closure:selftest lint:ci-int12-step:selftest lint:pre-verify-tanda:selftest lint:refs-file-readers:selftest lint:scan-box-secrets:selftest lint:fabricated-paths lint:fabricated-paths:selftest lint:purge-scratch:selftest lint:race-report:selftest lint:race-groups:selftest lint:race-full-services-parity lint:modules-race-partition lint:hot-race-partition lint:release-smoke-answers lint:release-smoke-stamp lint:session-record:selftest lint:launch-placeholders:selftest lint:overlay-seal:selftest lint:export-identity:selftest lint:public-prose:selftest lint:baseline-shrink:selftest lint:cra-pack:selftest lint:guard-bash lint:prepush-treesnap lint:catalog-dodo-discount lint:prepush-refclass
)
