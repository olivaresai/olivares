...
#
# Battery for check-c04-tls-headers.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-tls-headers.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c04tls.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
LINE='enable_tls_version_and_cipher_suite_headers = true'
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-tls-headers.sh"
	cp "$ROOT/design/c04-tls-headers.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-TLS-HEADERS-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-tls-headers.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live TLS-headers pin is CLEAN"
else
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: TLS headers false is FAIL"
else
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped TLS headers is FAIL"
else
	bad "dropped flag should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/c04-tls-headers.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-TLS-HEADERS-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/c04-tls-headers.json"
run
...
echo "check-c04-tls-headers selftest: $pass passed, $fail failed"
