...
#
# provider-onboarding-smoke.sh — replay the provider onboarding path headless against the REAL
...
CANARY="sk-provider-smoke-CANARY-0123456789-ABCD"
...
# The profile always sets HOME for the child; the default only keeps a missing HOME from being an
# unbound-variable death: writing under a directory that does not exist fails by name instead.
env > "${HOME:-/nonexistent-profile-home}/provider-smoke-child.env"
printf '{"type":"system","subtype":"init","session_id":"provider-smoke"}\n'
sleep 30
SH
...
token_rc=0
SETUP_TOKENS="$(grep -oE 'olst_[A-Z0-9]+' "$WORK/engine.log")" || token_rc=$?
[ "$token_rc" -le 1 ] || {
  echo "COULD NOT LOOK: could not read $WORK/engine.log for the setup token (grep rc=$token_rc)" >&2
  exit 2
}
...
grep -q "$CANARY" <<<"$ADD_JSON" && fail "the create response echoed the credential"
...
grep -q "$CANARY" <<<"$TEST_JSON" && fail "the test response echoed the credential"
...
#        wired (deny-closed)"
# That gate is CORRECT and is not this script's subject. A read-only mount is the
# ordinary launch, which is what the provider path has to be proven on.
WS_JSON="$("${CLI[@]}" agent workspace add "$WS_DIR" --name provider-smoke --mode ro -o json)" \
...
RUN_JSON="$("${CLI[@]}" agent session create --name provider-smoke-1 --workspace "$WS_REF" \
  --provider-profile "$PROFILE" -o json)" || fail "session create failed"
RUN="$(printf '%s' "$RUN_JSON" | jqp 'd["run_ref"]')"
ok "run $RUN"

for _ in $(seq 1 100); do [ -s "$HOME_DIR/provider-smoke-child.env" ] && break; sleep 0.1; done
[ -s "$HOME_DIR/provider-smoke-child.env" ] || fail "the child never recorded its environment"
...
assert_eq "the child received the registered credential" \
  "$(grep -c "^OPENAI_API_KEY=$CANARY$" "$HOME_DIR/provider-smoke-child.env" || true)" "1"
assert_eq "the child received the provider's endpoint" \
  "$(grep -c "^OPENAI_BASE_URL=https://127.0.0.1:$DOUBLE_PORT$" "$HOME_DIR/provider-smoke-child.env" || true)" "1"
assert_eq "no host inference bearer was injected" \
  "$(grep -c '^ANTHROPIC_AUTH_TOKEN=' "$HOME_DIR/provider-smoke-child.env" || true)" "0"
...
REFUSAL="$("${CLI[@]}" agent session create --name provider-smoke-2 --workspace "$WS_REF" \
  --provider-profile "$PROFILE" 2>&1 || true)"
grep -qi "revoked" <<<"$REFUSAL" \
