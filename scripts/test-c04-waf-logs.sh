...
#
# Battery for check-c04-waf-logs.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-waf-logs.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c04waf.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-waf-logs.sh"
	cp "$ROOT/design/c04-waf-logs.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-WAF-LOGS-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-waf-logs.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live WAF log pin is CLEAN"
else
...
p = Path(sys.argv[1])
text = p.read_text()
start = text.find('resource "aws_wafv2_web_acl_logging_configuration" "alb"')
if start < 0:
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped logging_configuration is FAIL"
else
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: /olivares/ log group name is FAIL"
else
	bad "wrong log group name should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: Firehose destination is FAIL"
else
	bad "Firehose dest should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/c04-waf-logs.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-WAF-LOGS-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/c04-waf-logs.json"
run
...
echo "check-c04-waf-logs selftest: $pass passed, $fail failed"
