...
# check-c13-05-overlay-catalog-diverge.sh: hub-safe HOLD so
...
grep -q 'Overlay main does not match the sold cards' "$DOC" \
  || fail "prepare doc lost overlay-main diverge"
...
if data.get("schema") != "c13-05-catalog-diverge-prep/v1":
    fail("unknown schema %r" % data.get("schema"))
if data.get("overlay_matches_sold") is not False:
    fail("overlay_matches_sold must stay false")
...
if hold != {"caeptransmit", "circuit-breaker"}:
    fail("HOLD doc drifted from named airs slugs")
print("json-ok")
PY

say "check-c13-05-catalog-diverge-prep: CLEAN — diverge HOLD; hub-safe; overlay not remasured."
exit 0
