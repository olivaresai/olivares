...
CHECK="$ROOT/scripts/check-c13-01-iso42001-remeasure-prep.sh"
...
stage() {
  ...
  cp "$ROOT/design/c13-01-iso42001-remeasure-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C13-01-ISO42001-REMEASURE-PREP-2026-08-20.md" "$TMP/tree/design/"
  cp "$ROOT/commercial/module-slug-package.json" "$TMP/tree/commercial/"
  cp "$CHECK" "$TMP/tree/scripts/"
  chmod +x "$TMP/tree/scripts/check-c13-01-iso42001-remeasure-prep.sh"
}
run() {
  local rc=0
  unset OLIVARES_ENT_DIR || true
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-c13-01-iso42001-remeasure-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe remasure HOLD is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c13-01-iso42001-remeasure-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
PY
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (iso42001_in_catalog true) is killed"
else bad "catalog true stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
printf '\niso42001 catalog landed\n' >> \
  "$TMP/tree/design/C13-01-ISO42001-REMEASURE-PREP-2026-08-20.md"
run
...
stage
rm -f "$TMP/tree/design/c13-01-iso42001-remeasure-prep-2026-08-20.json"
run
...
echo "check-c13-01-iso42001-remeasure-prep selftest: $pass passed, $fail failed"
