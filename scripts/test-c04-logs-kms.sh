...
#
# Battery for check-c04-logs-kms.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-logs-kms.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c04log.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-logs-kms.sh"
	cp "$ROOT/design/c04-logs-kms.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-LOGS-KMS-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-logs-kms.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live logs CMK pin is CLEAN"
else
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped one log-group bind is FAIL"
else
...
stage
python3 - "$TMP/tree/design/c04-logs-kms.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-LOGS-KMS-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/c04-logs-kms.json"
run
...
echo "check-c04-logs-kms selftest: $pass passed, $fail failed"
