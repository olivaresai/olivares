...
CHECK="$ROOT/scripts/check-cfg-18-helm-claims-prep.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/cfg18prep.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0; fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
bad() { printf 'FAIL %s\n' "$1" >&2; fail=$((fail + 1)); }
...
stage() {
  ...
  cp "$ROOT/design/cfg-18-helm-claims-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/CFG-18-HELM-CLAIMS-PREP-2026-08-20.md" "$TMP/tree/design/"
  cp "$ROOT/scripts/check-helm-claims.sh" "$TMP/tree/scripts/"
  cp "$CHECK" "$TMP/tree/scripts/"
  chmod +x "$TMP/tree/scripts/check-helm-claims.sh" \
    "$TMP/tree/scripts/check-cfg-18-helm-claims-prep.sh"
...
run() {
  ...
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-cfg-18-helm-claims-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe helm-claims pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/cfg-18-helm-claims-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
cat >"$TMP/tree/Taskfile.yml" <<'EOF'
  lint:addon-sets:
    cmds:
      - bash scripts/check-helm-claims.sh
...
stage
printf '\nFIRMA A claimed\n' >> \
  "$TMP/tree/design/CFG-18-HELM-CLAIMS-PREP-2026-08-20.md"
run
...
stage
rm -f "$TMP/tree/design/cfg-18-helm-claims-prep-2026-08-20.json"
run
...
echo "check-cfg-18-helm-claims-prep selftest: $pass passed, $fail failed"
