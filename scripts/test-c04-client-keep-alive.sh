...
#
# Battery for check-c04-client-keep-alive.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-client-keep-alive.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c04ka.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-client-keep-alive.sh"
	cp "$ROOT/design/c04-client-keep-alive.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-CLIENT-KEEP-ALIVE-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-client-keep-alive.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live keep-alive pin is CLEAN"
else
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: client_keep_alive 60 is FAIL"
else
	bad "60 should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped client_keep_alive is FAIL"
else
	bad "dropped flag should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/c04-client-keep-alive.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-CLIENT-KEEP-ALIVE-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/c04-client-keep-alive.json"
run
...
echo "check-c04-client-keep-alive selftest: $pass passed, $fail failed"
