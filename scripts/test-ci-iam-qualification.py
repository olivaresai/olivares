...
# Protocol controls run against the first registered family; each known family's
# registry shape fixes its phase count (clean, race, then three per mutant).
FAMILY = IAM.FAMILIES[0]
PHASES = {"scim": 18, "workspace": 11, "ldap": 8}
...
class Fixture:
    def __init__(self, family=FAMILY, mode="valid", alteration=None, variant_inputs=None):
...
class CommandControls(unittest.TestCase):
    def run_case(self, mode="valid", alteration=None, expected=2, family=FAMILY):
    ...
    def test_registered_families_pass(self):
        for family in IAM.FAMILIES:
            with self.subTest(family=family):
                ...
                self.assertEqual(len(record["phases"]), PHASES[family])
    ...
    @unittest.skipUnless("ldap" in IAM.FAMILIES, "the certificate wire leaf is an LDAP case")
    def test_named_wire_leaf_is_required(self):
        ...
        for mode, reason in (("missing_wire", "missing-terminal"), ("skip_wire", "skip-contract")):
            with self.subTest(mode=mode):
                record = self.run_case(mode, family="ldap")
    ...
    def test_unregistered_family_refuses_at_entry(self):
        ...
        self.assertEqual(IAM.FAMILIES, ("workspace",))
        for family in sorted(set(PHASES) - set(IAM.FAMILIES)):
            with self.subTest(family=family):
                fixture = Fixture()
                try:
    ...
    def test_stop_at_phase_entry_refuses(self):
        qualification = IAM.Qualification(FAMILY, Path("unused-no-write"))
        qualification.stop()
    ...
    def test_missing_terminal_guard_is_causal(self):
        ...
        command = json.loads((ROOT / ("ci/iam-qualification/" + FAMILY + ".json")).read_text())["commands"]["baseline"]
