...
#
# Battery for check-c04-waf-fail-open.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-c04-waf-fail-open.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c04fo.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-c04-waf-fail-open.sh"
	cp "$ROOT/design/c04-waf-fail-open.json" "$TMP/tree/design/"
	cp "$ROOT/design/C04-WAF-FAIL-OPEN-2026-08-20.md" "$TMP/tree/design/"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-c04-waf-fail-open.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live fail-open pin is CLEAN"
else
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: enable_waf_fail_open true is FAIL"
else
	bad "fail-open true should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: dropped enable_waf_fail_open is FAIL"
else
	bad "dropped fail-open should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/c04-waf-fail-open.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
stage
echo 'estate applied' >>"$TMP/tree/design/C04-WAF-FAIL-OPEN-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/c04-waf-fail-open.json"
run
...
echo "check-c04-waf-fail-open selftest: $pass passed, $fail failed"
