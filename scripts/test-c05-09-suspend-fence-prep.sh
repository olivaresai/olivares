...
stage() {
  ...
  cp "$ROOT/design/c05-09-suspend-fence-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C05-09-SUSPEND-FENCE-PREP-2026-08-20.md" "$TMP/tree/design/"
  cp "$ROOT/cloud/control-plane/internal/billing/fence.go" \
    "$TMP/tree/cloud/control-plane/internal/billing/"
  cp "$ROOT/cloud/control-plane/internal/tenant/manager.go" \
    "$TMP/tree/cloud/control-plane/internal/tenant/"
  cp "$ROOT/cloud/control-plane/internal/billing/polar.go" \
    "$TMP/tree/cloud/control-plane/internal/billing/"
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe suspend-fence pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c05-09-suspend-fence-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
stage
cat >>"$TMP/tree/cloud/control-plane/internal/billing/fence.go" <<'EOF'
func EventNotStale() {}
EOF
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (EventNotStale landed) is killed"
else bad "EventNotStale stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
sed -i 's/func (m \*Manager) Suspend(ctx context.Context, polarCustomerID, reason, polarEventID string)/func (m *Manager) Suspend(ctx context.Context, polarCustomerID, subscriptionID, reason, polarEventID string)/' \
  "$TMP/tree/cloud/control-plane/internal/tenant/manager.go"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (Suspend takes subscriptionID) is killed"
else bad "Suspend signature stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
sed -i 's/billing.BelongingSubscription(t.PolarSubscriptionID, subscriptionID)/billing.BelongingSubscription(nil, subscriptionID)/' \
  "$TMP/tree/cloud/control-plane/internal/tenant/manager.go"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (UpdatePlan fence dropped) is killed"
else bad "UpdatePlan fence stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
...
stage
rm -f "$TMP/tree/design/c05-09-suspend-fence-prep-2026-08-20.json"
run
