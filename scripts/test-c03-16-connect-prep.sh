...
stage() {
  ...
  cp "$ROOT/design/c03-16-connect-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C03-16-CONNECT-PREP-2026-08-20.md" "$TMP/tree/design/"
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe connect pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c03-16-connect-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
printf 'export function handleConnect() {}\n' > \
  "$TMP/tree/commercial/license-worker/src/connect/handler.ts"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (handler.ts landed) is killed"
else bad "handler.ts stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
...
stage
rm -f "$TMP/tree/design/c03-16-connect-prep-2026-08-20.json"
run
