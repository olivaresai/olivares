...
#
# Battery for scripts/modules-race-partition.sh and for the CI wiring that consumes it.
#
# A. The rule. Every package `go list ./...` discovers inside ./modules is owned by exactly
#    one partition. A package owned by nobody is never raced and all four jobs still go
#    green, which is the failure here that is worse than a red, so the property is checked
#    on the live inventory, on synthetic inventories holding packages this tree does not
#    have, and against mutants of the rule itself: a control only ever seen to pass has
#    never been seen to work. The selector refusals belong here too — a selector the shell
#    cannot evaluate must be refused, not skipped.
#
...
#
# D. The job's environment. race-modules exports the selector pair AND the dedicated-owner
#    exclusion (R111-SR1) for the whole job, so this battery inherits all three in every
#    partition. The cases that mean "no selector" share run(), which unsets them; each state
#    the job can hand down is exported against that seam and against a bare child that
#    bypasses it, which must still see the state. A case that means a REGIME states it: the
#    exclusion changes what a selection owns, so cases (4)-(4d) set or unset it explicitly
#    rather than measuring whatever the job left in the environment. Inheriting it silently
#    is what reddened job 103739990101 on a tree whose selection logic was correct.
#
...
SUT="${ROOT}/scripts/modules-race-partition.sh"
WF="${ROOT}/.github/workflows/mainline-ci.yml"
TASKFILE="${ROOT}/Taskfile.yml"

[ -r "${SUT}" ] || { printf 'test-modules-race-partition: COULD NOT LOOK — %s is unreadable\n' "${SUT}" >&2; exit 2; }
[ -r "${WF}" ] || { printf 'test-modules-race-partition: COULD NOT LOOK — %s is unreadable\n' "${WF}" >&2; exit 2; }
...
blind() {
	printf 'test-modules-race-partition: COULD NOT LOOK — %s\n' "$*" >&2
	exit 2
}
...
# The SUT runs with NEITHER selector set and NO dedicated-owner exclusion, whatever this
# battery inherited. Every case that means "no selector" goes through here, and race-modules
# exports all three for its whole job, so without the unset those cases race the partition
# that hosts them (section D). A case that wants a selector sets both halves on a direct
# `bash` command, as (4), refusal() and (24) do; a case that wants the exclusion sets it
# there too, as (4b) does.
run() {
	local sut="$1"
	shift
	local rc=0
	(
		unset OLIVARES_MODULES_RACE_PARTITION OLIVARES_MODULES_RACE_PARTITIONS OLIVARES_MODULES_RACE_DEDICATED
		exec bash "${sut}" "$@"
...
ok "the live ./modules inventory is discoverable" "${INV_N} package(s)"
...
if [ "${CENSUS_N:-0}" = "${INV_N}" ]; then
	ok "the census counts sum to the inventory" "${CENSUS_N} = ${INV_N}"
else
	bad "the census counts do not sum to the inventory" "census=${CENSUS_N:-?} inventory=${INV_N}"
fi
...
#
#     The regime is DECLARED, not inherited: `env -u` drops any dedicated-owner exclusion the
#     job exported, because this case compares four selections against the COMPLETE inventory
#     and an exclusion legitimately removes a package from those selections. Inheriting it
#     turned this row red in job 103739990101 while the selection logic was correct; the
#     exclusion's own regime is (4b)-(4d) below, which is where it is proved instead of
#     silently weakening this one.
: >"${TMP}/union"
i=1
while [ "${i}" -le 4 ]; do
	env -u OLIVARES_MODULES_RACE_DEDICATED \
		OLIVARES_MODULES_RACE_PARTITION="${i}" OLIVARES_MODULES_RACE_PARTITIONS=4 \
...
if cmp -s "${TMP}/union" "${TMP}/inv.sorted"; then
	ok "the four selections, concatenated, ARE the inventory" "$(grep -c . "${TMP}/union") = ${INV_N}"
else
	bad "the four selections are not the inventory" "$(diff "${TMP}/inv.sorted" "${TMP}/union" | head -3 | tr '\n' ' ')"
fi
...
# (4b) THE DEDICATED REGIME, exactly as race-modules hands it down (R111-SR1). The value is
#      READ FROM THE WORKFLOW rather than typed here, so this case measures the state the real
#      job exports; a hardcoded copy would keep passing after the job stopped exporting it.
#      Under that state the package must appear in NONE of the four selections: it is owned by
#      the race-sessions job, and a partition that also carried it would race it twice.
DEDICATED="$(grep -oE '^[[:space:]]*OLIVARES_MODULES_RACE_DEDICATED:[[:space:]]*[^[:space:]]+' "${WF}" | head -1 | grep -oE '[^[:space:]]+$' || true)"
[ -n "${DEDICATED}" ] || blind "race-modules' OLIVARES_MODULES_RACE_DEDICATED could not be read from ${WF}; cases (4b)-(4d) measure the state that job exports"
grep -qxF "${DEDICATED}" "${TMP}/inv" || blind "the workflow excludes '${DEDICATED}', which is not in the live ./modules inventory; that exclusion would drop nothing today and the wrong thing tomorrow"
: >"${TMP}/union.dedicated"
carried=""
unannounced=""
i=1
while [ "${i}" -le 4 ]; do
	OLIVARES_MODULES_RACE_DEDICATED="${DEDICATED}" \
		OLIVARES_MODULES_RACE_PARTITION="${i}" OLIVARES_MODULES_RACE_PARTITIONS=4 \
		bash "${SUT}" --packages >"${TMP}/sel.${i}" 2>"${TMP}/sel.${i}.err" ||
		blind "partition ${i} of 4 could not be selected under the dedicated owner '${DEDICATED}'"
	if grep -qxF "${DEDICATED}" "${TMP}/sel.${i}"; then carried="${carried} ${i}"; fi
	grep -q 'dedicated owner:' "${TMP}/sel.${i}.err" || unannounced="${unannounced} ${i}"
	cat "${TMP}/sel.${i}" >>"${TMP}/union.dedicated"
	i=$((i + 1))
done
if [ -z "${carried}" ] && [ -z "${unannounced}" ]; then
	ok "the dedicated package is in NONE of the four selections" "${DEDICATED##*/}, indexes 1-4, each announced"
elif [ -n "${carried}" ]; then
	bad "a partition carries the dedicated package" "index(es)${carried} would race ${DEDICATED} a second time"
else
	bad "a partition excluded the dedicated package SILENTLY" "index(es)${unannounced} printed no 'dedicated owner:' line"
fi

# (4c) NO OMISSION, the property R111-SR1 rests on: the four partitions the race-modules jobs
#      run, plus the one package the race-sessions job owns, are the inventory EXACTLY. A
#      package dropped here is never raced by anybody and all five jobs still go green, which
#      is the failure worse than a red. Derived from --packages alone, never from --check.
LC_ALL=C sort -o "${TMP}/union.dedicated" "${TMP}/union.dedicated"
{ cat "${TMP}/union.dedicated"; printf '%s\n' "${DEDICATED}"; } | LC_ALL=C sort >"${TMP}/union.whole"
DEDUPS="$(LC_ALL=C uniq -d <"${TMP}/union.whole" | head -2 | tr '\n' ' ')"
if cmp -s "${TMP}/union.whole" "${TMP}/inv.sorted" && [ -z "${DEDUPS}" ]; then
	ok "the four partitions PLUS the dedicated job ARE the inventory" "$(grep -c . "${TMP}/union.dedicated") + 1 = ${INV_N}, no duplicate"
else
	bad "the four partitions plus the dedicated job are not the inventory" "$(diff "${TMP}/inv.sorted" "${TMP}/union.whole" | head -3 | tr '\n' ' ')dup: ${DEDUPS:-none}"
fi

# (4d) CONTROL POSITIVE: without the exclusion the same four selections DO carry that package,
#      in exactly one of them. Otherwise (4b) would hold for a package this tree simply lacks,
#      and (4c) would be arithmetic over an exclusion that never happened.
#      Custody as in (4b): the selection is COMPLETED into a file and its status read, then
#      the file is searched. `bash "${SUT}" --packages | grep -qxF` answers two questions with
#      one number and gets both wrong — a MATCH closes the pipe, the producer takes SIGPIPE and
#      the pipeline returns 141, so the control positive counts a carrying partition as not
#      carrying; and a selection that FAILED returns non-zero too, so a broken SUT would read as
#      "0 of 4 carry it" and redden this case with the wrong cause.
CARRYING=0
i=1
while [ "${i}" -le 4 ]; do
	env -u OLIVARES_MODULES_RACE_DEDICATED \
		OLIVARES_MODULES_RACE_PARTITION="${i}" OLIVARES_MODULES_RACE_PARTITIONS=4 \
		bash "${SUT}" --packages >"${TMP}/unexcluded.${i}" 2>/dev/null ||
		blind "partition ${i} of 4 could not be selected WITHOUT the dedicated owner; a failed producer is not a membership answer"
	if grep -qxF -- "${DEDICATED}" "${TMP}/unexcluded.${i}"; then CARRYING=$((CARRYING + 1)); fi
	i=$((i + 1))
done
if [ "${CARRYING}" = 1 ]; then
	ok "control positive: unexcluded, one partition does carry it" "1 of 4 owns ${DEDICATED##*/}"
else
	bad "control positive FAILED: the exclusion is not what (4b) measured" "${CARRYING} of 4 carry ${DEDICATED} without it"
fi
...
#     none of them in the cost table, all of them under the same module path.
{
	cat "${TMP}/inv"
	printf '%s\n' \
		'github.com/olivaresai/olivares/modules/futuretelemetry' \
		'github.com/olivaresai/olivares/modules/futuretelemetry/ingest' \
		'github.com/olivaresai/olivares/modules/zzz-last-alphabetically'
} >"${TMP}/future"
...
refusal() { # refusal <label> <expected-rc> <env-index> <env-count>
	...
	OLIVARES_MODULES_RACE_PARTITION="${idx}" OLIVARES_MODULES_RACE_PARTITIONS="${cnt}" \
...
rc=0
OLIVARES_MODULES_RACE_PARTITION="${HUGE}" OLIVARES_MODULES_RACE_PARTITIONS=4 \
	bash "${SUT}" --packages >"${TMP}/out" 2>"${TMP}/err" || rc=$?
...
# `echo` stands in for `go test`: the argv assembly is the thing under test, and running the
# real suite here would cost hours and prove nothing this does not.
...
rc=0
OLIVARES_MODULES_RACE_PARTITION=3 OLIVARES_MODULES_RACE_PARTITIONS=4 \
	bash "${SUT}" echo >"${TMP}/out" 2>"${TMP}/err" || rc=$?
WANT_N="$(OLIVARES_MODULES_RACE_PARTITION=3 OLIVARES_MODULES_RACE_PARTITIONS=4 bash "${SUT}" --packages 2>/dev/null | grep -c . || true)"
GOT_N="$(tr ' ' '\n' <"${TMP}/out" | grep -c 'github.com/olivaresai/olivares/modules' || true)"
...
#     exact recipe it had.
rc=0
OLIVARES_MODULES_RACE_PARTITION=2 OLIVARES_MODULES_RACE_PARTITIONS=4 \
	bash "${SUT}" echo >"${TMP}/out" 2>"${TMP}/err" || rc=$?
...
# (25) The argv runs INSIDE ./modules. `go test` resolves `./...` from the working
#      directory, so this is the difference between racing ./modules and racing the root.
run "${SUT}" pwd
if [ "$(rc)" = 0 ] && [ "$(head -1 "${TMP}/out")" = "${ROOT}/modules" ]; then
	ok "the argv runs inside ./modules" "$(head -1 "${TMP}/out" | tail -c 30)"
else
	bad "the argv should run inside ./modules" "got '$(head -1 "${TMP}/out")'"
fi
...
printf 'github.com/olivaresai/olivares/modules/PKGDROP\n' >"${TMP}/mut.inv"
cat "${TMP}/inv" >>"${TMP}/mut.inv"

# (28) A package that reaches NO partition: the failure that goes green without this control.
if mutate drop 's|^\t\tPLACED="${PLACED}${best}:${w}:${p}".*|\t\tif [ "${p}" != "github.com/olivaresai/olivares/modules/PKGDROP" ]; then PLACED="${PLACED}${best}:${w}:${p}"$'"'"'\\n'"'"'; fi|'; then
	rc=0
...
if mutate double 's|^\t\tPLACED="${PLACED}${best}:${w}:${p}".*|\t\tPLACED="${PLACED}${best}:${w}:${p}"$'"'"'\\n'"'"'; if [ "${p}" = "github.com/olivaresai/olivares/modules/PKGDROP" ]; then PLACED="${PLACED}1:${w}:${p}"$'"'"'\\n'"'"'; fi|'; then
	rc=0
...
wiring() {
	...
	[ "${n}" -ge 2 ] || {
		printf 'race-modules declares no partition matrix\n'
		findings=$((findings + 1))
	}
	...
	has 'OLIVARES_MODULES_RACE_PARTITION: ' "${mods}" || {
		printf 'race-modules does not pass the partition index to the task\n'
		findings=$((findings + 1))
	}
	...
	local declared num
	declared="$(grep -oE 'OLIVARES_MODULES_RACE_PARTITIONS:.*' "${mods}" | head -1 || true)"
	if [ -z "${declared}" ]; then
		printf 'race-modules does not declare a partition count\n'
		findings=$((findings + 1))
	...
	has 'continue-on-error' "${mods}" && {
		printf 'race-modules has a continue-on-error step\n'
		findings=$((findings + 1))
	}
	...
	has 'ci-fail-race-modules-.*matrix.partition' "${mods}" || {
	...
	has 'needs:.*race-modules' "${agg}" || {
		printf 'the race-hot aggregator does not depend on race-modules\n'
		findings=$((findings + 1))
	}
	has 'RESULT_MODULES: ${{ needs.race-modules.result }}' "${agg}" || {
		printf 'the aggregator does not read race-modules.result\n'
		findings=$((findings + 1))
	}
	has 'race-modules=$RESULT_MODULES' "${agg}" || {
		printf 'the aggregator does not judge race-modules in its verdict loop\n'
		findings=$((findings + 1))
	}
...
wf_mutant "a concurrency group without matrix.partition is caught" \
	's|group: mainline-ci-race-modules-${{ matrix.partition }}|group: mainline-ci-race-modules|' \
	'cancel each other'
wf_mutant "fail-fast: true is caught" \
	'/^  race-modules:/,/^    runs-on:/ s|fail-fast: false|fail-fast: true|' \
	'fail-fast'
wf_mutant "a declared count that is not the matrix length is caught" \
	's|OLIVARES_MODULES_RACE_PARTITIONS: ${{ strategy.job-total }}|OLIVARES_MODULES_RACE_PARTITIONS: "3"|' \
	'is not the matrix length'
wf_mutant "an aggregator that stops judging race-modules is caught" \
	's|"race-modules=$RESULT_MODULES" |"race-rest=$RESULT_REST" |' \
	'verdict loop'
wf_mutant "a shared failure-log name is caught" \
	's|ci-fail-race-modules-${{ matrix.partition }}.log|ci-fail-race-modules.log|g' \
...
if grep -q 'with-pg-env.sh bash scripts/modules-race-partition.sh go test -race' "${TASKFILE}"; then
	ok "the Taskfile recipe runs the partition helper through the wrapper" "test:race-hot:modules"
else
...
stream_gap() {
	...
	while IFS= read -r line; do
		...
		last="${stamp}"
		printf '%s %s\n' "${stamp}" "${line}"
	done < <(cd "${STREAM}" && GOWORK=off GOFLAGS= go test -count=1 -p "${width}" -v ./a ./b 2>&1) >"${TMP}/stream-p${width}.log"
	if [ -z "${first}" ]; then
		STREAM_GAP=""
		return 1
	fi
...
if stream_gap 1 && [ -n "${STREAM_GAP}" ]; then
	...
	blind "the width-1 streaming probe produced no output"
fi
...
if stream_gap 2 && [ -n "${STREAM_GAP}" ]; then
	...
	blind "the width-2 streaming probe produced no output"
fi

# ── D. THE JOB'S ENVIRONMENT IS NOT A FIXTURE ────────────────────────────────────────────
# race-modules exports the selector pair for the whole job, and this battery is a step of that
...
jobblock "${WF}" race-modules >"${TMP}/ambient-job"
...
MATRIX_N="${#MATRIX[@]}"
[ "${MATRIX_N}" -ge 2 ] || blind "the race-modules matrix could not be read from ${WF}; section D exports its values"
...
# unset. The dedicated-owner exclusion is dropped here so these cases measure the PAIR alone:
# the exclusion is a regime of its own, proved in (4b)-(4d), and inheriting it would make a
# bare child's selection shorter for a reason none of these cases is about. Nothing leaks back
# into the battery.
ambient() {
	...
	(
		unset OLIVARES_MODULES_RACE_PARTITION OLIVARES_MODULES_RACE_PARTITIONS OLIVARES_MODULES_RACE_DEDICATED
		[ "${idx}" = - ] || export OLIVARES_MODULES_RACE_PARTITION="${idx}"
		[ "${cnt}" = - ] || export OLIVARES_MODULES_RACE_PARTITIONS="${cnt}"
		if [ "${via}" = seam ]; then
			run "${SUT}" "$@"
		else
			bare "${SUT}" "$@"
...
# (k) Each value race-modules exports, through the seam: the argv is `./...` and --packages is
...
if [ "${total}" -lt "${DECLARED_CASES}" ]; then
	printf 'test-modules-race-partition: COULD NOT LOOK — %d case(s) ran, %d declared. A run that measures less than it claims is a FAILED run.\n' \
		"${total}" "${DECLARED_CASES}" >&2
	exit 2
fi
printf 'test-modules-race-partition: %d passed, %d failed (%d declared)\n' "${pass}" "${fail}" "${DECLARED_CASES}"
[ "${fail}" -eq 0 ] || exit 1
exit 0
