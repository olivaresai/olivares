...
CHECK="$ROOT/scripts/check-c03-25-deactivated-prep.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c0325prep.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0; fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
bad() { printf 'FAIL %s\n' "$1" >&2; fail=$((fail + 1)); }
...
stage() {
  ...
  cp "$ROOT/design/c03-25-deactivated-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C03-25-DEACTIVATED-PREP-2026-08-20.md" "$TMP/tree/design/"
  ...
  chmod +x "$TMP/tree/scripts/check-c03-25-deactivated-prep.sh"
}
run() {
  local rc=0
  unset OLIVARES_ENT_DIR || true
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-c03-25-deactivated-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe deactivated pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c03-25-deactivated-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
...
stage
printf '\nexport function holderHasDeploymentAccess() { return true }\n' >> \
  "$TMP/tree/commercial/license-worker/src/store/store.ts"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (store access-cut) is killed"
else bad "store access-cut stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
printf '\nexport function deactivateDeployment() {}\n' >> \
  "$TMP/tree/commercial/license-worker/src/store/db.ts"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (deactivateDeployment) is killed"
else bad "deactivateDeployment stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi
...
stage
rm -f "$TMP/tree/design/c03-25-deactivated-prep-2026-08-20.json"
run
...
echo "check-c03-25-deactivated-prep selftest: $pass passed, $fail failed"
