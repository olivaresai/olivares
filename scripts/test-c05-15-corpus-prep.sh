...
if [ ! -f "$ROOT"/cloud/control-plane/internal/billing/c05_15_corpus_test.go ]; then
	printf '%s\n' "test-c05-15-corpus-prep: COULD NOT LOOK — cloud/control-plane/internal/billing/c05_15_corpus_test.go is not in this tree" >&2
	exit 2
fi
if [ ! -f "$ROOT"/cloud/control-plane/internal/billing/dodoenvelope.go ]; then
	printf '%s\n' "test-c05-15-corpus-prep: COULD NOT LOOK — cloud/control-plane/internal/billing/dodoenvelope.go is not in this tree" >&2
	exit 2
fi
CHECK="$ROOT/scripts/check-c05-15-corpus-prep.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/c0515prep.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0; fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
bad() { printf 'FAIL %s\n' "$1" >&2; fail=$((fail + 1)); }
...
stage() {
  ...
  cp "$ROOT/design/c05-15-corpus-prep-2026-08-20.json" "$TMP/tree/design/"
  cp "$ROOT/design/C05-15-CORPUS-CONTRACT-PREP-2026-08-20.md" "$TMP/tree/design/"
  ...
  chmod +x "$TMP/tree/scripts/check-c05-15-corpus-prep.sh"
}
run() {
  local rc=0
  unset OLIVARES_ENT_DIR || true
  OLIVARES_ROOT="$TMP/tree" bash "$TMP/tree/scripts/check-c05-15-corpus-prep.sh" \
...
stage
run
if [ "$(cat "$TMP/rc")" = 0 ]; then ok "hub-safe corpus pin is CLEAN"
else bad "live pin should be CLEAN ($(cat "$TMP/err"))"; fi

stage
python3 - "$TMP/tree/design/c05-15-corpus-prep-2026-08-20.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p, encoding="utf-8"))
d["cloud_addons_honoured"] = True
json.dump(d, open(p, "w", encoding="utf-8"))
PY
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (cloud add-ons honoured) is killed"
else bad "cloud add-ons stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
sed -i 's/"raw_body"/"raw_x"/g' \
  "$TMP/tree/commercial/dodo-sandbox/evidence/dodo-10/wh-deliveries/delivery-0010.json"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then ok "mutant (lost raw_body) is killed"
else bad "lost raw_body stayed rc=$(cat "$TMP/rc") ($(cat "$TMP/err"))"; fi

stage
printf '\nFIRMA A claimed\n' >> \
  "$TMP/tree/design/C05-15-CORPUS-CONTRACT-PREP-2026-08-20.md"
run
...
stage
rm -f "$TMP/tree/design/c05-15-corpus-prep-2026-08-20.json"
run
...
echo "check-c05-15-corpus-prep selftest: $pass passed, $fail failed"
