...
#
# Battery for check-alc-01-s1-named-nil.sh. Both firing directions.

set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHECK="$ROOT/scripts/check-alc-01-s1-named-nil.sh"
_tmp_base="${TMPDIR:-/workspace/.olivares-tmptest}"
mkdir -p "$_tmp_base"
TMP="$(mktemp -d "$_tmp_base/alc01s.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
pass=0
fail=0
ok() { printf 'ok   %s\n' "$1"; pass=$((pass + 1)); }
...
stage() {
	...
	chmod +x "$TMP/tree/scripts/check-alc-01-s1-named-nil.sh"
...
run() {
	...
	OLIVARES_ROOT="$TMP/tree" \
		bash "$TMP/tree/scripts/check-alc-01-s1-named-nil.sh" \
...
if [ "$(cat "$TMP/rc")" = 0 ]; then
	ok "no-fire: live named-nil seam is CLEAN"
else
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: missing newManagedSCIM is FAIL"
else
	bad "missing seam should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi
...
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
	ok "firing: non-nil seam is FAIL"
else
	bad "non-nil seam should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
python3 - "$TMP/tree/design/alc-01-s1-named-nil.json" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
if [ "$(cat "$TMP/rc")" = 1 ]; then
	...
	bad "boot call should FAIL 1 ($(cat "$TMP/rc") $(cat "$TMP/err"))"
fi

stage
echo 'managed SCIM shipped' >>"$TMP/tree/design/ALC-01-S1-NAMED-NIL-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 1 ]; then
...
stage
rm -f "$TMP/tree/design/ALC-01-S1-NAMED-NIL-2026-08-20.md"
run
if [ "$(cat "$TMP/rc")" = 2 ]; then
	ok "missing named-nil doc is COULD NOT LOOK"
else
...
echo "check-alc-01-s1-named-nil selftest: $pass passed, $fail failed"
