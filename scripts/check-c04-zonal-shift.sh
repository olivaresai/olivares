...
#
# C04 remainder: ALB enable_zonal_shift named true. Unapplied.
# 0 CLEAN · 1 finding · 2 LOOK.

set -euo pipefail
say() { printf '%s\n' "$*"; }
fail() { say "check-c04-zonal-shift: FAIL — $*" >&2; exit 1; }
cannot() { say "check-c04-zonal-shift: COULD NOT LOOK — $*" >&2; exit 2; }

ROOT="${OLIVARES_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
cd "$ROOT" || cannot "cannot enter $ROOT"

JSON="${OLIVARES_C04ZS_JSON:-design/c04-zonal-shift.json}"
DOC="${OLIVARES_C04ZS_DOC:-design/C04-ZONAL-SHIFT-2026-08-20.md}"
TF="${OLIVARES_C04ZS_TF:-deploy/aws/modules/ingress/main.tf}"
...
if data.get("schema") != "c04-zonal-shift/v1":
...
if not re.search(r"enable_zonal_shift\s*=\s*true", block):
    raise SystemExit("ALB enable_zonal_shift lost true")
if re.search(r"enable_zonal_shift\s*=\s*false", block):
    raise SystemExit("ALB enable_zonal_shift is false")
PY

say "check-c04-zonal-shift: CLEAN — zonal shift named on; estate unapplied."
exit 0
