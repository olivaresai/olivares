...
// attrs is a flat, typed view over an OTLP attribute set. Cowork carries an
...
// never type-switches AnyValue by hand. It mirrors the claude connector's view
// (the OTLP wire shape is identical); it is duplicated rather than imported
// because the claude version is package-private and the two connectors stay
...
// objectVal returns the attribute decoded as a key/value object. Cowork carries a
// structured tool input either as a native OTLP kvlist or as a JSON string; both
// are handled, returning nil if the attribute is absent or not object-shaped.
