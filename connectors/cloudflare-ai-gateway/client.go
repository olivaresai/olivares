...
func (e *apiFault) Error() string {
	if len(e.errs) > 0 {
		...
		return fmt.Sprintf("cfaigateway api error (status %d): %s", e.status, strings.Join(parts, "; "))
	}
	if e.msg != "" {
		return fmt.Sprintf("cfaigateway api error (status %d): %s", e.status, e.msg)
	}
	return fmt.Sprintf("cfaigateway api error (status %d)", e.status)
}
...
func (c *client) fetch(ctx context.Context, path string, q url.Values) (*envelope, error) {
	...
	if len(body) > 0 {
		if jerr := json.Unmarshal(body, &env); jerr != nil {
			...
			return nil, fmt.Errorf("cfaigateway: decode %s: %w", path, jerr)
		}
	}
...
func decodeRows(result json.RawMessage) ([]json.RawMessage, error) {
	...
	if err := json.Unmarshal(result, &rows); err != nil {
		return nil, fmt.Errorf("cfaigateway: decode result rows: %w", err)
	}
	return rows, nil
}
