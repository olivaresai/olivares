...
// encoding/base64): a signed RS256 assertion JWT is exchanged at the token URI
// for a short-lived access token. Implementing it here keeps the connector
// dependency-free (no golang.org/x/oauth2, no go-jose) and boundary-clean. Tokens
// are cached until shortly before expiry, never logged, and never emitted.

const (
	// readOnlyScope is the single least-privilege read scope this connector needs.
	// cloud-platform.read-only grants read access across GCP — Resource Manager
	// list, IAM serviceAccounts list and Cloud Logging entries:list — without any
	// write capability. The operator still binds read-only IAM roles to the SA (see
	// doc.go); the scope is the upper bound, the roles the actual grant.
...
// connector reads. The private key never leaves memory and is never logged.
...
// staticTokenSource returns a fixed operator-supplied bearer token (e.g. injected
// by a Workload Identity Federation sidecar or ADC helper that refreshes it out
// of band). It is also how tests drive the connector without a private key.
...
func newSATokenSource(saJSON []byte, client *http.Client, tokenURIOverride string) (*saTokenSource, error) {
	...
	if err := json.Unmarshal(saJSON, &sa); err != nil {
		...
		return nil, fmt.Errorf("gcp-audit: credentials are not valid JSON")
	}
	if sa.ClientEmail == "" || sa.PrivateKey == "" {
		return nil, fmt.Errorf("gcp-audit: credentials JSON must carry client_email and private_key")
	}
...
// mint signs a fresh assertion and exchanges it for an access token. The only
// thing POSTed is grant_type + the short-lived signed assertion — never the
// private key. A non-2xx surfaces only the status; the credential never appears
// in any error.
func (s *saTokenSource) mint(ctx context.Context) (string, time.Duration, error) {
	...
	if err != nil {
		return "", 0, fmt.Errorf("gcp-audit: build token request: %w", err)
	}
	...
	if err != nil {
		return "", 0, fmt.Errorf("gcp-audit: token exchange: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 8<<10))
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", 0, fmt.Errorf("gcp-audit: token endpoint status %d", resp.StatusCode)
	}
	var tr struct {
	...
	if err := json.Unmarshal(body, &tr); err != nil || tr.AccessToken == "" {
		return "", 0, fmt.Errorf("gcp-audit: token response has no access_token")
	}
...
// assertion builds and signs the RS256 JWT-bearer assertion {iss, scope, aud,
// iat, exp}. iss is the SA email; aud is the token URI; the scope is the single
// read-only scope.
func (s *saTokenSource) assertion() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("gcp-audit: marshal assertion header: %w", err)
	}
	cb, err := json.Marshal(claims)
	if err != nil {
		return "", fmt.Errorf("gcp-audit: marshal assertion claims: %w", err)
	}
	signingInput := b64url(hb) + "." + b64url(cb)
	digest := sha256.Sum256([]byte(signingInput))
	sig, err := rsa.SignPKCS1v15(rand.Reader, s.key, crypto.SHA256, digest[:])
	if err != nil {
		return "", fmt.Errorf("gcp-audit: sign assertion: %w", err)
	}
...
func parseRSAPrivateKey(pemStr string) (*rsa.PrivateKey, error) {
	...
	if block == nil {
		return nil, fmt.Errorf("gcp-audit: private_key is not valid PEM")
	}
	if k, err := x509.ParsePKCS8PrivateKey(block.Bytes); err == nil {
		rk, ok := k.(*rsa.PrivateKey)
		if !ok {
			return nil, fmt.Errorf("gcp-audit: private_key is not an RSA key")
		}
		return rk, nil
	}
	rk, err := x509.ParsePKCS1PrivateKey(block.Bytes)
	if err != nil {
		return nil, fmt.Errorf("gcp-audit: parse private_key: %w", err)
	}
	return rk, nil
}
