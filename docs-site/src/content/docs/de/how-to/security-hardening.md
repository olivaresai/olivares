...
| **Der Bind ist nicht der Schutz** | Die Engine bindet standardmäßig **alle Schnittstellen**: Sie ist ein Server. Was sie schützt, ist TLS an, keine Standard-Anmeldedaten und ein einmalig verwendbares Setup-Token — nichts davon hängt vom Bind ab. | Beschränken Sie ihn **bewusst**, wenn Sie das wollen: `--listen 127.0.0.1:8443` für das Binary, `OLIVARES_BIND=127.0.0.1` für den Compose-Stack. Stellen Sie Ihren eigenen Ingress/TLS davor, für ein Zertifikat, dem Browser vertrauen, und einen Hostnamen, den Passkeys akzeptieren — siehe [Self-Hosting](/de/how-to/self-hosting/). |
