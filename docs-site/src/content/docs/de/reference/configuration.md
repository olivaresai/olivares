...
| `OLIVARES_AUDIT_LEGALHOLD_RECONCILE` | No | — | Boolescher Schalter, kein Dateipfad, für den Legal-Hold-Reconciler mit langem Zeithorizont, der Object-Lock-Legal-Holds auf die archivierten Audit-Segmente von Tenants mit aktivem Legal Hold setzt. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_reg` kompiliert wurden, und zwar beim Start der Engine; eine Änderung wirkt erst nach einem Neustart. Nur ein Wert, den `strconv.ParseBool` als true akzeptiert, etwa `true` oder `1`, schaltet ihn ein; ist die Variable nicht gesetzt, false oder nicht parsbar, bleibt er ohne Fehler ausgeschaltet. Außerdem bleibt er ausgeschaltet, sofern das Audit-Archiv nicht auf einem Sink konfiguriert ist, der Object-Lock-Legal-Holds setzen kann. |
...
| `OLIVARES_CIRCUIT_BREAKER_CONFIG` | No | — | Boolescher Schalter, trotz des Suffixes `_CONFIG` kein Dateipfad, für den Runtime-Circuit-Breaker; seine Regeln pro Tenant werden nicht hier festgelegt. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_airs` kompiliert wurden, und zwar beim Start der Engine; eine Änderung wirkt erst nach einem Neustart. Ist die Variable nicht gesetzt oder hat sie einen Wert, den `strconv.ParseBool` als false akzeptiert, bleibt der Breaker ausgeschaltet. Ein Wert, der kein boolescher Wert ist, wird als Fehler protokolliert und lässt ihn ebenfalls ausgeschaltet. Ein true-Wert schaltet ihn ein, wenn Moduldaten verfügbar sind; andernfalls wird eine Warnung protokolliert und er bleibt ausgeschaltet. |
...
| `OLIVARES_COMPUTER_USE_CONFIG` | No | — | Pfad zur JSON-Policy-Datei des Computer-Use-Gates. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_airs` kompiliert wurden. Ist die Variable nicht gesetzt, bleibt das Gate ausgeschaltet; eine nicht lesbare oder nicht parsbare Datei lässt es jede Nutzung von Computer Use verweigern, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_CONTENT_FIREWALL_CONFIG` | No | — | Pfad zur JSON-Policy-Datei der Content-Firewall, die den Inferenz-Traffic im Inline-Proxy prüft. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_airs` kompiliert wurden. Ist die Variable nicht gesetzt, bleibt diese Prüfung ausgeschaltet; eine nicht lesbare oder nicht parsbare Datei lässt die Firewall jede von ihr geprüfte Anfrage verweigern, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_CREDENTIAL_MINTER_CONFIG` | No | — | Pfad zur JSON-Konfiguration des serverbezogenen Minters für Upstream-Credentials (Token Exchange nach RFC 8693) des MCP-Gateways. Enthält Secrets und ist deshalb eine Datei statt eines Werts. Wird von Builds gelesen, die mit dem Tag `enterprise` kompiliert wurden, wenn die Engine startet und das Gateway für eine Upstream-URL aufbaut. Ist die Variable nicht gesetzt, bleibt das statische Upstream-Credential erhalten; eine nicht lesbare oder nicht parsbare Datei verweigert jedes Upstream-Credential, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_ELICITATION_MEDIATOR_CONFIG` | No | — | Pfad zur JSON-Policy-Datei des MCP-Elicitation-Mediators. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_airs` kompiliert wurden. Ist die Variable nicht gesetzt, bleibt die Mediation ausgeschaltet; eine nicht lesbare oder nicht parsbare Datei lässt den Mediator jede Elicitation verweigern, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_MODEL_GATEWAY_CHAT_MODE` | Nein | `disabled` | Aktivierungsmodus für den synchronen, kontrollierten Chat-Versand. Nicht gesetzt oder `disabled` hält ihn ausgeschaltet; `development_precheck` aktiviert ausdrücklich die bestehende nicht atomare Fail-open-Routing-Vorprüfung, keinen verbindlichen Kostenrahmen. Jeder andere Wert verhindert den Start. |
| `OLIVARES_MODEL_GATEWAY_PROFILES_CONFIG` | Nein | — | Pfad zur lokalen JSON-Datei mit unveränderlichen Ausführungsprofilen für das Modell-Gateway. Ist die Variable nicht gesetzt, werden keine Profile geladen; eine nicht lesbare oder ungültige Datei verhindert den Start. |
...
| `OLIVARES_ONBOARDING_CONFIG` | No | — | Pfad zur JSON-Konfiguration des Assessors für die Onboarding-Readiness. Wird von Builds gelesen, die mit dem Tag `enterprise` kompiliert wurden, und zwar bei jeder Ausführung von `olivares enterprise readiness`, nicht beim Start der Engine. `{}` wählt das Standardprofil. Ist die Variable nicht gesetzt oder ist die Datei nicht lesbar oder nicht parsbar, meldet dieser Befehl, dass die Onboarding-Readiness nicht konfiguriert ist. |
...
| `OLIVARES_PQC_POSTURE_CONFIG` | No | — | Pfad zur JSON-Konfiguration des Assessors für die Post-Quanten-Posture. Wird von Builds gelesen, die mit dem Tag `enterprise` kompiliert wurden, und zwar bei jeder Ausführung von `olivares enterprise pqc-posture`, nicht beim Start der Engine. `{}` wählt CNSA 2.0 mit dem Zieljahr 2033. Ist die Variable nicht gesetzt oder ist die Datei nicht lesbar oder nicht parsbar, meldet dieser Befehl, dass die PQC-Posture nicht konfiguriert ist. |
...
| `OLIVARES_RENDER_INSPECTOR_CONFIG` | No | — | Pfad zur JSON-Policy-Datei des Inspektors für gerenderte MCP-App-Inhalte. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_airs` kompiliert wurden. Ist die Variable nicht gesetzt, bleibt diese Prüfung ausgeschaltet; eine nicht lesbare oder nicht parsbare Datei lässt den Inspektor sämtliche gerenderten Inhalte verweigern, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_RETENTION_GOVERNOR_CONFIG` | No | — | Pfad zur JSON-Konfiguration des Governors für regulatorische Mindestaufbewahrungsfristen. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_reg` kompiliert wurden, und zwar beim Start der Engine. Ist die Variable nicht gesetzt, wird keine Mindestaufbewahrungsfrist erzwungen; eine nicht lesbare oder nicht parsbare Datei setzt die Mindestaufbewahrungsfrist jeder Aufbewahrungsklasse auf 100 Jahre, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_RTBF_DEPTH_CONFIG` | No | — | Pfad zur JSON-Konfiguration des RTBF-Koordinators für Crypto-Shredding. Wird nur von Builds gelesen, die mit den Tags `enterprise` und `addon_reg` kompiliert wurden, und zwar beim Start der Engine. Ist die Variable nicht gesetzt, bleibt das Crypto-Shredding des Open Core unverändert; eine nicht lesbare oder nicht parsbare Datei blockiert jedes Shredding, bis die Datei korrigiert und die Engine neu gestartet ist. |
...
| `OLIVARES_SESSION_RUNTIME_OPENCODE_BIN` | No | — | Explizit festgelegte offizielle OpenCode-Programmdatei, die die Sitzungslaufzeit verwenden darf. Wenn sie angegeben ist, wird der OpenCode-ACP-Treiber auf diesem Knoten registriert. Andernfalls bleiben OpenCode-Profile beobachtbar, können jedoch nicht gestartet werden. Andere Anbieter-Treiber bleiben unverändert. |
...
| `OLIVARES_TOOL_PIN_CONFIG` | No | — | Pfad zur optionalen JSON-Konfiguration (`require_pin_approval`) des MCP-Tool-Pin-Stores, den mit dem Tag `enterprise` kompilierte Builds unabhängig davon betreiben, ob diese Variable gesetzt ist. Wird beim Start der Engine gelesen. Ist die Variable nicht gesetzt, bleibt es bei Trust on First Use; eine nicht lesbare oder nicht parsbare Datei verlangt für unbekannte Tools die Freigabe durch einen Operator, bis die Datei korrigiert und die Engine neu gestartet ist. |
