---
title: "CLI-Referenz: olivares"
description: "Verifizierte Subcommands und Flags für das einzelne olivares-Binary, einschließlich der secure-by-default serve-Optionen."
---

Olivares AI wird als ein einziges statisches Go-Binary namens `olivares` ausgeliefert. Dasselbe Artefakt ist die Engine, die eingebettete Web-UI (vom selben Origin wie die API ausgeliefert) und der Edge-Collector — die Rolle wird durch den ausgeführten Subcommand ausgewählt. Diese Seite dokumentiert die Befehlsschnittstelle des Community-Builds (AGPL). Die folgenden Abschnitte erläutern die Befehle, denen Sie zuerst begegnen; die am Ende aus dem Binary generierte [vollständige Befehlsreferenz](#complete-command-reference) deckt jeden Befehl dieses Builds ab. Die Schnittstelle ist noch in Bewegung (siehe [Stabilität](#stability) am Ende).

Wie Sie das Binary beziehen und ausführen, steht unter [Self-Hosting](/de/how-to/self-hosting/). Für Konfiguration, die in Umgebungsvariablen statt in Flags lebt, siehe [Konfiguration](/de/reference/configuration/).
...
Der Root-Befehl sortiert seine Subcommands in die Abschnitte, die `olivares --help` ausgibt: Setup und Konfiguration, Betrieb, Governance, Beobachtung und Diagnose, Sicherheit sowie Release und Upgrade.

Die folgenden Abschnitte erklären die Befehle, denen Sie zuerst begegnen — was sie mit Ihrer Installation tun und was sicher exponiert werden kann. Die erschöpfende Liste steht in der am Seitenende **aus dem Binary generierten** [vollständigen Befehlsreferenz](#complete-command-reference), einschließlich Flags, Exit-Codes und Output-Vertrag. Der Generator durchläuft den Befehlsbaum des Community-Binarys (`TestCLIRefDump`) und schreibt dieses Ergebnis in den markierten Bereich. Push-Gate und CI führen diese Prüfung über `lint:public-counts` aus, das `scripts/check-cli-ref-docs.sh` aufruft: das Skript leitet Flags, Defaults, Exit-Codes und Output-Verträge aus dem Binary neu ab und schlägt fehl, wenn diese Seite oder eine der sechs Locale-Kopien vom Binary abweicht. Eine separate Prüfung, `lint:cli-coverage`, verifiziert nur, dass im Quellcode deklarierte Befehlsnamen irgendwo in dieser Referenz erwähnt werden; sie leitet Flags nicht neu ab. Befehlsnamen, Flag-Namen, Hilfetexte, Defaults und Bedeutungen der Exit-Codes in dem generierten Bereich sind die originalen englischen Zeichenketten des Binarys — auch auf den Locale-Kopien dieser Seite, wo dieser Block keine menschliche Übersetzung ist. Wenn ein eingesetztes Binary und diese Seite widersprechen, ist `olivares <subcommand> --help` auf diesem Binary maßgeblich.

:::note[Ein Binary, mehrere Rollen]
Es gibt keinen separaten "Server"- und "Agent"-Download. `olivares serve` ist die Engine; `olivares collector` ist der Data-Plane-Collector für die verteilte Topologie. Beide laden dieselben Source-Connectors auf dieselbe Weise — nur wohin die Beobachtungen gehen, unterscheidet sich. Siehe [Architektur](/de/explanation/architecture/overview/).
:::

## olivares version

Gibt Version, Commit, Build-Datum, OS/Arch und die Go-Runtime-Version auf stdout aus.

```sh
olivares version
```

Der Versionsstring wird zur Build-Zeit injiziert. Ein Build aus einem Working Tree, der nicht von einem getaggten Release erzeugt wurde, meldet eine Entwicklungsversion (zum Beispiel `dev`), behandeln Sie den Versionsstring also nicht als Provenienzgarantie — verifizieren Sie Releases stattdessen mit den signierten Artefakten. Siehe [Ein Release verifizieren](/de/how-to/verify-a-release/).

## olivares serve

Betreibt die Control Plane: den HTTP-Server (REST API plus die eingebettete Web-UI auf demselben Origin) und den gRPC-Server. **TLS ist standardmäßig an**, die Listener binden **standardmäßig auf Loopback**, und es gibt **keine Default-Credentials**.
...
Jedes Flag von `serve` mit Typ und Default steht im generierten Abschnitt
[`olivares serve`](#command-olivares-serve) unten. Dieser Abschnitt behandelt nur die vier,
die verändern, was die Installation exponiert: `--listen` und `--grpc-listen` (Bind-Adresse),
`--insecure` (verschlüsselter oder unverschlüsselter Transport) und `--seed-demo` (echte oder
synthetische Daten).

Der Default-Store ist SQLite (pure-Go, Single-Node, geeignet für air-gapped Installationen). `postgres` auszuwählen ist das, was Sie für Multi-Tenant- oder Scale-out-Deployments tun, wo Row-Level Security die Tenant-Absicherung liefert. Siehe [Konfiguration](/de/reference/configuration/) und [Self-Hosting](/de/how-to/self-hosting/).

:::caution[`--insecure` ist nur für localhost-Entwicklung]
`--insecure` stellt Klartext-HTTP und -gRPC bereit. Bearer-Token reisen auf einem Klartext-Transport im Klartext, verwenden Sie dieses Flag also nie auf einer Adresse, die über den lokalen Host hinaus erreichbar ist. Außerhalb von `--insecure` weigert sich der gRPC-Server, ohne TLS zu starten, statt stillschweigend herabzustufen.
:::

### `--seed-demo` ist nur für Demos und verweigert Nicht-Loopback

`--seed-demo` provisioniert eine **synthetische, fabrizierte** Estate zusammen mit einem Demo-Administrator, dessen Passwort **öffentlich** ist (es liegt im Quellbaum). Es existiert allein dazu, die Web-UI und End-to-End-Tests gegen live-geformte Daten rendern zu lassen.

Da die Demo-Credential öffentlich ist, **weigert sich `serve`, mit `--seed-demo` auf irgendeinem Nicht-Loopback-Bind zu starten** und beendet sich mit einem Fehler, der Sie anweist, auf `127.0.0.1` zu binden oder eine echte Installation ohne das Flag auszuführen. Behandeln Sie `--seed-demo` als Wegwerf-Sache: verwenden Sie ein verfügbares Datenverzeichnis und richten Sie es nie auf Daten, die Ihnen wichtig sind.

:::danger[Führen Sie `--seed-demo` niemals als echte Installation aus]
Der Demo-Administrator hat ein öffentlich bekanntes Passwort. Eine echte Installation ist `serve` **ohne** `--seed-demo`, wobei die Engine ein einmaliges Setup-Token prägt und Sie Ihren eigenen Administrator erstellen. Mischen Sie die beiden nicht: ein Demo-Datenverzeichnis ist kein Produktions-Datenverzeichnis.
:::

### First-Boot-Setup

Beim ersten Boot einer Installation, die keine Nutzer hat, gibt `serve` einen Block auf stdout aus, der das einmalige Setup-Token (Präfix `olst_`) und den Request enthält, den Sie zum Bootstrappen des ersten Administrators benötigen. Der Ablauf ist:

1. Lesen Sie das `olst_`-Token aus dem stdout der Engine (beim Container-Deployment lesen Sie es aus den Container-Logs).
2. Erstellen Sie den ersten Administrator, indem Sie das Token, eine E-Mail und ein Passwort an den Setup-Endpunkt posten (`POST /v1/setup`).
3. Melden Sie sich an (`POST /v1/auth/login`), um ein Session-Token (Präfix `olvs_`) zu erhalten.

Das Setup-Token wird einmal angezeigt und ist single-use; sobald ein Nutzer existiert, wird kein Token mehr geprägt. Olivares AI verwendet **opake** Bearer-Token (keine JWTs); API-Keys tragen das Präfix `olvk_`. Für den vollständigen Authentifizierungsvertrag und die Regeln zur Tenant-Auflösung siehe das [Security-Modell](/de/explanation/security/security-model/) und die [API-Referenz](/reference/api/).

### Ausführen

```sh
# Bauen, dann ausführen (es gibt kein "task serve" / "task run"-Target).
task build
./bin/olivares serve
# Lesen Sie das einmalige olst_-Setup-Token aus dem stdout dieses Prozesses.
```

Oder führen Sie das Container-Deployment aus und lesen Sie das Setup-Token aus den Logs; siehe [Self-Hosting](/de/how-to/self-hosting/).

## olivares collector

Führt das Binary als **Edge-Collector** für die verteilte Topologie aus. Ein Collector lädt die in Ihrer Sources-Konfiguration benannten Source-Connectors lokal und **pusht** deren Beobachtungen über gRPC an einen entfernten Core. Er öffnet **keinen eingehenden Listener** — der Collector ist die secure-default Data Plane: er wählt aus, er akzeptiert keine Verbindungen.

```sh
olivares collector --core-addr host:port [flags]
```

Der Collector authentifiziert sich gegenüber dem Core in zwei Schichten: einem Bearer-Token, das ein Ingest-Principal hält, und — wenn der Core mutual TLS erzwingt — einem Collector-Client-Zertifikat. So läuft die Data Plane auf Kundeninfrastruktur, während ein zentraler Core aggregiert: ein ausfallender Collector sitzt nie im Datenpfad irgendeines Agenten.

:::note[Welche Sources ein Collector lädt]
Sowohl `serve` als auch `collector` verdrahten ihre Source-Connectors aus derselben Konfiguration, die vor dem Start der Runtime aus der Umgebung gelesen wird. Eine unkonfigurierte oder leere Source warnt ehrlich, statt den Prozess fehlschlagen zu lassen. Das Konfigurieren von Sources wird in [Eine Quelle anbinden](/de/how-to/connect-a-source/) und [Claude Code anbinden](/de/how-to/connect-claude-code/) behandelt; der Konfigurationsmechanismus steht in [Konfiguration](/de/reference/configuration/).
:::

Der collector-Subcommand ist der **Mechanismus** für den verteilten Pfad. Das Packaging drumherum (eine Flotte von Collectors, signierte Charts, OCI-Images) ist Teil der Deployment-Story und nicht dieser CLI-Seite — siehe [Architektur](/de/explanation/architecture/overview/) und [Self-Hosting](/de/how-to/self-hosting/).

## olivares openapi

Gibt das OpenAPI-3.1-Dokument der Engine auf stdout aus, ohne dass ein laufender Server nötig ist.

```sh
olivares openapi > openapi.json              # stabiler Kernvertrag
olivares openapi --beta > openapi.beta.json  # Beta-Dokument der Modulrouten
```

Ohne Flag gibt der Befehl denselben stabilen Kernvertrag aus, den die Engine unter `GET /openapi.json` ausliefert, deterministisch eingerückt für saubere Diffs; er ist die Quelle der Wahrheit für die [API-Referenz](/reference/api/) und die typisierte Codegenerierung des Web-Clients. `--beta` gibt das **Beta**-Dokument der Modulrouten (`/v1/m/<ns>/…`) aus, das unter `GET /openapi.beta.json` bereitsteht und in der [Modulrouten-Referenz](/reference/api-beta/) gerendert wird. Die feldgenauen Formen bleiben als typisierte Interfaces ausgedrückt (siehe auch [Modul-Übersicht](/de/reference/modules/overview/)).

:::note[Die generierte Referenz bleibt Englisch]
Der folgende technische Block wird — einschließlich der Marker — aus demselben Dump des
Binarys regeneriert wie die englische Seite, und das Gate vergleicht ihn bei jedem Push.
Die Befehlsnamen, Help-Beschreibungen, Flags, Exit-Codes und Output-Verträge bleiben
absichtlich auf Englisch, weil genau diese Strings das Binary ausgibt; das ist keine
menschliche Übersetzung. Die redaktionelle Anleitung darum ist lokalisiert.
:::
...
Dies ist ein pre-1.0-Produkt in aktiver Entwicklung. Die obigen Subcommands und Flags sind im aktuellen Binary bestätigt, aber die vollständige CLI-Schnittstelle entwickelt sich noch: Subcommands, Flags, Defaults und Ausgabeformate können sich vor einem stable-Release ändern. Im Zweifel führen Sie `olivares <subcommand> --help` gegen den exakten Build aus, den Sie deployt haben, und behandeln das als maßgeblich gegenüber jedem Dokument. Was heute implementiert ist gegenüber dem Geplanten, siehe [Ehrlichkeit und Grenzen](/de/start/honesty-and-limits/). Die REST/gRPC-API-Schnittstelle selbst unterliegt der [API-Stabilitätsrichtlinie](/de/reference/api-stability/).
