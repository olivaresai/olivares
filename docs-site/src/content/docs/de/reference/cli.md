...
Die folgenden Abschnitte erklären die Befehle, denen Sie zuerst begegnen — was sie mit Ihrer Installation tun und was sicher exponiert werden kann. Die erschöpfende Liste steht in der am Seitenende **aus dem Binary generierten** [vollständigen Befehlsreferenz](#complete-command-reference), einschließlich Flags, Exit-Codes und Output-Vertrag. Sie ist ein Snapshot dieses Builds, keine Live-Ausgabe: Push-Gate und CI führen `lint:cli-coverage` aus, das prüft, ob im Quellcode deklarierte Befehle irgendwo in dieser Referenz dokumentiert sind, jedoch diesen Abschnitt nicht neu ableitet und Flags, Exit-Codes oder Output-Verträge nicht vergleicht. Wenn Seite und Binary widersprechen, ist `olivares <subcommand> --help` maßgeblich.
...
Der folgende technische Block wird bytegenau aus dem englischen Output des Binarys übernommen,
einschließlich der Marker. Die Befehlsnamen, Help-Beschreibungen, Flags, Exit-Codes und
Output-Verträge bleiben absichtlich auf Englisch, weil genau diese Strings das Binary ausgibt;
die redaktionelle Anleitung darum ist lokalisiert.
:::
