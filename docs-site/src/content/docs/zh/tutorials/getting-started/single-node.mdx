...
- **Go 1.26+** 用于构建二进制（存储是纯 Go 的 SQLite，因此无需 C
  工具链）。先前的带标签发布已经交付过签名归档和软件包。本树的 canon 是
  **v26.9.0**；安装面证人仍将 GitHub 归档和本机软件包记为
  **not-published**，直到该 tag 的发布作业运行
  （`docs/releases/v26.9.0-install-surfaces.json`）。在那些工件存在之前，从
  checkout 构建。`main` 上的已验证安装器（`scripts/install.sh`）是独立于 tag
  的已发布面。[验证一次发布](/zh/how-to/verify-a-release/)是工件存在后的信任链。
...
引擎默认绑定**所有网络接口**（`:8443`）：它是一台服务器。上面的命令传入
`--listen 127.0.0.1:8443`，把本教程的安装限制在你所在的这台主机上——这是一个明确的决定。
去掉这两个标志，控制台就会从网络应答；那时请把你自己的 ingress 和 TLS 放在它前面
