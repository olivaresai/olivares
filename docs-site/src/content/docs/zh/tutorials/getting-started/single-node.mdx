...
- **Go 1.26+** 用于构建二进制（存储是纯 Go 的 SQLite，因此无需 C
  工具链）。已发布的版本 **v26.9.0** 随附签名归档和本机软件包：安装面证人将两者记为
  **published**（`docs/releases/v26.9.0-install-surfaces.json`）。本教程从 checkout 构建；
  若要安装已发布的版本，请使用 `main` 上的已验证安装器（`scripts/install.sh`）。
  [验证一次发布](/zh/how-to/verify-a-release/)是这些工件的信任链。
