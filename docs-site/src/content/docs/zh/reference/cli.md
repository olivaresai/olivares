---
title: "CLI 参考：olivares"
description: "单一 olivares 二进制文件经过验证的子命令与标志，包括默认即安全的 serve 选项。"
---

Olivares AI 以一个名为 `olivares` 的静态 Go 二进制文件交付。同一份产物既是引擎、又是内嵌的 Web UI（从与 API 相同的来源提供服务），还是边缘采集器（edge collector）——角色由你所运行的子命令选定。本页记录 community（AGPL）构建的命令表面。下文先解释最先用到的命令；末尾的[完整命令参考](#complete-command-reference)由二进制生成，涵盖该构建中的每一条命令。该表面仍在变动中（参见末尾的[稳定性](#stability)）。

关于如何获取并运行该二进制文件，参见 [自托管](/zh/how-to/self-hosting/)。关于存放在环境变量而非标志中的配置，参见 [配置](/zh/reference/configuration/)。
...
根命令将其子命令按 `olivares --help` 打印的分区排序：setup and configuration、
operate、govern、observe and diagnose、security，以及 release and upgrade。

下文解释最先用到的命令——它们对安装做什么、哪些表面可安全暴露；这部分不是表格能
承载的。完整清单见本页末尾的[完整命令参考](#complete-command-reference)：它**由二进制
生成**，涵盖构建中的每条命令及其 flag、exit code 和 output contract。生成器遍历
community 二进制的命令树（`TestCLIRefDump`）并把该遍历写入标记区域。push gate 与 CI
通过 `lint:public-counts` 调用 `scripts/check-cli-ref-docs.sh`：它从二进制重新推导
flag、默认值、exit code 与 output contract，并在本页或六份 locale 副本任一与二进制
不一致时失败。另一项检查 `lint:cli-coverage` 只验证源码中声明的命令名是否在本参考
某处被提及，并不重新推导 flag。生成区域内的命令名、flag 名、help 文本、默认值与
exit code 含义是二进制原有的英文字符串——locale 副本上的该块也不是人工翻译。若已
部署的二进制与本页不一致，以该二进制上的 `olivares <subcommand> --help` 为准。

:::note[单一二进制，多重角色]
不存在单独的「服务端」与「代理」下载。`olivares serve` 是引擎；`olivares collector` 是面向分布式拓扑的数据面采集器。两者以相同方式加载相同的源连接器（source connectors）—— 唯一的区别在于观测数据流向何处。参见 [架构](/zh/explanation/architecture/overview/)。
:::
...
版本字符串在构建期注入。一个由非打标发布（tagged release）所产出的工作树构建出来的产物会报告一个开发版本（例如 `dev`），因此不要将版本字符串视为来源（provenance）的保证 —— 应改用签名产物来验证发布。参见 [验证一次发布](/zh/how-to/verify-a-release/)。

## olivares serve

运行 control plane：HTTP 服务器（REST API 加上同源的内嵌 Web UI）与 gRPC 服务器。**TLS 默认开启**，监听器**默认绑定 loopback（回环）**，且**不存在默认凭据**。
...
`serve` 接受的每个 flag、类型与默认值，都列在下文生成参考的
[`olivares serve`](#command-olivares-serve) 中。本节只说明改变安装暴露面的四项：
`--listen` 与 `--grpc-listen`（绑定位置）、`--insecure`（传输是否加密），以及
`--seed-demo`（数据是否真实）。

默认存储为 SQLite（纯 Go、单节点，适合离线（air-gapped）安装）。选择 `postgres` 是你在多租户或横向扩展（scale-out）部署时所做的选择，其中行级安全（row-level security）提供了租户兜底。参见 [配置](/zh/reference/configuration/) 与 [自托管](/zh/how-to/self-hosting/)。

:::caution[`--insecure` 仅限 localhost 开发]
`--insecure` 以明文提供 HTTP 与 gRPC 服务。Bearer 令牌在明文传输上以明文传送，因此切勿在任何可从本机之外抵达的地址上使用此标志。在 `--insecure` 之外，gRPC 服务器会拒绝在没有 TLS 的情况下启动，而不是悄然降级。
:::

### `--seed-demo` 仅供演示且拒绝非 loopback

`--seed-demo` 配置一份**合成的、虚构的** estate，连同一个密码**公开**（它就在源代码树中）的演示管理员。它存在的唯一目的是让 Web UI 与端到端测试得以对照具有真实形态的数据进行渲染。
...
:::danger[切勿把 `--seed-demo` 当作真实安装运行]
该演示管理员拥有一个众所周知的密码。真实安装是**不带** `--seed-demo` 的 `serve`，其中引擎会铸造一个一次性设置令牌，由你创建你自己的管理员。不要把两者混用：一个演示数据目录不是一个生产数据目录。
:::
...
设置令牌只显示一次且单次使用；一旦存在某个用户，便不再铸造令牌。Olivares AI 使用**不透明（opaque）**的 bearer 令牌（不是 JWT）；API 密钥携带前缀 `olvk_`。关于完整的认证契约与租户解析规则，参见 [安全模型](/zh/explanation/security/security-model/) 与 [API 参考](/reference/api/)。
...
或者运行容器部署并从日志中读取设置令牌；参见 [自托管](/zh/how-to/self-hosting/)。

## olivares collector

将该二进制文件作为面向分布式拓扑的**边缘采集器**运行。一个采集器在本地加载你的 sources 配置中指定的源连接器，并通过 gRPC 把它们的观测数据**推送**到远端 core。它不开启**任何入站监听器** —— 采集器是默认即安全的数据面：它向外拨号（dial out），不接受连接。
...
:::note[采集器加载哪些源]
`serve` 与 `collector` 都从同一份配置接线其源连接器，该配置在运行时启动之前从环境中读取。一个未配置或为空的源会如实告警，而不是使进程失败。配置源的内容见 [连接一个数据源](/zh/how-to/connect-a-source/) 与 [连接 Claude Code](/zh/how-to/connect-claude-code/)；配置机制见 [配置](/zh/reference/configuration/)。
:::

collector 子命令是分布式路径的**机制**。围绕它的封装（一支采集器舰队、签名 chart、OCI 镜像）属于部署故事，而非本 CLI 页面 —— 参见 [架构](/zh/explanation/architecture/overview/) 与 [自托管](/zh/how-to/self-hosting/)。
...
默认输出与引擎在 `GET /openapi.json` 提供的契约相同（稳定核心路径），并采用确定性
缩进以便 clean diff；它是所渲染 [API 参考](/reference/api/)与 Web client typed-code
generation 的 source of truth。`--beta` 输出 **beta** module-route 文档
（`/v1/m/<ns>/…`，服务于 `GET /openapi.beta.json`，渲染于
[模块路由参考](/reference/api-beta/)），从 module 注册的 route 反射而来，field-level shape
仍以 typed interface 表达（另见[模块概览](/zh/reference/modules/overview/)）。

:::note[生成的参考内容保留英文]
下方技术块（含 marker）与英文页一样，从同一份二进制 dump 重新生成，并由 gate 在每次
push 时比较。command 名称、help description、flag、exit code 与 output contract 刻意
保留英文，因为二进制文件实际输出的正是这些字符串，不是人工翻译。该块周围的编辑说明
已本地化。
:::
...
这是一个处于积极开发中、1.0 之前的产品。上文的子命令与标志在当前二进制文件中已确认，但完整的 CLI 接口面仍在演进：子命令、标志、默认值与输出格式都可能在一次稳定发布之前发生变化。如有疑问，请针对你所部署的确切构建运行 `olivares <subcommand> --help`，并将其视为权威，优先于任何文档。关于今天已实现的内容与计划中的内容，参见 [诚实与局限](/zh/start/honesty-and-limits/)。REST/gRPC API 接口面本身由 [API 稳定性策略](/zh/reference/api-stability/) 治理。
