...
没有默认凭据、设置令牌只能使用一次。默认监听地址是 `:8443`——所有网络接口，因为这是一台服务器（`cmd/olivares/binddefaults.go`）。
2026-09-04 在全新安装的公开版二进制文件上使用真实 TLS 测得（包括一次在
**:8460** 上的运行）；面板文本相同。

欢迎面板（`announceQuickstart`，`:154-163`）带有编号。引擎会打印
`https://localhost:8443`，并在令牌下方列出这台主机应答的其他所有地址。**不要使用 IP 地址完成通行密钥仪式。** 浏览器会拒绝将 IP 地址
...
横幅对默认绑定打印 `localhost`，并在令牌下方列出这台主机的其他地址；通行密钥仪式需要的是名称，而不是地址。请在地址栏中将主机地址改为
`localhost`。
