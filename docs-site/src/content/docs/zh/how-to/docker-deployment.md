...
:::note[Beta——26.9.1 尚未发布]
Olivares AI 处于 **beta** 阶段。下文的镜像坐标只有在**版本
`26.9.1` 发布之后**才能解析；在此之前各 registry 上没有任何可拉取的内容。
...
Tag **不带前导 `v`**：
`:26.9.1` 固定一个版本，`:latest` 浮动，而 `:26.9.1-fips` / `:26.9.1-stig`
...
可变 tag（`:26.9.1`、`:latest`）用于评估。在生产环境中，请固定你所验证的
