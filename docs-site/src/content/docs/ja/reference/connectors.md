...
再スキャンには `poll_seconds` を伴う別の `sources` エントリを配線する。
各エントリは**それ自身の `name`** で登録されるため、同じ kind のエントリを複数並べて動かせる ——
`okta` と `entra` は単一の `idp` コネクタが担うが、それぞれ独自の設定・状態・ライフサイクルを持つ
別個のソースである。一意でなければならないのは名前であり、コネクタではない。グループ/ロールの
メンバーシップは型付き名簿スナップショットのみを通って移動し、決してエッジとしては移動しない。
...
それはブートごとに一度しかスキャンを実行しないからである。（同じ kind のエントリが 2 つあること自体は
もはや障害ではない。それぞれが自身の `name` で登録される。理由はあくまで頻度であり、ブートごとに
一度のパスは再スキャンではない。）
