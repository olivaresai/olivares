...
以下のセクションでは、最初に使うコマンドが install に何を行い、何を安全に公開できるかを説明します。網羅的な一覧は、末尾のバイナリから **生成された** [完全なコマンドリファレンス](#complete-command-reference) にあり、フラグ、exit code、output contract を含みます。これは live output ではなくその build の snapshot です。push gate と CI の `lint:cli-coverage` は、source で宣言された command がこの reference のどこかに文書化されているかを確認しますが、このセクションを再導出したり、フラグ、exit code、output contract を比較したりはしません。このページとバイナリが異なる場合は、`olivares <subcommand> --help` が正式です。
...
:::note[生成リファレンスは英語のまま]
以下の技術ブロックは marker を含め、バイナリの英語出力から byte 単位でそのまま
複製されています。command 名、help description、flag、exit code、output contract は、
バイナリが実際に出力する文字列であるため、意図的に英語のままです。その周囲の編集文は
ローカライズされています。
:::
