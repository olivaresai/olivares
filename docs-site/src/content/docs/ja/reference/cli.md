---
title: "CLI リファレンス: olivares"
description: "単一の olivares バイナリの検証済みサブコマンドとフラグ。セキュアな serve のデフォルトオプションを含む。"
---

Olivares AI は `olivares` という名前の 1 つの静的 Go バイナリとして出荷されます。同じ成果物がエンジン、組み込み Web UI（API と同じオリジンから提供されます）、そしてエッジコレクタを兼ねます — 役割は実行するサブコマンドによって選択されます。このページは community（AGPL）build のコマンドサーフェスを文書化します。以下では最初に使うコマンドを説明し、末尾のバイナリから生成された [完全なコマンドリファレンス](#complete-command-reference) がその build の全コマンドを網羅します。サーフェスはまだ動いています（末尾の [安定性](#stability) を参照）。

バイナリの入手と実行方法については、[セルフホスティング](/ja/how-to/self-hosting/) を参照してください。フラグではなく環境変数に存在する設定については、[設定](/ja/reference/configuration/) を参照してください。
...
ルートコマンドは、`olivares --help` が表示する「setup/configuration」「operate」「govern」「observe/diagnose」「security」「release/upgrade」の各セクションにサブコマンドを分類します。

以下のセクションでは、最初に使うコマンドが install に何を行い、何を安全に公開できるかを説明します。網羅的な一覧は、末尾のバイナリから **生成された** [完全なコマンドリファレンス](#complete-command-reference) にあり、フラグ、exit code、output contract を含みます。生成器は community バイナリのコマンドツリー（`TestCLIRefDump`）を走査し、その結果をマークされた領域に書き込みます。push gate と CI は `lint:public-counts` 経由で `scripts/check-cli-ref-docs.sh` を実行し、フラグ、デフォルト、exit code、output contract をバイナリから再導出し、このページまたは 6 つのロケールコピーのいずれかが食い違うと失敗します。別の検査 `lint:cli-coverage` は、source で宣言されたコマンド名がこの reference のどこかに言及されていることだけを確認し、フラグは再導出しません。生成領域のコマンド名、フラグ名、help テキスト、デフォルト、exit code の意味は、バイナリ本来の英語文字列です。ロケール版でもこのブロックは人間による翻訳ではありません。デプロイしたバイナリとこのページが異なる場合は、そのバイナリ上の `olivares <subcommand> --help` が正式です。

:::note[単一バイナリ、複数の役割]
別々の「サーバー」と「エージェント」のダウンロードはありません。`olivares serve` がエンジンであり、`olivares collector` が分散トポロジのためのデータプレーンコレクタです。両者は同じソースコネクタを同じ方法でロードします — 観測値の行き先だけが異なります。[アーキテクチャ](/ja/explanation/architecture/overview/) を参照してください。
:::

## olivares version

バージョン、コミット、ビルド日付、OS/arch、Go ランタイムバージョンを stdout に出力します。

```sh
olivares version
```

バージョン文字列はビルド時に注入されます。タグ付きリリースによって生成されなかった作業ツリーからのビルドは開発バージョン（例えば `dev`）を報告するため、バージョン文字列を来歴の保証として扱わないでください — 代わりに署名済み成果物でリリースを検証してください。[リリースを検証する](/ja/how-to/verify-a-release/) を参照してください。

## olivares serve

コントロールプレーンを実行します：HTTP サーバー（REST API と、同じオリジン上の組み込み Web UI）と gRPC サーバー。**TLS はデフォルトでオン** であり、リスナーは **デフォルトで loopback** にバインドし、**デフォルトの認証情報はありません**。
...
`serve` が受け取るすべてのフラグと型・デフォルト値は、以下の生成リファレンスの
[`olivares serve`](#command-olivares-serve) にあります。ここでは、install が何を公開するかを変える 4 つだけを扱います。`--listen` と `--grpc-listen`（bind 先）、`--insecure`（transport の暗号化）、`--seed-demo`（データが実物か合成か）です。

デフォルトのストアは SQLite（純 Go、単一ノード、エアギャップ環境のインストールに適する）です。`postgres` を選択するのは、行レベルセキュリティがテナントの最後の砦を提供するマルチテナントまたはスケールアウトのデプロイメントのためです。[設定](/ja/reference/configuration/) と [セルフホスティング](/ja/how-to/self-hosting/) を参照してください。

:::caution[`--insecure` はローカルホスト開発専用]
`--insecure` は平文の HTTP と gRPC を提供します。Bearer トークンは平文トランスポート上を平文で流れるため、ローカルホストの外から到達可能ないかなるアドレスでもこのフラグを決して使用しないでください。`--insecure` の外では、gRPC サーバーは黙ってダウングレードするのではなく、TLS なしでの起動を拒否します。
:::

### `--seed-demo` はデモ専用であり、非 loopback を拒否する

`--seed-demo` は、パスワードが **公開されている**（ソースツリーに存在します）デモ管理者とともに、**合成され捏造された** estate をプロビジョニングします。これは純粋に、Web UI とエンドツーエンドテストをライブの形をしたデータに対してレンダリングするために存在します。

デモ認証情報は公開されているため、`serve` は **いかなる非 loopback バインドでも `--seed-demo` での起動を拒否** し、`127.0.0.1` にバインドするか、このフラグなしで実際のインストールを実行するよう指示するエラーで終了します。`--seed-demo` は使い捨てとして扱ってください：使い捨てのデータディレクトリを使い、大切なデータには決して向けないでください。

:::danger[`--seed-demo` を実際のインストールとして決して実行しないこと]
デモ管理者は公知のパスワードを持ちます。実際のインストールは `--seed-demo` **なし** の `serve` であり、そこではエンジンがワンタイムのセットアップトークンを発行し、あなた自身の管理者を作成します。この 2 つを混在させないでください：デモのデータディレクトリは本番のデータディレクトリではありません。
:::

### 初回起動セットアップ

ユーザーのいないインストールの初回起動時、`serve` は、ワンタイムのセットアップトークン（プレフィックス `olst_`）と、最初の管理者をブートストラップするために必要なリクエストを含むブロックを stdout に出力します。フローは次のとおりです：

1. エンジンの stdout から `olst_` トークンを読み取ります（コンテナデプロイメントでは、コンテナログから読み取ります）。
2. トークン、メールアドレス、パスワードをセットアップエンドポイント（`POST /v1/setup`）にポストすることで最初の管理者を作成します。
3. ログイン（`POST /v1/auth/login`）してセッショントークン（プレフィックス `olvs_`）を取得します。

セットアップトークンは一度だけ表示され、単回使用です。ユーザーが存在すると、トークンは発行されません。Olivares AI は **不透明（opaque）** な Bearer トークンを使用します（JWT ではありません）。API キーはプレフィックス `olvk_` を持ちます。完全な認証契約とテナント解決ルールについては、[セキュリティモデル](/ja/explanation/security/security-model/) と [API リファレンス](/reference/api/) を参照してください。
...
あるいはコンテナデプロイメントを実行し、ログからセットアップトークンを読み取ります。[セルフホスティング](/ja/how-to/self-hosting/) を参照してください。

## olivares collector

バイナリを分散トポロジのための **エッジコレクタ** として実行します。コレクタは、あなたのソース設定で指定されたソースコネクタをローカルでロードし、その観測値を gRPC 経由でリモートコアに **プッシュ** します。**インバウンドリスナーを一切開きません** — コレクタはセキュアなデフォルトのデータプレーンです：外にダイヤルするのであって、接続を受け入れません。

```sh
olivares collector --core-addr host:port [flags]
```

コレクタは 2 層でコアに対して認証します：インジェストプリンシパルを保持する Bearer トークンと、— コアが相互 TLS を強制する場合 — コレクタクライアント証明書です。これが、中央のコアが集約する一方でデータプレーンが顧客インフラ上で動作する仕組みです：失敗したコレクタが、いかなるエージェントのデータパスにも居座ることはありません。

:::note[コレクタがどのソースをロードするか]
`serve` と `collector` はともに、ランタイム起動前に環境から読み取られる同じ設定からソースコネクタを配線します。未設定または空のソースは、プロセスを失敗させるのではなく正直に警告します。ソースの設定は [ソースを接続する](/ja/how-to/connect-a-source/) と [Claude Code を接続する](/ja/how-to/connect-claude-code/) でカバーされています。設定の仕組みは [設定](/ja/reference/configuration/) にあります。
:::

collector サブコマンドは分散パスの **仕組み** です。その周辺のパッケージング（コレクタのフリート、署名済みチャート、OCI イメージ）は、この CLI ページではなくデプロイメントのストーリーの一部です — [アーキテクチャ](/ja/explanation/architecture/overview/) と [セルフホスティング](/ja/how-to/self-hosting/) を参照してください。

## olivares openapi

実行中のサーバーを必要とせずに、エンジンの OpenAPI 3.1 ドキュメントを stdout に出力します。

```sh
olivares openapi > openapi.json              # 安定コア契約
olivares openapi --beta > openapi.beta.json  # beta module-route ドキュメント
```

デフォルトではエンジンが `GET /openapi.json` で提供する安定コア契約を、diff できるよう決定論的にインデントして出力します。これは [API リファレンス](/reference/api/) と Web クライアントの型付きコード生成の信頼できる情報源です。`--beta` は **beta** module-route ドキュメント（`/v1/m/<ns>/…`）を出力し、`GET /openapi.beta.json` で提供され、[module-route リファレンス](/reference/api-beta/) でレンダリングされます。フィールドレベルの形は型付きインターフェースのままです（[モジュール概要](/ja/reference/modules/overview/) も参照）。

:::note[生成リファレンスは英語のまま]
以下の技術ブロックは marker を含め、英語ページと同じバイナリ dump から再生成され、
push のたびに gate が比較します。command 名、help description、flag、exit code、
output contract は、バイナリが実際に出力する文字列であるため、意図的に英語のままです。
これは人間による翻訳ではありません。その周囲の編集文はローカライズされています。
:::
...
これは活発に開発中の 1.0 以前の製品です。上記のサブコマンドとフラグは現在のバイナリで確認されていますが、完全な CLI サーフェスはまだ進化しています：サブコマンド、フラグ、デフォルト、出力形式は安定版リリースの前に変わる可能性があります。疑わしいときは、デプロイした正確なビルドに対して `olivares <subcommand> --help` を実行し、それをいかなるドキュメントよりも権威あるものとして扱ってください。今日実装されているものと計画されているものについては、[正直さと限界](/ja/start/honesty-and-limits/) を参照してください。REST/gRPC API サーフェス自体は [API 安定性ポリシー](/ja/reference/api-stability/) によって統治されます。
