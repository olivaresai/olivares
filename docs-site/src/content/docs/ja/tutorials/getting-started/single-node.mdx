...
- バイナリをビルドするための **Go 1.26+**（ストアは純粋 Go の SQLite なので、
  C ツールチェーンは不要）。以前のタグ付きリリースは既に署名済みアーカイブとパッケージを出荷しています。
  このツリーの canon は **v26.9.1** です。インストール面の証人は、そのタグの
  リリースジョブが走るまで GitHub アーカイブとネイティブパッケージを
  **not-published** と記録しています
  （`docs/releases/v26.9.1-install-surfaces.json`）。それらの成果物が存在するまで
  チェックアウトからビルドしてください。`main` 上の検証済みインストーラ
  （`scripts/install.sh`）はタグに依存しない公開面です。
  [リリースの検証](/ja/how-to/verify-a-release/)は、成果物が存在してからのチェーンです。
...
エンジンはデフォルトで**すべてのインターフェース**（`:8443`）にバインドします。これは
サーバーだからです。上のコマンドは `--listen 127.0.0.1:8443` を渡し、このチュートリアルの
インストールを今いるホストだけに制限しています。これは明示的な決定です — その 2 つの
フラグを外せばコンソールはネットワークから応答します。そのときは独自のイングレスと TLS を
手前に置いてください（[ハードニング](/ja/how-to/security-hardening/)を参照）。
