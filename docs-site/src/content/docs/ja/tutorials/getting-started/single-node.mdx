...
- バイナリをビルドするための **Go 1.26+**（ストアは純粋 Go の SQLite なので、
  C ツールチェーンは不要）。公開済みリリース **v26.9.0** は署名済みアーカイブとネイティブ
  パッケージを提供しています。インストール面の証人は両方を **published** と記録しています
  （`docs/releases/v26.9.0-install-surfaces.json`）。このチュートリアルはチェックアウトから
  ビルドします。代わりに公開リリースを入れるには、`main` 上の検証済みインストーラ
  （`scripts/install.sh`）を使います。[リリースの検証](/ja/how-to/verify-a-release/)は
  それらの成果物のためのチェーンです。
