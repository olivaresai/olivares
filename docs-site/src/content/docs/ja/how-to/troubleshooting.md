...
スロットルが試行を拒否したことを意味します。自身の繰り返しの失敗でロックされたアカウントか、
失敗が続くクライアントアドレスがすでに試していたアカウントです。履歴のきれいなアカウントが、
同じアドレスからの他者の失敗でロックされることはありません。代わりに 1 秒待ちます。
いずれも自動的に解除されます。上限を引き上げるのではなく、失敗の発生源を調査してください。
