...
:::note[ベータ ── 26.9.0 のイメージは公開済み]
Olivares AI は **ベータ** です。以下のイメージ座標は解決します。リリース `26.9.0` がそれらを
Docker Hub と `ghcr.io` に公開しました（インストール面の証人 `docs/releases/v26.9.0-install-surfaces.json`）。
