---
title: "Referencia de CLI: olivares"
...
El comando raíz ordena sus subcomandos en las secciones que imprime `olivares --help`:
configuración inicial y configuración, operación, gobierno, observación y diagnóstico,
seguridad, y release y actualización.

Las secciones siguientes explican los comandos que encontrarás primero — qué hacen a tu
...
Imprime la versión, el commit, la fecha de build, el OS/arch y la versión del runtime de Go por stdout.

```sh
olivares version
```

La cadena de versión se inyecta en tiempo de build. Una build a partir de un árbol de trabajo que no fue producida por una release etiquetada reporta una versión de desarrollo (por ejemplo `dev`), así que no trates la cadena de versión como una garantía de procedencia — verifica las releases con los artefactos firmados en su lugar. Consulta [Verificar una release](/es/how-to/verify-a-release/).
...
Todos los flags que acepta `serve`, con su tipo y valor por defecto, se enumeran bajo
[`olivares serve`](#command-olivares-serve) en la referencia generada siguiente. Esta sección
solo cubre los cuatro que cambian lo que expone la instalación: `--listen` y
`--grpc-listen` (dónde enlaza), `--insecure` (si el transporte está cifrado) y
`--seed-demo` (si los datos son reales).

El almacén por defecto es SQLite (Go puro, nodo único, apto para instalaciones en entorno aislado). Seleccionar `postgres` es lo que haces para despliegues multi-tenant o de escalado horizontal, donde la seguridad a nivel de fila proporciona el respaldo del tenant. Consulta [Configuración](/es/reference/configuration/) y [Autoalojamiento](/es/how-to/self-hosting/).

:::caution[`--insecure` es solo para desarrollo en localhost]
`--insecure` sirve HTTP y gRPC en texto plano. Los tokens bearer viajan en claro sobre un transporte de texto plano, así que nunca uses este flag en ninguna dirección alcanzable más allá del host local. Fuera de `--insecure`, el servidor gRPC se niega a arrancar sin TLS en lugar de degradarse en silencio.
:::

### `--seed-demo` es solo para demos y rechaza no-loopback

`--seed-demo` aprovisiona un estate **sintético, fabricado** junto con un administrador de demo cuya contraseña es **pública** (vive en el árbol de fuentes). Existe únicamente para hacer que la interfaz web y los tests de extremo a extremo se rendericen contra datos realistas.

Como la credencial de demo es pública, `serve` **se niega a arrancar con `--seed-demo` en cualquier enlace no-loopback** y sale con un error que te indica enlazar a `127.0.0.1` o ejecutar una instalación real sin el flag. Trata `--seed-demo` como desechable: usa un directorio de datos prescindible y nunca lo apuntes a datos que te importen.

:::danger[Nunca ejecutes `--seed-demo` como instalación real]
El administrador de demo tiene una contraseña públicamente conocida. Una instalación real es `serve` **sin** `--seed-demo`, donde el motor acuña un token de configuración de un solo uso y tú creas tu propio administrador. No mezcles ambos: un directorio de datos de demo no es un directorio de datos de producción.
:::

### Configuración de primer arranque

En el primer arranque de una instalación que no tiene usuarios, `serve` imprime un bloque por stdout que contiene el token de configuración de un solo uso (prefijo `olst_`) y la petición que necesitas para bootstrap del primer administrador. El flujo es:

1. Lee el token `olst_` del stdout del motor (en el despliegue con contenedor, léelo de los logs del contenedor).
2. Crea el primer administrador enviando el token, un email y una contraseña al endpoint de configuración (`POST /v1/setup`).
3. Inicia sesión (`POST /v1/auth/login`) para obtener un token de sesión (prefijo `olvs_`).

El token de configuración se muestra una vez y es de un solo uso; una vez que existe un usuario, no se acuña ningún token. Olivares AI usa tokens bearer **opacos** (no JWT); las claves de API llevan el prefijo `olvk_`. Para el contrato de autenticación completo y las reglas de resolución de tenant, consulta el [Modelo de seguridad](/es/explanation/security/security-model/) y la [Referencia de la API](/reference/api/).
...
O ejecuta el despliegue con contenedor y lee el token de configuración de los logs; consulta [Autoalojamiento](/es/how-to/self-hosting/).

## olivares collector

Ejecuta el binario como un **colector de borde** para la topología distribuida. Un colector carga localmente los conectores de fuente nombrados en tu configuración de fuentes y **empuja** sus observaciones a un core remoto sobre gRPC. No abre **ningún listener entrante** — el colector es el data plane seguro por defecto: marca hacia fuera, no acepta conexiones.

```sh
olivares collector --core-addr host:port [flags]
```

El colector se autentica ante el core en dos capas: un token bearer que porta un principal de ingesta y — cuando el core exige TLS mutuo — un certificado de cliente del colector. Así corre el data plane sobre la infraestructura del cliente mientras un core central agrega: un colector que falla nunca se sienta en el camino de datos de ningún agente.

:::note[Qué fuentes carga un colector]
Tanto `serve` como `collector` cablean sus conectores de fuente desde la misma configuración, leída del entorno antes de que arranque el runtime. Una fuente no configurada o vacía avisa honestamente en lugar de hacer fallar el proceso. La configuración de fuentes se cubre en [Conectar una fuente](/es/how-to/connect-a-source/) y [Conectar Claude Code](/es/how-to/connect-claude-code/); el mecanismo de configuración está en [Configuración](/es/reference/configuration/).
:::

El subcomando collector es el **mecanismo** del camino distribuido. El empaquetado a su alrededor (una flota de colectores, charts firmados, imágenes OCI) forma parte de la historia de despliegue en lugar de esta página de CLI — consulta [Arquitectura](/es/explanation/architecture/overview/) y [Autoalojamiento](/es/how-to/self-hosting/).

## olivares openapi

Imprime el documento OpenAPI 3.1 del motor por stdout, sin necesidad de un servidor en ejecución.
...
Por defecto emite el mismo contrato que el motor sirve en `GET /openapi.json` (las rutas
estables del núcleo), indentado de forma determinista para que la salida produzca diffs
limpios; es la fuente de verdad para la [referencia de la API](/reference/api/) renderizada
y para la generación de código tipado del cliente web. `--beta` emite el documento **beta**
de rutas de módulos (`/v1/m/<ns>/…`, servido en `GET /openapi.beta.json`, renderizado en la
[referencia de rutas de módulos](/reference/api-beta/)), reflejado a partir de las rutas que
registran los módulos, cuyas formas a nivel de campo siguen expresadas como interfaces
...
Este es un producto pre-1.0 en desarrollo activo. Los subcomandos y flags de arriba están confirmados en el binario actual, pero la superficie completa de la CLI sigue evolucionando: subcomandos, flags, valores por defecto y formatos de salida pueden cambiar antes de una release estable. Ante la duda, ejecuta `olivares <subcommand> --help` contra la build exacta que desplegaste, y trata eso como autoritativo por encima de cualquier documento. Para lo que está implementado hoy frente a lo planificado, consulta [Honestidad y límites](/es/start/honesty-and-limits/). La propia superficie de API REST/gRPC se rige por la [política de estabilidad de la API](/es/reference/api-stability/).
