...
:::note[Filosofía de configuración]
El motor se configura mediante flags y variables de entorno, no mediante un fichero de
...
`olivares serve` ejecuta el servidor HTTP REST/web y el servidor gRPC en un mismo proceso, con la interfaz web servida desde el mismo origen que la API. Los flags de más abajo son las entradas de configuración verificadas de ese comando.
...
| `--checkpoint-interval` | `1h` | Cada cuánto se escribe un checkpoint de auditoría firmado sobre cada cadena de tenant. `0` desactiva. |
| `--insecure` | off | Sirve HTTP/gRPC en texto plano. Peligroso; solo para desarrollo en localhost. |
| `--seed-demo` | off | Carga un estate de ejemplo sintético para demos/E2E. Se niega a arrancar en un enlace no-loopback. |

TLS está activado por defecto. Sin un `--tls-cert`/`--tls-key` suministrado, el motor garantiza un certificado autofirmado en el directorio de datos una vez, por adelantado, antes de que ningún listener acepte una conexión, de modo que tanto el servidor HTTP como el gRPC usen el mismo certificado y ninguno recaiga en texto plano. Cuando genera un certificado autofirmado registra `cert_fingerprint_sha256` (el resumen del certificado, lo que muestra un navegador) y `pin_sha256` (el resumen del SPKI de la hoja). `--pin-sha256` toma el segundo, en base64 o hex; la huella del certificado es otro resumen — se analiza sin error, 32 bytes en cualquiera de las dos escrituras, y luego falla en el handshake con `TLS SPKI pin mismatch`, que indica el valor que hay que usar.

:::caution[`--insecure` es solo loopback por intención]
`--insecure` sirve HTTP y gRPC en texto plano, lo que expondría los tokens bearer en el cable. El camino gRPC **falla cerrado**: fuera de `--insecure` el servidor se niega a construir un listener en texto plano en lugar de degradarse en silencio. Usa `--insecure` solo contra `127.0.0.1` durante el desarrollo local, nunca en una dirección publicada.
:::

:::danger[`--seed-demo` es sintético y autoprotegido]
`--seed-demo` aprovisiona un administrador de demo con una **contraseña pública del árbol de fuentes** y datos de estate fabricados. Es solo para demos y E2E. El motor se niega a arrancarlo en un listener no-loopback: si `--listen` o `--grpc-listen` no son una dirección loopback, el comando sale con un error. Usa un directorio de datos desechable; nunca lo apuntes a datos reales.
:::

Un listado completo de flags — incluidos los flags exclusivos de Postgres y de TLS mutuo usados en despliegues distribuidos — está en la [referencia de CLI](/es/reference/cli/). Esta página documenta la superficie de configuración común; algunos flags avanzados rigen topologías multi-nodo descritas en [la visión general de la arquitectura](/es/explanation/architecture/overview/).

## Variables de entorno

Los tres grupos siguientes son los que un operador encuentra primero, descritos con su
comportamiento. Tras ellos aparece el inventario completo, generado a partir de las propias
fuentes del motor para que no pueda quedar por detrás del binario.

### Directorio de datos

| Variable | Efecto |
| --- | --- |
| `OLIVARES_DATA_DIR` | Directorio de datos por defecto cuando no se da `--data-dir`. Sin él, el motor usa una instalación existente en `./olivares-data`; si no, `$XDG_DATA_HOME/olivares` o `~/.local/share/olivares` — nunca el directorio de trabajo actual, que es donde dejaría claves privadas. |

El directorio de datos alberga la clave de firma de auditoría, el certificado y la clave TLS, y — para el motor SQLite — el fichero del almacén. Persístelo entre reinicios.

### Cableado de fuentes reales

| Variable | Efecto |
| --- | --- |
| `OLIVARES_SOURCES_CONFIG` | Ruta a un fichero JSON que cablea las fuentes de observación reales y los proveedores de roster de identidad antes de que arranque el motor. |
...
El policy decision point de autorización se selecciona en la raíz de composición por entorno. El motor nativo de control de acceso basado en atributos (ABAC) y el control de acceso basado en roles (RBAC) gobiernan siempre; el PDP externo, cuando se selecciona, es una capa adicional de **solo restricción** que nunca puede ampliar el acceso.

| Variable | Efecto |
| --- | --- |
| `OLIVARES_PDP_ENGINE` | Selecciona el PDP externo: `cedar`, `opa` o `none` (vacío o `none` significa solo ABAC nativo). |
| `OLIVARES_PDP_CEDAR_FILE` | Solo motor Cedar: ruta al fichero de política Cedar del operador. |
| `OLIVARES_PDP_OPA_URL` | Solo motor OPA: URL base del endpoint de Open Policy Agent. |
| `OLIVARES_PDP_OPA_PATH` | Solo motor OPA: ruta de decisión consultada bajo ese endpoint. |
| `OLIVARES_PDP_OPA_TOKEN` | Solo motor OPA: token bearer para el endpoint de OPA. |
...
| `OLIVARES_CIRCUIT_BREAKER_CONFIG` | No | — | Interruptor booleano, no una ruta de archivo pese al sufijo `_CONFIG`, del disyuntor (circuit breaker) de runtime; sus reglas por tenant no se definen aquí. Solo lo leen los builds compilados con las etiquetas `enterprise` y `addon_airs`, al arrancar el motor; un cambio surte efecto tras un reinicio. Si no se define o tiene un valor que `strconv.ParseBool` acepta como false, el disyuntor permanece desactivado. Un valor que no es booleano se registra como error y también lo deja desactivado. Un valor true lo activa cuando hay datos de módulo disponibles; en caso contrario, se registra una advertencia y permanece desactivado. |
...
| `OLIVARES_COMPUTER_USE_CONFIG` | No | — | Ruta al archivo JSON de políticas del gate de computer use. Solo la leen los builds compilados con las etiquetas `enterprise` y `addon_airs`. Si no se define, el gate permanece desactivado; un archivo ilegible o que no se puede analizar hace que deniegue todo computer use hasta que se corrija el archivo y se reinicie el motor. |
...
| `OLIVARES_CONTENT_FIREWALL_CONFIG` | No | — | Ruta al archivo JSON de políticas del content firewall que inspecciona el tráfico de inferencia en el proxy inline. Solo la leen los builds compilados con las etiquetas `enterprise` y `addon_airs`. Si no se define, esa inspección permanece desactivada; un archivo ilegible o que no se puede analizar hace que el firewall deniegue toda solicitud que inspeccione hasta que se corrija el archivo y se reinicie el motor. |
...
| `OLIVARES_CREDENTIAL_MINTER_CONFIG` | No | — | Ruta a la configuración JSON del emisor de credenciales upstream por servidor (token exchange de RFC 8693) de la puerta de enlace MCP. Contiene secretos, por eso es un archivo y no un valor. La leen los builds compilados con la etiqueta `enterprise` cuando el motor arranca y construye la puerta de enlace para una URL upstream. Si no se define, se mantiene la credencial upstream estática; un archivo ilegible o que no se puede analizar deniega toda credencial upstream hasta que se corrija el archivo y se reinicie el motor. |
...
| `OLIVARES_ELICITATION_MEDIATOR_CONFIG` | No | — | Ruta al archivo JSON de políticas del mediador de elicitación MCP. Solo la leen los builds compilados con las etiquetas `enterprise` y `addon_airs`. Si no se define, la mediación permanece desactivada; un archivo ilegible o que no se puede analizar hace que el mediador deniegue toda elicitación hasta que se corrija el archivo y se reinicie el motor. |
...
| `OLIVARES_RTBF_DEPTH_CONFIG` | No | — | Ruta a la configuración JSON del coordinador de crypto-shredding para RTBF. Solo la leen los builds compilados con las etiquetas `enterprise` y `addon_reg`, al arrancar el motor. Si no se define, el crypto-shredding del open core no cambia; un archivo ilegible o que no se puede analizar bloquea todo crypto-shredding hasta que se corrija el archivo y se reinicie el motor. |
...
El motor selecciona su motor de almacén desde `--engine`.

| Motor | Cuándo usarlo | Notas |
| --- | --- | --- |
| `sqlite` (por defecto) | Binario único, nodo único, instalaciones en entorno aislado. | Almacén embebido en Go puro; cero dependencias externas. Sin `--dsn`, el fichero del almacén vive en el directorio de datos. |
| `postgres` | Despliegues multi-tenant y de escalado horizontal. | Añade aislamiento de tenant con seguridad a nivel de fila. Requiere un rol de aplicación de mínimo privilegio. |

SQLite es el valor por defecto y no necesita servicio externo. Elegir `postgres` opta por el respaldo de seguridad a nivel de fila que aísla a los tenants: el motor **se niega a arrancar** contra un superusuario de Postgres o un rol `BYPASSRLS` salvo que ese guard se sobrescriba explícitamente, porque tal rol desactivaría el respaldo de aislamiento de tenant. El overlay de Postgres de Compose aprovisiona el rol de aplicación de mínimo privilegio en la primera inicialización para que ese respaldo sea real.

:::tip[El almacén por defecto es intencionadamente aburrido]
SQLite no es aquí un valor por defecto de juguete. Es el almacén listo para entornos aislados, de cero dependencias, para la topología de nodo único, y es el almacén que ejecuta el despliegue de un solo comando con Docker Compose. Pásate a Postgres cuando necesites aislamiento multi-tenant o escala horizontal, no antes. Consulta [autoalojamiento](/es/how-to/self-hosting/) y [la visión general de la arquitectura](/es/explanation/architecture/overview/).
:::

## Intervalo de checkpoint de auditoría

El ledger de auditoría es append-only, hash-chained y anclado por checkpoints firmados con Ed25519. `--checkpoint-interval` controla cada cuánto se escribe un checkpoint firmado sobre cada cadena de tenant (por defecto `1h`; `0` desactiva el checkpointing). Se escribe un checkpoint final de apagado antes de que el almacén se cierre, de modo que la cadena queda anclada también en un apagado limpio además de en el intervalo. El camino de exportación firmada y reenvío se cubre en [reenviar la auditoría a Splunk](/es/how-to/forward-audit-to-splunk/).

## Valores por defecto seguros

Estas son las posturas en efecto sin configuración más allá de `serve`. Son la postura de seguridad por defecto del producto, no un endurecimiento opcional.
...
En una instalación nueva el motor imprime un bloque `FIRST-BOOT SETUP` por salida estándar que contiene el token de configuración de un solo uso. El administrador lo usa para crear el primer usuario y luego se autentica. Bajo Docker Compose el token se lee de los logs del contenedor:
...
El endpoint de configuración y el endpoint de login forman parte del contrato OpenAPI del producto; consulta la [referencia de la API](/reference/api/). El modelo de tokens opacos de sesión y de clave de API que hay detrás de ellos se describe en [el modelo de seguridad](/es/explanation/security/security-model/).

## Qué no cubre esta página

Esta es la superficie de configuración verificada y común. **No** enumera todos los flags avanzados para topologías multi-nodo y de TLS mutuo — esos pertenecen a los despliegues distribuidos y en entornos aislados descritos en [la visión general de la arquitectura](/es/explanation/architecture/overview/) y listados en su totalidad en la [referencia de CLI](/es/reference/cli/). Cuando un ajuste está en fase de diseño o es específico de una topología, se documenta allí en lugar de presentarse aquí como un mando estable.

Para los límites de lo que el producto observa y dónde la cobertura está escalonada por niveles, lee [honestidad y límites](/es/start/honesty-and-limits/).
