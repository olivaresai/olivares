...
| **El bind no es la protección** | El motor se liga a **todas las interfaces** por defecto: es un servidor. Lo que lo protege es TLS activado, cero credenciales por defecto y un token de configuración de un solo uso — nada de eso depende del bind. | Restrínjelo **deliberadamente** cuando quieras: `--listen 127.0.0.1:8443` para el binario, `OLIVARES_BIND=127.0.0.1` para el stack de Compose. Ponle delante tu propio ingress/TLS para un certificado que los navegadores acepten y un nombre de host válido para passkeys —ver [self-hosting](/es/how-to/self-hosting/). |
