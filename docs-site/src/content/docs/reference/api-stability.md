...
description: >-
  The versioning scheme, stability tiers, deprecation signalling (RFC 9745 /
  RFC 8594 headers) and minimum support windows for the REST API, the gRPC
  mirror, the live-ingest wire contract, the Terraform provider and the client
  SDKs.
---
...
:::note[Pre-1.0 status]
Olivares AI is pre-1.0 (see [Honesty and limits](/start/honesty-and-limits/)).
...
| [Event bus contract](/reference/events/) (AsyncAPI 3.0) — its event types are also what the eventing platform delivers to [external webhook subscriptions](/reference/events/#external-subscriptions-eventing-platform); the subscription-management routes are module routes (`/v1/m/eventing/`, in the beta document below), but each **event type** carries its own stability tier from the in-code catalog | `info.version` (`1.0.0-preview`) | **beta** (document); per-type tiers for event types |
| Module routes — `/v1/m/<ns>/…`, the product's modules, in the [beta module-route document](/reference/api-beta/) | URL major (`/v1/`); reflected from the routes the modules register | **beta** |
...
[beta document](/reference/api-beta/), kept distinct from the stable core
...
`x-stability` marker — `stable` in the [core contract](/reference/api/), `beta`
in the [module-route document](/reference/api-beta/) — and each document links
...
- **The connector SDK** (the Go contract third-party connectors build
  ...
  [Build and ship a connector](/how-to/build-a-connector/).
...
- [Using the client SDKs](/how-to/use-the-client-sdks/)
- [Build and ship a connector](/how-to/build-a-connector/) — the connector SDK
  contract and lifecycle
- [Manage as code (Terraform)](/how-to/manage-as-code/)
- [Module XIX — own API + manage-as-code](/reference/modules/xix-api-manage-as-code/)
- [Event bus (AsyncAPI 3.0)](/reference/events/)
