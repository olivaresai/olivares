...
description: >-
  The defensive control plane: deterministic guardrails that produce minimal-data
  findings, prioritized anomalies, and hash-chain-verified incident timelines —
  detective by default, with inline enforcement an opt-in, governed, off-by-default
  seam.
---
...
Module IX reacts to [`finding.reported`](/reference/events/) (persisting other modules'
high-severity findings into the tenant's security view) and to
[`guardrail.observed`](/reference/events/), the detective-input channel of already-redacted
...
runtime-ingestion layer described in the [event bus reference](/reference/events/): it is
...
- [Event bus reference](/reference/events/) — `finding.reported`, `guardrail.observed` and the runtime-ingestion channel.
- [Live-ingest — the in-process observe producer](/reference/modules/live-ingest/) — the deny-closed module that publishes the live `guardrail.observed` feed this module consumes.
- [Module III — the read/write access map](/reference/modules/iii-access-map/) — the drift this module correlates.
- [Modules catalog](/reference/modules/overview/) — module IX's layer and actuation status.
- [Govern and approve](/how-to/govern-and-approve/) — acting on findings and enforcement.
- [Forward audit to Splunk](/how-to/forward-audit-to-splunk/) — exporting verifiable evidence to a SIEM/WORM store.
- [Honesty & limits](/start/honesty-and-limits/) — what is built, observed and actuated today.
