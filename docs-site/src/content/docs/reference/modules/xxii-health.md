...
description: >-
  Reliability of the AI estate's agents and MCP servers: what is healthy, what is
  degraded or down, and what depends on what. How health is derived from signals the
  product can prove, what it materializes, and the honest limits.
---
...
XXII consumes [`edge.observed`](/reference/events/) from the bus for passive liveness
...
[`finding.reported`](/reference/events/) channel — the product-wide alert stream that
[module XV (notifications)](/reference/modules/xv-notify/) routes to Slack, PagerDuty
...
- **No actuation.** This module governs and observes by nature — it has no actuation
  surface (see [modules overview](/reference/modules/overview/)). It detects and
...
- [Event bus reference](/reference/events/) — `edge.observed` (liveness) and `finding.reported` (the signals XXII emits).
- [Module XV — output integrations & notifications](/reference/modules/xv-notify/) — routes XXII's health findings to destinations.
- [Modules overview](/reference/modules/overview/) — where XXII sits and the actuation split.
- [Architecture overview](/explanation/architecture/overview/) — the engine, the bus and the core layer.
- [Honesty & limits](/start/honesty-and-limits/) — what the product observes today versus what it actuates.
