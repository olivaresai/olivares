...
- [Event bus reference](/reference/events/) — the `guardrail.observed` / `ObservedText`
  payload (a redacted excerpt on a JSON fallback, not the sealed sum) and `edge.observed`.
- [Module IX — security, guardrails & audit](/reference/modules/ix-security/) — the
  detector chain that consumes the `guardrail.observed` feed this module publishes.
- [Module XVI — voice & realtime agents](/reference/modules/xvi-voice/) — the consumer
...
- [Honesty & limits](/start/honesty-and-limits/) — why empty halves are declared, not faked.
