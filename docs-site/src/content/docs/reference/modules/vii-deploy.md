...
description: >-
  The one module that acts on your infrastructure: it plans and governs the
  declarative lifecycle of agents and MCP servers and their wiring to the estate.
  Mutations are HITL-gated, dry-run-before-apply, and reversible — and live apply
  stays deny-closed (503) until an executor is provisioned.
---
...
Mutating apply/retire is **not live by default**. The actuation seam ([`Executor`](/reference/modules/overview/))
...
[`edge.observed`](/reference/events/) event (`Source = policy`) carrying only references
...
- [Modules catalog](/reference/modules/overview/) — the Govern/Observe vs Actuate split and the `503` seam.
- [Module III — the access map](/reference/modules/iii-access-map/) — consumes the PERMITTED wiring this module declares.
- [Event bus reference](/reference/events/) — the `edge.observed` event and its minimal-data payload.
- [Govern and approve](/how-to/govern-and-approve/) — the HITL approval flow behind every mutation.
- [Honesty & limits](/start/honesty-and-limits/) — what actuates today and what does not.
- [Architecture overview](/explanation/architecture/overview/) — where module VII sits in the Management layer.
