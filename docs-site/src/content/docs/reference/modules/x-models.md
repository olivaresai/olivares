...
description: >-
  The governance layer over the whole AI model stack — Claude, OpenAI, Gemini and
  local inference. A versioned reference catalog, capability matrix and routing
  policy that resolves a primary + fallback chain; it routes but does not yet
...
- **Enrichment of the live estate** — the module listens to the
  [`cost.sampled`](/reference/events/) stream and enriches the discovered `Model`/`Provider`
...
`claude-sonnet`, `claude-haiku`, `claude-fable`, `claude-mythos`), with deprecated/legacy
versions kept under longer prefixes so current ids resolve to the current price tier.
...
an admin-tier action distinct from the read-tier resolve. The routes are published in the
separate **beta** [module-route reference](/reference/api-beta/), not the stable core
contract; their field-level shapes live in the product's typed interfaces.

## What it consumes & produces

The module **consumes** `cost.sampled` from the [event bus](/reference/events/) to enrich
...
- [Modules catalog](/reference/modules/overview/) — where module X sits and its actuation status.
- [Access & resource map](/reference/modules/iii-access-map/) — the R/RW map and least-privilege drift.
- [Event bus reference](/reference/events/) — the `cost.sampled` event this module consumes.
- [Architecture overview](/explanation/architecture/overview/) — engine, layers and connectors.
- [Govern and approve](/how-to/govern-and-approve/) — acting on routing and governance.
- [Honesty & limits](/start/honesty-and-limits/) — the observe-broadly / actuate-on-a-subset contract.
