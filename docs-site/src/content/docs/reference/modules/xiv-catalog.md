...
description: >-
  The internal, curated registry of approved-for-the-org agents, MCP servers,
  skills, templates, models and third-party connectors. How an entry is
...
- **No actuation, no provisioning.** Module XIV records and governs the *request*; it
  ...
  until an executor is provisioned). See [Honesty & limits](/start/honesty-and-limits/).
...
- [Modules catalog](/reference/modules/overview/) — where module XIV sits and the
  govern/observe vs actuate split.
- [Govern and approve](/how-to/govern-and-approve/) — the human-in-the-loop approval flow.
- [Architecture overview](/explanation/architecture/overview/) — the engine, layers and
  the shared data model entries declare into.
- [Event bus reference](/reference/events/) — the bus this module deliberately does not
  consume.
- [Honesty & limits](/start/honesty-and-limits/) — the actuation posture across the product.
