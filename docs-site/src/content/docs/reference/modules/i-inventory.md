...
description: >-
  Passive discovery and cataloging of everything in the estate — agents, sessions,
  MCP servers, skills, tools, models, providers and non-human identities. How
  entities are materialized from observations, what the catalog records, and the limits.
---
...
[`edge.observed`](/reference/events/) and [`cost.sampled`](/reference/events/) facts onto
...
the [event bus reference](/reference/events/) and the product's typed interfaces.
...
- [Event bus reference](/reference/events/) — the `edge.observed`, `cost.sampled` and `finding.reported` events it consumes.
