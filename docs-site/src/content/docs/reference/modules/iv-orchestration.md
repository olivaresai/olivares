...
description: >-
  The observe-and-govern plane for how agents coordinate: a derived
...
already-observed access edges, a sibling of the access map ([module III](/reference/modules/iii-access-map/)),
...
[govern and approve](/how-to/govern-and-approve/).

## What it consumes & produces on the bus

It consumes exactly one channel: [`edge.observed`](/reference/events/). A
...
per edge. It produces findings on [`finding.reported`](/reference/events/):
...
- [Modules catalog](/reference/modules/overview/) — module IV's layer and honest actuation status.
- [Event bus reference](/reference/events/) — `edge.observed` in, `finding.reported` out.
- [Access & resource map](/reference/modules/iii-access-map/) — the sibling graph this one derives alongside.
- [Govern and approve](/how-to/govern-and-approve/) — the two-phase, human-in-the-loop fire.
- [Honesty & limits](/start/honesty-and-limits/) — what actuates today and what is still a seam.
- [Architecture overview](/explanation/architecture/overview/) — where module IV sits in the Intelligence layer.
