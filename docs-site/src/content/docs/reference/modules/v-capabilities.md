...
description: >-
  The capability management overlay: which MCP server exposes which tool, what its
  transport and secret references are, which agent is wired to which capability, its
  version history, and its basic connection health — governed, audited, and untrusted
  by default where the MCP spec says it must be.
---
...
connected to"*; [module III](/reference/modules/iii-access-map/) answers *"which
...
Module V is fed by the [event bus](/reference/events/), not by its own polling. It
...
- [Modules catalog](/reference/modules/overview/) — where module V sits and its actuation status.
- [Module III — access & resource map](/reference/modules/iii-access-map/) — the R/RW view this module is deliberately distinct from.
- [Event bus reference](/reference/events/) — the `edge.observed` and `finding.reported` payloads it consumes.
- [Architecture overview](/explanation/architecture/overview/) — the engine-plus-modules composition.
- [Govern and approve](/how-to/govern-and-approve/) — acting on what the catalog surfaces.
- [Honesty & limits](/start/honesty-and-limits/) — the product's honest govern-vs-actuate contract.
