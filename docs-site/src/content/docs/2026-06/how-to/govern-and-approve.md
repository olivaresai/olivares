...
description: "How an operator governs the estate: identity and permissions, the
  deny-by-default RBAC model, the restrict-only policy seam, and the
  human-in-the-loop posture where decisions are recorded in the audit ledger."
slug: 2026-06/how-to/govern-and-approve
---
...
[security model](/2026-06/explanation/security/security-model/).
:::
...
Module III is the differentiator. It is **read-first** — it observes through logs,
...
Read [the threat model](/2026-06/explanation/security/threat-model/) for what each tier can and
cannot attest.
:::
...
[modules catalog](/2026-06/reference/modules/overview/) for how identity, permissions and
...
Supported `format` values are `cef`, `leef`, `syslog`, `otlp` and `ocsf`. Every record
...
[forwarding audit to Splunk](/2026-06/how-to/forward-audit-to-splunk/) for a complete
file-tail pipeline.

The least-privilege drift these decisions act on is the access map's
permitted-vs-observed result. The [zero-to-graph tutorial](/2026-06/tutorials/zero-to-graph/)
...
* [Security model](/2026-06/explanation/security/security-model/) — privilege, tenant scoping,
  self-audit, and the minimal-data posture in full.
* [Threat model](/2026-06/explanation/security/threat-model/) — the assets, trust boundaries,
  and what each coverage tier can attest.
* [Modules catalog](/2026-06/reference/modules/overview/) — how identity, permissions and
  governance (module VI) compose with the access map (module III) and findings
  (module IX).
* [Connect a source](/2026-06/how-to/connect-a-source/) — wire the signals that drift and
