...
description: "Capture read/write access to PostgreSQL from its native pgAudit
  trail — the clean-tier signal: READ/WRITE taken verbatim from the audit CLASS,
  never inferred from SQL, with the connector reading only the log file."
sidebar:
  order: 1
slug: 2026-06/how-to/connectors/pgaudit
---
...
(see [the identity dependency](/2026-06/how-to/connect-a-source/#the-hard-dependency-per-agent-identity)).

## 2. Declare the source

In your [sources config](/2026-06/how-to/connect-a-source/#wiring-a-real-source)
...
| `shared_accounts` | no | — | comma-separated roles / application\_names that are shared; their edges are honestly marked `approximate` |
...
![The access map with the Permitted-vs-Observed drift panel open: pgAudit edges render read/write with clean-tier badges, and unexpected accesses are listed on the right.](../../../../../assets/console/2026-06/access-map-drift-light.png)

This capture is the real console over the seeded demo estate — the same view,
with your tables and roles, is what a wired pgAudit shows. Note the honesty
axes on every edge: `attributed` versus `approximate` (a shared pool
identity), and the per-resource coverage badge.
...
* [Connect a source](/2026-06/how-to/connect-a-source/) — the connector model and the
  honest-tier taxonomy.
* [CloudTrail](/2026-06/how-to/connectors/cloudtrail/) — the same clean-tier idea for
  S3 objects.
