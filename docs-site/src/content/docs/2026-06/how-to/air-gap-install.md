...
outbound calls at boot**, so nothing inside the gap reaches the internet.
...
scripts/airgap-mirror.sh \
  --bundle olivares-airgap-v1.2.3.tar.gz \
...
> default), so nothing inside the gap reaches the internet.
...
* [Verify what you downloaded](/2026-06/how-to/verify-a-release/) — the non-air-gapped
  verification chain (signature, SBOM, OpenVEX, SLSA).
* [Self-host the control plane](/2026-06/how-to/self-hosting/) — the single-binary, Compose
