...
description: "Encrypted, ledger-continuity-safe backups with olivares dr:
  scheduled bundles for SQLite and Postgres, the restore that verifies the
  chain, the drill you can run without touching production — and the two keys
  that decide whether your evidence survives."
slug: 2026-06/how-to/backup-and-restore
---
...
command (`--engine postgres --dsn …`), or hand it a pre-made dump with
`--snapshot-file`. For near-zero RPO, `--pitr-ref` produces a keys+manifest
...
* A Postgres backup without `--admin-dsn` (a dedicated `NOSUPERUSER
  BYPASSRLS` role) warns that the captured tenant set may be RLS-limited and
  **incomplete** — provision the admin role for full multi-tenant coverage.

**Scheduling:** the Compose stack ships a
[backup profile](/2026-06/tutorials/getting-started/docker-compose/#3-encrypted-dr-backups-the-backup-profile),
the Helm chart a
[CronJob](/2026-06/tutorials/getting-started/kubernetes/#4-scheduled-encrypted-backups);
...
[CLI reference](/2026-06/reference/cli/); for the failure-mode walkthroughs, the
[troubleshooting page](/2026-06/how-to/troubleshooting/).
