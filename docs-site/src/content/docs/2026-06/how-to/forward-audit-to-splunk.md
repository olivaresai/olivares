...
description: Get the control plane's governance findings and its tamper-evident
  audit ledger into Splunk by tailing a file with a Universal Forwarder —
  without a native Splunk-to-Splunk emitter. Honest about which stream is which.
slug: 2026-06/how-to/forward-audit-to-splunk
---
...
([push to your SIEM](/2026-06/how-to/cookbook/push-to-siem/)). This page documents the
...
|---|---|---|
| **Governance / findings** | the notification stream module IX routes (health, spend, security, compliance findings) | the `filelog` output connector appends it to a file; or `splunkhec` pushes it; or an [eventing sink](/2026-06/how-to/cookbook/push-to-siem/) subscribed to `finding.reported` |
...
| `format` | per-line format: `json` | `cef` | `leef` | `syslog` | `otlp` | `ocsf` | `asim` (default `json`) |
...
Supported `format` values are `cef`, `leef`, `syslog`, `otlp` and `ocsf`. Line
formats (`cef`/`leef`/`syslog`) stream as `text/plain`; `otlp`/`ocsf` stream as
...
the bad-request message now both name all five formats.
:::
...
or an `{"export_complete":true,...}` JSON line for `otlp`/`ocsf`. **Its absence means
...
* ✅ **File-tail forwarding** (UF tails a file) — supported, for both streams.
* ✅ **Splunk HEC push** — supported for the findings stream (`splunkhec`
  ...
  at-least-once) — see [push to your SIEM](/2026-06/how-to/cookbook/push-to-siem/).
* ✅ **Offline ledger re-verification** — both the pull export and the push pump
  carry the hash-chain fields verbatim, so a SIEM can re-verify integrity.
* ❌ **Native Splunk S2S emitter** — not implemented (post-v1).
* ❌ **Automatic ledger *file* sink** — to get the ledger into a local file you
