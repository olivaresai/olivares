...
slug: 2026-06/explanation/open-core-and-licensing
---
...
- **The enterprise threat-intel add-on is not a live "feed".** It ships a base catalog
  ...
  release cadence.
- **The commercial licence is not "available self-serve".** No checkout exists: every
  commercial object is blocked behind ratification, counsel review and a tested gate
  (the privately-maintained pricing canon, `commerce_inventory.gate`). The live route is
...
it sits a small, **additive** commercial line in `enterprise/`, built only with
`-tags enterprise` and absent from the public binary. The commercial license is
**both** a legal exception to copyleft **and** the entitlement to that line — so
the open and commercial editions are **not** identical, while nothing published
open is ever moved behind the wall (the GitLab `ee/` model, not a feature paywall
on the core).
...
| `modules/` | **AGPL-3.0-only** | the 28 modules (inventory, the R/RW map, FinOps, evals, guardrails, …) |
...
| `enterprise/` | **commercial** | additive add-ons, build-tag gated, never in the public binary: multi-IdP federation, content firewall/DLP, hook hardening, threat-intel feed, server-tool egress, CyberArk Conjur, incident close-loop, the seat entitlement (`LicenseRef-Olivares-Commercial`) |
...
* **A commercial license.** Organizations that need to avoid the AGPL's obligations
  ...
  commercial license — available self-serve, with **enterprise@olivares.ai** for
  larger arrangements. The same license is also the entitlement to the additive
  `enterprise/` add-ons above.
...
* **User seats** — the community build admits the bootstrap superadmin **plus one
  more** active account; provisioning a further account returns
  `user_cap_requires_enterprise`. The enterprise build lifts the cap to the seat
  count attested by a commercial license. This is a fair-monetization edition
  boundary, not a security control — existing accounts over the limit are never
  removed, and because the open binary is AGPL the check lives in open source.

See [Honesty & limits](/2026-06/start/honesty-and-limits/) for the full
open-vs-enterprise picture.
...
*consumed* rather than displayed is the closed enterprise build — for example its
seat policy reads the attested seat count to lift the community user cap — a local
decision in the commercial edition, never a check in the open binary. So the open
...
* [Security model](/2026-06/explanation/security/security-model/) — why attestation-only
