...
description: Account for AI spend from the cost stream, slice it by any
  attribution dimension, forecast the period, and enforce budgets that deny the
  spend at the cap — money-free on the wire, opt-in and fail-open. What it does,
  and its limits.
slug: 2026-06/reference/modules/xi-finops
---
...
Each `cost.sampled` event (a `CostSample` — see the [event bus](/2026-06/reference/events/)) is
recorded two ways:
...
FinOps **consumes** `cost.sampled` off the [event bus](/2026-06/reference/events/) and **produces**
...
written to the append-only ledger and audited. See [Govern and approve](/2026-06/how-to/govern-and-approve/).
...
* [Event bus reference](/2026-06/reference/events/) — the `cost.sampled` / `CostSample` and `finding.reported` payloads.
* [Modules catalog](/2026-06/reference/modules/overview/) — where module XI sits and its honest actuation status.
* [Architecture overview](/2026-06/explanation/architecture/overview/) — the engine, layers and the cost stream.
* [Govern and approve](/2026-06/how-to/govern-and-approve/) — acting on a budget-denied action.
* [Honesty & limits](/2026-06/start/honesty-and-limits/) — the deny-closed-seam policy across modules.
