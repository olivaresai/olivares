---
title: "Manage Olivares AI as code (Terraform)"
description: >-
  Declare and reconcile control-plane objects — agents, policies, identity
  bindings and deployments — with the Olivares AI Terraform/OpenTofu provider,
  authenticated by an opaque API token against the engine's REST API.
---
...
reconcile real infrastructure. See [module VII](/reference/modules/vii-deploy/) and
...
- [API stability policy](/reference/api-stability/) — the versioning/deprecation commitment the provider relies on (it warns once per run when a response carries a deprecation signal).
- [Module XIX — own API + manage-as-code](/reference/modules/xix-api-manage-as-code/).
- [Module VII — deployment & integration](/reference/modules/vii-deploy/) — the 503 seam caveat above.
- [Govern and approve](/how-to/govern-and-approve/) — how policy and approvals govern what you declare.
