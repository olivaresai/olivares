...
description: >-
  Capture read/write access to S3 objects from CloudTrail data events — the
  readOnly flag taken verbatim, the IAM principal as the origin, honest
...
[pgAudit](/how-to/connectors/pgaudit/) for Postgres.
...
[`aws` cloud-discovery connector](/reference/connectors/), not this one.
...
- **An assumed role shared by many callers cannot name the real caller.**
  ...
  identity ([the identity dependency](/how-to/connect-a-source/#the-hard-dependency-per-agent-identity)).
...
- [pgAudit](/how-to/connectors/pgaudit/) — the same clean-tier discipline for
  PostgreSQL.
- [Connect a source](/how-to/connect-a-source/) — the connector model.
- [Connectors & coverage tiers](/reference/connectors/) — where every store
  honestly sits.
