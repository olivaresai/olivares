...
as a live session process is the D04 Community runtime seam
(`modules/sessions/cliruntime`, hub PR #2547). This hour does not duplicate
...
self-signed certificate. An interactive operator runs `olivares quickstart`
instead (TLS on, loopback-only, no default credentials). Measured on this box
on 2026-09-17, `quickstart --quiet` printed the setup token in **3 s**.
...
The printed banner still shows `https://127.0.0.1:PORT`. Open
`https://localhost:PORT` before you enroll a passkey. A browser refuses an IP
as a WebAuthn relying party. The product already says so in the address advice
...
`olivares agent tool install` from D05).
...
A live official-CLI session (PTY, attach, stop) is D04, not this page. This
hour proves the hook PEP path that D04 reuses.
...
false from v26.10 (D19).** The console now has a Providers screen that accepts the
...
The D04 runtime contract lives in hub PR #2547.

## Gaps this lane closed (measured 2026-09-17)
...
Passkey-at-IP advice was already printed (`cmd/olivares/consoleaddr.go`). This
lane did not re-work that paragraph.
