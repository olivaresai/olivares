...
description: >-
  Operator steps to run Olivares AI securely: keep the secure defaults, govern
  destructive actions with human-in-the-loop approvals, verify a release before
  running it, and keep your evidence off-box. Defensive posture, by design.
---
...
[threat model](/explanation/security/threat-model/) explain the assets, trust
...
guarantee — see [govern and approve](/how-to/govern-and-approve/). The operational steps:
...
[verify what you downloaded](/how-to/verify-a-release/). The one rule that has no
exceptions:
...
- **No mandatory telemetry, no control-plane egress by default.** The data plane (the
  collectors) always runs on your infrastructure, and the access map stores **relations,
  never payloads, secrets or PII** — minimal-data is a property of the wire, not a
  setting. What crosses your perimeter is what **you** configure to cross it — calls to
  your model APIs, the SIEM/webhook outputs you wire (the off-box export above), an
  external embedding provider if you provision one. That is the structural argument for
  data residency, GDPR and air-gapped operation; it is a description of the architecture
  and of your configuration, **not a guarantee**.

## Related

- [Security model](/explanation/security/security-model/) — privilege, tenant scoping, self-audit, minimal-data.
- [Threat model](/explanation/security/threat-model/) — assets, trust boundaries, and what each coverage tier can attest.
- [Govern and approve](/how-to/govern-and-approve/) — the RBAC/PDP model and the approval workflow in depth.
- [Verify what you downloaded](/how-to/verify-a-release/) — the full release-verification chain.
- [Self-hosting](/how-to/self-hosting/) and [air-gap install](/how-to/air-gap-install/) — the deployment topologies.
