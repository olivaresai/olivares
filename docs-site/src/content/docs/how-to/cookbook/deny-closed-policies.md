...
description: >-
  Wire the restrict-only policy decision point: a Cedar forbid overlay or a
  permit-by-default OPA policy, validated and dry-run before publishing —
  policies that can only take access away, never widen it.
sidebar:
  order: 1
---
...
([the model](/how-to/govern-and-approve/#the-policy-seam-abacpdp-only-restricts)).
