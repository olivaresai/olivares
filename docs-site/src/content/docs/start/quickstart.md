...
title: Quickstart
description: >-
  From zero to a populated read/write access graph with a real
  Permitted-vs-Observed drift result in about five minutes — first on the bundled
...
credentials, a one-time setup token, TLS), go to [self-hosting](/how-to/self-hosting/).
For a guided UI walkthrough, see the [zero-to-graph tutorial](/tutorials/zero-to-graph/).
...
[self-hosting](/how-to/self-hosting/).
:::

## 1. Build the single binary

From a checkout of the repository (needs Go 1.26+, [Task](https://taskfile.dev) and
pnpm — `task build` bundles the web UI before compiling; the store is pure-Go SQLite,
so no C toolchain):
...
:::tip[A key differentiated capability]
...
[connect a source](/how-to/connect-a-source/). The demo estate shows `firm` edges
...
- **Run it for real:** the getting-started tutorials walk every install
  ...
  [Kubernetes/Helm](/tutorials/getting-started/kubernetes/) and
  [air-gapped](/tutorials/getting-started/air-gapped/);
  [self-hosting](/how-to/self-hosting/) is the decision page across them.
- **Feed it real signals:** [connect a source](/how-to/connect-a-source/) and the
  [connectors catalog](/reference/connectors/) — what each source observes, its honest
  coverage tier, and how to wire identity so attribution becomes `firm`.
- **Harden it:** [security hardening](/how-to/security-hardening/) — secure defaults,
  human-in-the-loop approvals, and verifying a release before you run it.
- **Know the limits:** [Honesty & limits](/start/honesty-and-limits/) — what runs
