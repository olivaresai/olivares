...
повторных сканирований. Каждая запись регистрируется под **своим собственным `name`**, поэтому
несколько записей одного вида работают рядом: `okta` и `entra` обслуживает один коннектор `idp`,
и это по-прежнему два разных источника, у каждого своя конфигурация, свой статус и свой жизненный
цикл. Уникальным должно быть имя, а не коннектор. Членства в группах/ролях путешествуют только в
типизированном снимке реестра, никогда как рёбра.
...
`as_source: true`, который запускает сканирование лишь однажды за загрузку. (Две записи одного
вида больше не являются препятствием: каждая регистрируется под своим собственным `name`.
Причина — периодичность: один проход за загрузку не является повторным сканированием.)
