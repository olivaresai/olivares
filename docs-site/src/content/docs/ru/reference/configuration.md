...
| Адрес привязки | Все интерфейсы | `--listen` и `--grpc-listen` по умолчанию — `:8443` и `:8444`. Ограничивайте движок его собственным хостом намеренно: `--listen 127.0.0.1:8443 --grpc-listen 127.0.0.1:8444`. |
...
:::caution[Достижим по умолчанию, ограничивается по намерению]
Привязки по умолчанию принимают соединения на всех интерфейсах: это сервер, и защищают его включённый TLS, отсутствие учётных данных по умолчанию и одноразовый токен настройки. Намеренное решение оператора — это ограничение: `--listen 127.0.0.1:8443` для бинарника, `OLIVARES_BIND=127.0.0.1` для стека Compose. Никогда не сочетайте не-loopback-привязку с `--insecure`: движок отказывается от такой комбинации.
:::
