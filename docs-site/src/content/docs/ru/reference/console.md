...
description: >-
  Все маршруты консоли Olivares AI, проиндексированные по историческим категориям
  разделов, с требуемым разрешением RBAC и справочной страницей, которую открывает
...
Текущая боковая панель — это **девять областей с секциями**, а не пять разделов. После
Обзора она показывает области, которые этот субъект действительно может открыть, у каждой
есть страница каталога и раскрываемые модули, а внизу закреплена ссылка Настройки. Фильтр
навигации заменяет это сгруппированное дерево ранжированным списком разрешённых совпадений;
палитра команд использует тот же индекс, тот же порядок и ту же проекцию прав. Каталог
области — страница ссылок (записей, которые разрешают ваши права), а не живое чтение
возможности, доступности или готовности.
...
что видите в продукте. Таблицы ниже по-прежнему группируют эти строки по пяти историческим
категориям разделов (Эксплуатация, Автоматизация, Подключение, Управление, Подтверждение)
плюс вход, настройка и учётная запись. Это индекс, а не текущий порядок боковой панели.
Девять страниц каталогов областей сейчас стоят вместе с маршрутами, смонтированными вне
реестра функций.
...
Следующие заголовки — эти исторические категории разделов в порядке сгенерированного
индекса, а не порядок боковой панели. Текущая боковая панель перечисляет области так:
...
| Наблюдаемость и свидетельства | `/areas/observation` | Каталог области «Наблюдаемость и свидетельства»: состояние и активность, затраты и внедрение, аудит и записи, оценка и доказательства. Показывает записи, разрешённые вашими правами. | любой вошедший пользователь | — |
| Безопасность и идентичность | `/areas/security-identity` | Каталог области «Безопасность и идентичность»: идентификация и доступ, политики, защита и реагирование, границы управления. Показывает записи, разрешённые вашими правами. | любой вошедший пользователь | — |
