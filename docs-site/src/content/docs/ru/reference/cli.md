...
- **Loopback по умолчанию.** И HTTP-, и gRPC-слушатели по умолчанию привязаны к `127.0.0.1`. Выставление control plane за пределы локального хоста — это намеренное изменение, которое вы делаете, задав не-loopback-привязку и поставив перед ним собственный ingress.
