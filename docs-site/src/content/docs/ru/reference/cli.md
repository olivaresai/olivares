...
коды выхода и контракт вывода. Это снимок собранного бинарника, а не живой вывод: push-gate
и CI запускают `lint:cli-coverage`, который проверяет, что объявленные в исходниках команды
документированы где-либо в этом справочнике, и называет отсутствующие, но не выводит этот
раздел заново и не сравнивает флаги, коды выхода или контракты вывода. Если страница
расходится с бинарником, авторитетен `olivares <subcommand> --help`.
...
Следующий технический блок побайтно скопирован из английского вывода бинарного файла,
включая маркеры. Имена команд, описания help, флаги, коды выхода и контракты вывода
намеренно остаются на английском, поскольку именно эти строки выдаёт бинарный файл;
