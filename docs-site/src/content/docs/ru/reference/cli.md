---
title: "Справочник CLI: olivares"
description: "Проверенные подкоманды и флаги единого бинарника olivares, включая безопасные по умолчанию опции serve."
---

Olivares AI поставляется как один статический Go-бинарник с именем `olivares`. Один и тот же артефакт — это движок, встроенный веб-интерфейс (выдаваемый с того же origin, что и API) и edge-коллектор — роль выбирается подкомандой, которую вы запускаете. Эта страница документирует командную поверхность community-сборки (AGPL). Разделы ниже объясняют команды, с которыми вы столкнётесь первыми; [полный справочник команд](#complete-command-reference) в конце генерируется из бинарника и охватывает каждую команду этой сборки. Поверхность всё ещё движется (см. [Стабильность](#stability) в конце).

О том, как получить и запустить бинарник, см. [Self-hosting](/ru/how-to/self-hosting/). О конфигурации, которая живёт в переменных окружения, а не во флагах, см. [Конфигурация](/ru/reference/configuration/).
...
Корневая команда сортирует подкоманды по разделам, которые печатает `olivares --help`:
настройка и конфигурация, эксплуатация, управление, наблюдение и диагностика,
безопасность, а также релиз и обновление.

Следующие разделы объясняют команды, с которыми вы столкнётесь первыми: как они
воздействуют на установку и что безопасно публиковать — того таблица не передаёт. Полный
список находится в [полном справочнике команд](#complete-command-reference) в конце
страницы: он **сгенерирован из бинарника** и охватывает каждую команду сборки, её флаги,
коды выхода и контракт вывода. Генератор обходит дерево команд community-сборки
(`TestCLIRefDump`) и записывает этот обход в помеченную область. Push-gate и CI запускают
эту проверку через `lint:public-counts`, который вызывает `scripts/check-cli-ref-docs.sh`:
скрипт заново выводит флаги, значения по умолчанию, коды выхода и контракты вывода из
бинарника и падает, если эта страница или любая из шести локализованных копий расходится
с бинарником. Отдельная проверка `lint:cli-coverage` лишь удостоверяется, что имена
команд, объявленные в исходниках, где-то упомянуты в этом справочнике; флаги она заново
не выводит. Имена команд, имена флагов, тексты help, значения по умолчанию и смыслы
кодов выхода в сгенерированной области — это исходные английские строки бинарника,
в том числе на локализованных копиях этой страницы, где этот блок не является
человеческим переводом. Если развёрнутый бинарник и эта страница расходятся, авторитетен
`olivares <subcommand> --help` на этом бинарнике.

:::note[Один бинарник, несколько ролей]
Нет отдельной загрузки «сервера» и «агента». `olivares serve` — это движок; `olivares collector` — это коллектор плоскости данных для распределённой топологии. Оба загружают одни и те же source-коннекторы одинаковым образом — различается только то, куда уходят наблюдения. См. [Архитектура](/ru/explanation/architecture/overview/).
:::

## olivares version

Выводит версию, коммит, дату сборки, ОС/архитектуру и версию Go-рантайма в stdout.

```sh
olivares version
```

Строка версии внедряется во время сборки. Сборка из рабочего дерева, которое не было произведено тегированным релизом, сообщает версию для разработки (например `dev`), поэтому не воспринимайте строку версии как гарантию происхождения — вместо этого проверяйте релизы через подписанные артефакты. См. [Проверка релиза](/ru/how-to/verify-a-release/).

## olivares serve

Запускает control plane: HTTP-сервер (REST API плюс встроенный веб-интерфейс на том же origin) и gRPC-сервер. **TLS включён по умолчанию**, слушатели привязываются к **loopback по умолчанию**, и **учётных данных по умолчанию нет**.
...
Это свойства `serve`, а не опции:

- **TLS включён по умолчанию.** Если сертификат не предоставлен, движок генерирует самоподписанный сертификат в каталоге данных и логирует и SHA-256-отпечаток сертификата, и — под именем `pin_sha256` — SPKI-пин конечного сертификата; клиенты либо доверяют сертификату, либо передают это значение `pin_sha256` в `--pin-sha256`. Это два разных хэша разных байтов: отпечаток сертификата НЕ является пином. gRPC-сервер падает закрыто (fail closed): вне `--insecure` он не запустится в открытом виде.
- **Все интерфейсы по умолчанию.** И HTTP-, и gRPC-слушатели по умолчанию привязаны к wildcard-адресу — `--listen :8443` и `--grpc-listen :8444`, то есть `0.0.0.0` и, где ядро поддерживает IPv6, `::`, — потому что консоль является сервером; `quickstart` использует тот же default. Намеренное изменение — ограничить её этим хостом: `--listen 127.0.0.1:8443 --grpc-listen 127.0.0.1:8444` или `OLIVARES_BIND=127.0.0.1` для стека Compose. Защитные механизмы не перемещаются вместе с привязкой: TLS остаётся включённым, первичная настройка остаётся за одноразовым токеном, `--insecure` по-прежнему отклоняется вне хоста, если `--insecure-allow-public-bind` не объявляет терминацию TLS перед движком, а `--seed-demo` по-прежнему работает только на loopback.
- **Учётных данных по умолчанию нет.** При свежей установке без пользователей движок выпускает **одноразовый setup-токен** (префикс `olst_`) и печатает его **только в stdout** (никогда в логи). Вы создаёте первого администратора, отправив этот токен на setup-эндпойнт, затем входите в систему. См. [Настройка при первом запуске](#настройка-при-первом-запуске).

### Флаги

Все флаги `serve`, их типы и значения по умолчанию перечислены в сгенерированном
справочнике ниже, в разделе [`olivares serve`](#command-olivares-serve). Здесь рассмотрены
только четыре флага, меняющие то, что выставляет установка: `--listen` и
`--grpc-listen` (адрес привязки), `--insecure` (шифруется ли транспорт) и `--seed-demo`
(реальны ли данные).

Хранилище по умолчанию — SQLite (чистый Go, одноузловое, подходящее для air-gapped-установок). Выбор `postgres` — это то, что вы делаете для мульти-тенантных или scale-out-развёртываний, где row-level security обеспечивает страховку по тенантам. См. [Конфигурация](/ru/reference/configuration/) и [Self-hosting](/ru/how-to/self-hosting/).

:::caution[`--insecure` только для локальной разработки]
`--insecure` выдаёт HTTP и gRPC в открытом виде. Bearer-токены передаются открытым текстом по незашифрованному транспорту, поэтому никогда не используйте этот флаг на любом адресе, достижимом за пределами локального хоста. Вне `--insecure` gRPC-сервер отказывается запускаться без TLS, а не молча понижает уровень.
:::

### `--seed-demo` только для демо и отказывается от не-loopback

`--seed-demo` провижинит **синтетический, сфабрикованный** estate вместе с демо-администратором, чей пароль **публичен** (он лежит в дереве исходного кода). Он существует исключительно для того, чтобы веб-интерфейс и end-to-end-тесты рендерились против данных живой формы.

Поскольку демо-учётные данные публичны, `serve` **отказывается запускаться с `--seed-demo` на любой не-loopback-привязке** и завершается с ошибкой, направляющей вас привязаться к `127.0.0.1` или запустить настоящую установку без флага. Относитесь к `--seed-demo` как к одноразовому: используйте расходный каталог данных и никогда не направляйте его на данные, которые вам дороги.

:::danger[Никогда не запускайте `--seed-demo` как настоящую установку]
У демо-администратора публично известный пароль. Настоящая установка — это `serve` **без** `--seed-demo`, где движок выпускает одноразовый setup-токен, и вы создаёте собственного администратора. Не смешивайте эти два режима: демо-каталог данных — это не продакшен-каталог данных.
:::

### Настройка при первом запуске

При первом запуске установки, в которой нет пользователей, `serve` печатает в stdout блок, содержащий одноразовый setup-токен (префикс `olst_`) и запрос, который нужен для бутстрапа первого администратора. Поток таков:

1. Прочитайте токен `olst_` из stdout движка (при контейнерном развёртывании читайте его из логов контейнера).
2. Создайте первого администратора, отправив токен, email и пароль на setup-эндпойнт (`POST /v1/setup`).
3. Войдите в систему (`POST /v1/auth/login`), чтобы получить токен сессии (префикс `olvs_`).

Setup-токен показывается один раз и одноразов; как только пользователь существует, токен не выпускается. Olivares AI использует **непрозрачные** (opaque) bearer-токены (не JWT); API-ключи несут префикс `olvk_`. Полный контракт аутентификации и правила разрешения тенанта см. в [Модели безопасности](/ru/explanation/security/security-model/) и [Справочнике API](/reference/api/).

### Запуск

```sh
# Соберите, затем запустите (цели "task serve" / "task run" нет).
task build
./bin/olivares serve
# Прочитайте одноразовый setup-токен olst_ из stdout этого процесса.
```

Либо запустите контейнерное развёртывание и прочитайте setup-токен из логов; см. [Self-hosting](/ru/how-to/self-hosting/).

## olivares collector

Запускает бинарник как **edge-коллектор** для распределённой топологии. Коллектор локально загружает source-коннекторы, названные в вашей конфигурации источников, и **отправляет** их наблюдения в удалённое ядро по gRPC. Он не открывает **никакого входящего слушателя** — коллектор является плоскостью данных, безопасной по умолчанию: он сам набирает наружу, а не принимает соединения.

```sh
olivares collector --core-addr host:port [flags]
```

Коллектор аутентифицируется в ядре в два слоя: bearer-токеном, несущим ingest-принципала, и — когда ядро принудительно применяет mutual TLS — клиентским сертификатом коллектора. Так плоскость данных работает на инфраструктуре заказчика, пока центральное ядро агрегирует: упавший коллектор никогда не находится на пути данных какого-либо агента.

:::note[Какие источники загружает коллектор]
И `serve`, и `collector` подключают свои source-коннекторы из одной конфигурации, читаемой из окружения до старта рантайма. Ненастроенный или пустой источник предупреждает честно, а не валит процесс. Настройка источников описана в [Подключение источника](/ru/how-to/connect-a-source/) и [Подключение Claude Code](/ru/how-to/connect-claude-code/); механизм конфигурации — в [Конфигурации](/ru/reference/configuration/).
:::

Подкоманда collector — это **механизм** распределённого пути. Упаковка вокруг него (флот коллекторов, подписанные чарты, OCI-образы) относится к истории развёртывания, а не к этой странице CLI — см. [Архитектура](/ru/explanation/architecture/overview/) и [Self-hosting](/ru/how-to/self-hosting/).

## olivares openapi

Выводит документ OpenAPI 3.1 движка в stdout, без необходимости в запущенном сервере.
...
По умолчанию команда выдаёт тот же контракт, который движок обслуживает по
`GET /openapi.json` (стабильные основные пути), с детерминированными отступами для чистого
diff. Это источник истины для отрендеренного [Справочника API](/reference/api/) и генерации
типизированного кода веб-клиента. `--beta` выдаёт **beta**-документ маршрутов модулей
(`/v1/m/<ns>/…`, обслуживается по `GET /openapi.beta.json` и отображается в
[справочнике маршрутов модулей](/reference/api-beta/)); он отражается из регистрируемых
модулями маршрутов, а формы уровня полей по-прежнему выражены типизированными интерфейсами
(см. также [Обзор модулей](/ru/reference/modules/overview/)).

:::note[Сгенерированный справочник остаётся на английском]
Следующий технический блок, включая маркеры, заново генерируется из того же дампа
бинарника, что и английская страница, и gate сравнивает его при каждом push. Имена
команд, описания help, флаги, коды выхода и контракты вывода намеренно остаются на
английском: именно эти строки выдаёт бинарный файл, это не человеческий перевод.
Редакционное руководство вокруг блока локализовано.
:::
...
| `--grpc-listen` | `string` | `:8444` | gRPC listen address; the default is every interface, bind 127.0.0.1:8444 to restrict it |
...
| `--listen` | `string` | `:8443` | HTTP (REST + console) listen address; the default is every interface (0.0.0.0), bind 127.0.0.1:8443 to restrict it |
...
#### Command: olivares first-boot

Show this installation's console address and the state of first setup
...
|---|---|---|---|
| `--data-dir` | `string` | — | data directory (default $OLIVARES_DATA_DIR, an existing ./olivares-data, else $XDG_DATA_HOME/olivares or ~/.local/share/olivares) |
| `--new-token` | `bool` | `false` | mint a replacement one-time setup token and print it once; refused after an administrator exists |
...
| `--grpc-listen` | `string` | `:8444` | gRPC listen address when --start is used |
...
Это pre-1.0-продукт в активной разработке. Подкоманды и флаги выше подтверждены в текущем бинарнике, но полная поверхность CLI всё ещё развивается: подкоманды, флаги, значения по умолчанию и форматы вывода могут измениться до стабильного релиза. В случае сомнений запускайте `olivares <subcommand> --help` против той точной сборки, которую вы развернули, и считайте это авторитетнее любого документа. О том, что реализовано сегодня против запланированного, см. [Честность и ограничения](/ru/start/honesty-and-limits/). Сама поверхность REST/gRPC API регулируется [политикой стабильности API](/ru/reference/api-stability/).
