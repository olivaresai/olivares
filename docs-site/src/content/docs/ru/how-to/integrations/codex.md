...
Эта страница — путь **коннектора и PEP-хука**. Она не запускает официальную
CLI Codex как управляемого потомка. Этот путь эксплуатации —
[Управлять сеансом провайдера](/how-to/operate-provider-sessions/)
(`OLIVARES_SESSION_RUNTIME_CODEX_BIN` плюс профиль провайдера;
`CHANGELOG.md` `[26.9.0]`).
