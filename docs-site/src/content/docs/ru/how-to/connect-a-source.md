...
Несколько источников **одного** вида коннектора можно зарегистрировать и
запускать сразу (`CHANGELOG.md` `[26.9.0]` Fixed). Runtime индексировал каждый
источник по имени Descriptor коннектора, поэтому вторая строка roster того же
вида сохранялась и показывалась — и движок её отклонял. Источники теперь
регистрируются под именем оператора (`name` строки roster), а descriptor
коннектора хранится рядом. `grok-home-a` и `grok-home-b` открываются со своей
конфигурацией и ссылками на учётные данные, запускают свой экземпляр и процесс
и независимо ротируются, падают и останавливаются. `sources plan` / `validate`
больше не объявляют ограничение «один экземпляр на коннектор», которое движок
перестал применять. Две записи идентичности, которые обслуживает один
коннектор (`okta` и `entra` делят `idp`) с `as_source: true`, теперь
подключают обе. Само по себе это не разделяет два сеанса провайдера или home
конфигурации от конца до конца — это
