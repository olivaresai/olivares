---
title: "Référence CLI : olivares"
description: "Sous-commandes et drapeaux vérifiés du binaire unique olivares, y compris les options de serve sécurisées par défaut."
---

Olivares AI est livré sous la forme d'un unique binaire Go statique nommé `olivares`. Le même artefact est le moteur, l'interface web embarquée (servie depuis la même origine que l'API) et le collecteur de périphérie — le rôle est sélectionné par la sous-commande que vous exécutez. Cette page documente la surface de commandes du build communautaire (AGPL). Les sections ci-dessous expliquent les commandes que vous rencontrerez d'abord ; la [Référence complète des commandes](#complete-command-reference) à la fin est générée à partir du binaire et couvre chaque commande de ce build. La surface bouge encore (voir [Stabilité](#stability) à la fin).

Pour savoir comment obtenir et exécuter le binaire, voir [Auto-hébergement](/fr/how-to/self-hosting/). Pour la configuration qui vit dans les variables d'environnement plutôt que dans les drapeaux, voir [Configuration](/fr/reference/configuration/).
...
La commande racine trie ses sous-commandes dans les sections qu'affiche
`olivares --help` : configuration initiale et configuration, exploitation,
gouvernance, observation et diagnostic, sécurité, puis release et mise à niveau.

Les sections suivantes expliquent les commandes que vous rencontrerez d'abord — ce
qu'elles font à votre installation et ce qu'il est sûr d'exposer —, ce qu'un tableau
ne peut pas transmettre. Pour la liste exhaustive, la
[Référence complète des commandes](#complete-command-reference) à la fin de cette page
est **générée à partir du binaire** et couvre chaque commande du build avec ses
drapeaux, ses codes de sortie et son contrat de sortie. Le générateur parcourt l'arbre
de commandes du binaire communautaire (`TestCLIRefDump`) et écrit ce parcours dans la
région marquée. Le gate de push et la CI exécutent cette vérification via
`lint:public-counts`, qui invoque `scripts/check-cli-ref-docs.sh` : il redérive les
drapeaux, les valeurs par défaut, les codes de sortie et les contrats de sortie à
partir du binaire et échoue lorsque cette page ou l'une des six copies localisées
diverge. Une vérification distincte, `lint:cli-coverage`, vérifie seulement que les
noms de commandes déclarés dans la source sont mentionnés quelque part dans cette
référence ; elle ne redérive pas les drapeaux. Les noms de commandes, les noms de
drapeaux, les textes d'aide, les valeurs par défaut et les significations des codes
de sortie dans la région générée sont les chaînes anglaises originales du binaire —
y compris sur les copies localisées de cette page, où ce bloc n'est pas une
traduction humaine. Lorsque le binaire déployé et cette page divergent,
`olivares <subcommand> --help` sur ce binaire fait foi.

:::note[Binaire unique, plusieurs rôles]
Il n'y a pas de téléchargement « serveur » et « agent » séparés. `olivares serve` est le moteur ; `olivares collector` est le collecteur du plan de données pour la topologie distribuée. Tous deux chargent les mêmes connecteurs de source de la même manière — seul l'endroit où vont les observations diffère. Voir [Architecture](/fr/explanation/architecture/overview/).
:::

## olivares version

Affiche la version, le commit, la date de build, l'OS/arch et la version du runtime Go sur la sortie standard.

```sh
olivares version
```

La chaîne de version est injectée au moment du build. Un build issu d'un arbre de travail qui n'a pas été produit par une release taguée rapporte une version de développement (par exemple `dev`), donc ne traitez pas la chaîne de version comme une garantie de provenance — vérifiez plutôt les releases avec les artefacts signés. Voir [Vérifier une release](/fr/how-to/verify-a-release/).

## olivares serve

Exécute le control plane : le serveur HTTP (API REST plus l'interface web embarquée sur la même origine) et le serveur gRPC. **TLS est activé par défaut**, les écouteurs se lient à la **boucle locale par défaut**, et il n'y a **aucun identifiant par défaut**.
...
Tous les drapeaux acceptés par `serve`, avec leur type et leur valeur par défaut,
figurent sous [`olivares serve`](#command-olivares-serve) dans la référence générée
ci-dessous. Cette section ne couvre que les quatre qui modifient ce que l'installation
expose : `--listen` et `--grpc-listen` (où elle se lie), `--insecure` (si le transport
est chiffré) et `--seed-demo` (si les données sont réelles).

Le stockage par défaut est SQLite (pur-Go, mono-nœud, adapté aux installations en environnement isolé). Sélectionner `postgres` est ce que vous faites pour les déploiements multi-tenant ou à montée en charge, où la sécurité au niveau des lignes (row-level security) assure le filet de sécurité par tenant. Voir [Configuration](/fr/reference/configuration/) et [Auto-hébergement](/fr/how-to/self-hosting/).

:::caution[`--insecure` est réservé au développement localhost]
`--insecure` sert HTTP et gRPC en texte clair. Les jetons bearer transitent en clair sur un transport en texte clair, donc n'utilisez jamais ce drapeau sur une adresse joignable au-delà de l'hôte local. Hors `--insecure`, le serveur gRPC refuse de démarrer sans TLS plutôt que de se rétrograder silencieusement.
:::

### `--seed-demo` est réservé à la démo et refuse la non-boucle-locale

`--seed-demo` provisionne un parc **synthétique et fabriqué** avec un administrateur de démo dont le mot de passe est **public** (il vit dans l'arbre des sources). Il existe uniquement pour faire afficher l'interface web et les tests de bout en bout contre des données ayant la forme du réel.

Parce que l'identifiant de démo est public, `serve` **refuse de démarrer avec `--seed-demo` sur toute liaison non-boucle-locale** et se termine sur une erreur vous invitant à vous lier à `127.0.0.1` ou à exécuter une vraie installation sans le drapeau. Traitez `--seed-demo` comme jetable : utilisez un répertoire de données jetable, et ne le pointez jamais vers des données qui vous importent.

:::danger[N'exécutez jamais `--seed-demo` comme vraie installation]
L'administrateur de démo a un mot de passe publiquement connu. Une vraie installation, c'est `serve` **sans** `--seed-demo`, où le moteur émet un jeton de configuration à usage unique et où vous créez votre propre administrateur. Ne mélangez pas les deux : un répertoire de données de démo n'est pas un répertoire de données de production.
:::

### Configuration au premier démarrage

Au premier démarrage d'une installation qui n'a aucun utilisateur, `serve` affiche un bloc sur la sortie standard contenant le jeton de configuration à usage unique (préfixe `olst_`) et la requête dont vous avez besoin pour amorcer le premier administrateur. Le flux est :

1. Lisez le jeton `olst_` depuis la sortie standard du moteur (avec le déploiement en conteneur, lisez-le depuis les journaux du conteneur).
2. Créez le premier administrateur en envoyant le jeton, un e-mail et un mot de passe à l'endpoint de configuration (`POST /v1/setup`).
3. Connectez-vous (`POST /v1/auth/login`) pour obtenir un jeton de session (préfixe `olvs_`).

Le jeton de configuration est affiché une seule fois et est à usage unique ; une fois qu'un utilisateur existe, aucun jeton n'est émis. Olivares AI utilise des jetons bearer **opaques** (pas des JWT) ; les clés d'API portent le préfixe `olvk_`. Pour le contrat d'authentification complet et les règles de résolution de tenant, voir le [Modèle de sécurité](/fr/explanation/security/security-model/) et la [Référence de l'API](/reference/api/).

### Le lancer

```sh
# Build, puis exécution (il n'y a pas de cible "task serve" / "task run").
task build
./bin/olivares serve
# Lisez le jeton de configuration olst_ à usage unique sur la sortie standard de ce processus.
```

Ou exécutez le déploiement en conteneur et lisez le jeton de configuration depuis les journaux ; voir [Auto-hébergement](/fr/how-to/self-hosting/).

## olivares collector

Exécute le binaire comme un **collecteur de périphérie** pour la topologie distribuée. Un collecteur charge localement les connecteurs de source nommés dans votre configuration de sources et **pousse** leurs observations vers un cœur distant via gRPC. Il n'ouvre **aucun écouteur entrant** — le collecteur est le plan de données sécurisé par défaut : il compose vers l'extérieur, il n'accepte pas de connexions.

```sh
olivares collector --core-addr host:port [flags]
```

Le collecteur s'authentifie auprès du cœur sur deux couches : un jeton bearer portant un principal d'ingestion, et — lorsque le cœur impose le TLS mutuel — un certificat client de collecteur. C'est ainsi que le plan de données s'exécute sur l'infrastructure du client tandis qu'un cœur central agrège : un collecteur en panne ne se trouve jamais dans le chemin de données d'un agent.

:::note[Quelles sources un collecteur charge]
`serve` et `collector` câblent tous deux leurs connecteurs de source à partir de la même configuration, lue depuis l'environnement avant le démarrage du runtime. Une source non configurée ou vide avertit honnêtement plutôt que de faire échouer le processus. La configuration des sources est traitée dans [Connecter une source](/fr/how-to/connect-a-source/) et [Connecter Claude Code](/fr/how-to/connect-claude-code/) ; le mécanisme de configuration est dans [Configuration](/fr/reference/configuration/).
:::

La sous-commande collector est le **mécanisme** du chemin distribué. Le packaging qui l'entoure (une flotte de collecteurs, des charts signés, des images OCI) fait partie de l'histoire du déploiement plutôt que de cette page CLI — voir [Architecture](/fr/explanation/architecture/overview/) et [Auto-hébergement](/fr/how-to/self-hosting/).

## olivares openapi

Affiche le document OpenAPI 3.1 du moteur sur la sortie standard, sans avoir besoin d'un serveur en cours d'exécution.
...
Par défaut, la commande émet le même contrat que le moteur sert sur
`GET /openapi.json` (les chemins stables du cœur), indenté de façon déterministe
pour que la sortie produise des diffs propres ; c'est la source de vérité pour la
[Référence de l'API](/reference/api/) rendue et pour la génération de code typé du
client web. `--beta` émet le document **bêta** des routes de module
(`/v1/m/<ns>/…`, servi sur `GET /openapi.beta.json`, rendu dans la
[référence des routes de module](/reference/api-beta/)), reflété à partir des routes
enregistrées par les modules, dont les formes au niveau des champs restent exprimées
comme des interfaces typées (voir aussi l'[Aperçu des modules](/fr/reference/modules/overview/)).

:::note[La référence générée reste en anglais]
Le bloc technique suivant, marqueurs compris, est régénéré depuis le même dump du binaire
que la page anglaise, et le gate le compare à chaque push. Les noms de commandes, les
descriptions d'aide, les flags, les codes de sortie et les contrats de sortie restent
volontairement en anglais : ce sont exactement les chaînes qu'émet le binaire, pas une
traduction humaine. Le guide éditorial qui les entoure est localisé.
:::
...
C'est un produit pré-1.0 en développement actif. Les sous-commandes et drapeaux ci-dessus sont confirmés dans le binaire actuel, mais la surface CLI complète évolue encore : sous-commandes, drapeaux, valeurs par défaut et formats de sortie peuvent changer avant une release stable. En cas de doute, exécutez `olivares <subcommand> --help` contre le build exact que vous avez déployé, et traitez cela comme faisant autorité sur tout document. Pour ce qui est implémenté aujourd'hui par rapport à ce qui est planifié, voir [Honnêteté et limites](/fr/start/honesty-and-limits/). La surface de l'API REST/gRPC elle-même est régie par la [politique de stabilité de l'API](/fr/reference/api-stability/).
