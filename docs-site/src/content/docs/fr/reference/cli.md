...
drapeaux, ses codes de sortie et son contrat de sortie. Elle a été écrite en énumérant
l'arbre de commandes du binaire construit ; traitez-la donc comme un instantané de ce
build plutôt que comme une sortie en direct : le gate de push et la CI exécutent
`lint:cli-coverage`, qui vérifie que les commandes déclarées dans la source restent
documentées quelque part dans cette référence et nomme celles qui ne le sont pas,
mais ne redérive pas cette section et ne compare ni les drapeaux, ni les codes de
sortie, ni les contrats de sortie. Lorsque cette page et votre binaire divergent,
`olivares <subcommand> --help` fait foi.
...
Le bloc technique suivant, marqueurs compris, est reproduit octet pour octet depuis la sortie
anglaise du binaire. Les noms de commandes, les descriptions d'aide, les flags, les codes de
sortie et les contrats de sortie restent volontairement en anglais, car ce sont exactement les
chaînes qu'émet le binaire ; le guide éditorial qui les entoure est localisé.
:::
