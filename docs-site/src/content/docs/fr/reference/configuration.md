...
| `OLIVARES_CIRCUIT_BREAKER_CONFIG` | No | — | Interrupteur booléen, et non un chemin de fichier malgré le suffixe `_CONFIG`, du disjoncteur (circuit breaker) du runtime ; ses règles par tenant ne se définissent pas ici. Seuls les builds compilés avec les tags `enterprise` et `addon_airs` le lisent, au démarrage du moteur ; une modification prend effet après un redémarrage. Une variable non définie ou une valeur que `strconv.ParseBool` accepte comme false laisse le disjoncteur désactivé. Une valeur qui n’est pas booléenne est journalisée comme erreur et le laisse aussi désactivé. Une valeur true l’active lorsque des données de module sont disponibles ; sinon, un avertissement est journalisé et il reste désactivé. |
...
| `OLIVARES_COMPUTER_USE_CONFIG` | No | — | Chemin du fichier de politique JSON du gate de computer use. Seuls les builds compilés avec les tags `enterprise` et `addon_airs` le lisent. Si la variable n’est pas définie, le gate reste désactivé ; un fichier illisible ou impossible à analyser lui fait refuser tout computer use jusqu’à ce que le fichier soit corrigé et le moteur redémarré. |
...
| `OLIVARES_CONTENT_FIREWALL_CONFIG` | No | — | Chemin du fichier de politique JSON du content firewall qui inspecte le trafic d’inférence dans le proxy inline. Seuls les builds compilés avec les tags `enterprise` et `addon_airs` le lisent. Si la variable n’est pas définie, cette inspection reste désactivée ; un fichier illisible ou impossible à analyser fait refuser au firewall chaque requête qu’il inspecte jusqu’à ce que le fichier soit corrigé et le moteur redémarré. |
...
| `OLIVARES_CREDENTIAL_MINTER_CONFIG` | No | — | Chemin de la configuration JSON de l’émetteur d’identifiants upstream par serveur (token exchange RFC 8693) de la passerelle MCP. Elle contient des secrets, c’est pourquoi il s’agit d’un fichier plutôt que d’une valeur. Les builds compilés avec le tag `enterprise` le lisent lorsque le moteur démarre et construit la passerelle pour une URL upstream. Si la variable n’est pas définie, l’identifiant upstream statique est conservé ; un fichier illisible ou impossible à analyser refuse tout identifiant upstream jusqu’à ce que le fichier soit corrigé et le moteur redémarré. |
...
| `OLIVARES_ELICITATION_MEDIATOR_CONFIG` | No | — | Chemin du fichier de politique JSON du médiateur d’élicitation MCP. Seuls les builds compilés avec les tags `enterprise` et `addon_airs` le lisent. Si la variable n’est pas définie, la médiation reste désactivée ; un fichier illisible ou impossible à analyser fait refuser au médiateur chaque élicitation jusqu’à ce que le fichier soit corrigé et le moteur redémarré. |
...
| `OLIVARES_RTBF_DEPTH_CONFIG` | No | — | Chemin de la configuration JSON du coordinateur de crypto-shredding RTBF. Seuls les builds compilés avec les tags `enterprise` et `addon_reg` le lisent, au démarrage du moteur. Si la variable n’est pas définie, le crypto-shredding de l’open core reste inchangé ; un fichier illisible ou impossible à analyser bloque tout crypto-shredding jusqu’à ce que le fichier soit corrigé et le moteur redémarré. |
