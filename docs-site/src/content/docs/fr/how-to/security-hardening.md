...
| **La liaison n'est pas la protection** | Le moteur se lie à **toutes les interfaces** par défaut : c'est un serveur. Ce qui le protège, c'est TLS activé, aucun identifiant par défaut et un token de configuration à usage unique — rien de tout cela ne dépend de la liaison. | Restreignez-la **délibérément** quand vous le souhaitez : `--listen 127.0.0.1:8443` pour le binaire, `OLIVARES_BIND=127.0.0.1` pour la stack Compose. Placez devant votre propre ingress/TLS pour un certificat que les navigateurs acceptent et un nom d'hôte utilisable par les passkeys — voir [self-hosting](/fr/how-to/self-hosting/). |
