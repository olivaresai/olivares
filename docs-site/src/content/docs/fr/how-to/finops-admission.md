...
`drift` est vrai s'il reste des réserves expirées non soldées. FinOps émet
alors un constat `finops_reservation_drift`. Traitez-le comme une posture,
pas comme une réécriture silencieuse de la dépense.
