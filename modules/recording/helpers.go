...
// listCap per page), so a scan over a small owned set is complete.
...
// appendAudit appends one ledger event in the caller's transaction. The
// recording module's readable bindings ride TargetKind/TargetID/PayloadHash
// (ledger Meta is write-only on read paths); Meta stays small hashed context.
func appendAudit(ctx context.Context, sc store.Scope, actor, actorKind, action string, kind model.Kind, id model.ID, meta map[string]any, payloadHash []byte) (model.AuditEvent, error) {
	return sc.Audit().Append(ctx, model.AuditDraft{
		Actor:       actor,
		ActorKind:   actorKind,
		Action:      action,
		TargetKind:  kind,
...
// tsValue reads a timestamp column, ok=false when null/empty/malformed.
func tsValue(rec model.Record, col string) (model.Timestamp, bool) {
	s := rec.String(col)
	if s == "" {
		return model.Timestamp{}, false
	}
	t, err := model.ParseTimestamp(s)
	if err != nil {
		return model.Timestamp{}, false
	}
	return t, true
