...
// InspectReservations REPORTS the reservation ledger against its commits and
// releases, and writes nothing at all: no sweep, no finding, no event. It is the
// answer the budget-READ route gives, so holding read permission never moves a
// hold or files evidence. A lapsed hold the job has not swept yet is visible
// here as ActiveLapsed, which is drift by the same definition.
func (m *Module) InspectReservations(ctx context.Context, tenant model.TenantID) (AdmissionReconciliation, error) {
	...
	if out.Drift {
		out.Note = "reservation ledger drifted from caller settlement"
	} else {
...
// ReconcileReservations is the JOB an operator or a scheduler runs: it sweeps
// expired holds, inspects what is left and emits a posture finding when the
// ledger drifted from what its callers settled. Drift is a finding, never a
// silent rewrite — this never moves money. It needs budget WRITE, which is the
// difference between it and InspectReservations.
func (m *Module) ReconcileReservations(ctx context.Context, tenant model.TenantID) (AdmissionReconciliation, error) {
	...
	out, err := m.InspectReservations(ctx, tenant)
	if err != nil {
		return AdmissionReconciliation{}, err
	}
	out.SweptExpired = swept
	if out.Drift {
		if ref := m.emitReservationDrift(ctx, tenant, out); ref != "" {
			out.FindingRef = ref
