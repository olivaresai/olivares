...
func TestReserveDeniesExhaustedBudgetAndAudits(t *testing.T) {
	forEachAdmissionEngine(t, runReserveDeniesExhaustedBudgetAndAudits)
}

func runReserveDeniesExhaustedBudgetAndAudits(t *testing.T, cfg store.Config) {
...
func TestReserveIdempotentRetryDoesNotDoubleCount(t *testing.T) {
	forEachAdmissionEngine(t, runReserveIdempotentRetryDoesNotDoubleCount)
}

func runReserveIdempotentRetryDoesNotDoubleCount(t *testing.T, cfg store.Config) {
	m, st, tenant, _ := openFinCfg(t, cfg)
	createBudget(t, st, tenant, "global-block", budgetSpec{
...
func TestAdmissionReserve_ConcurrentExactlyMminus1(t *testing.T) {
	forEachAdmissionEngine(t, runAdmissionReserveConcurrentExactlyMminus1)
}

func runAdmissionReserveConcurrentExactlyMminus1(t *testing.T, cfg store.Config) {
	m, st, tenant, _ := openFinCfg(t, cfg)
	const M = 100
...
func TestAdmissionReserve_SameKeyParallelDoesNotDoubleCount(t *testing.T) {
	forEachAdmissionEngine(t, runAdmissionReserveSameKeyParallelDoesNotDoubleCount)
}

func runAdmissionReserveSameKeyParallelDoesNotDoubleCount(t *testing.T, cfg store.Config) {
	m, st, tenant, _ := openFinCfg(t, cfg)
	createBudget(t, st, tenant, "global-block", budgetSpec{
...
// Reserve. Without it a per-seat cap and a pooled budget are the same refusal to every
// caller, and a gateway publishes them as the same sentence.
func TestAdmissionSpendLimitDenyIsMarked(t *testing.T) {
	forEachAdmissionEngine(t, runAdmissionSpendLimitDenyIsMarked)
}
...
// TestAdmissionBudgetDenyIsNotMarkedSpendLimit is the other side of the same flag: a
// pooled budget refusal must not claim to be a seat limit, or the marker says nothing.
func TestAdmissionBudgetDenyIsNotMarkedSpendLimit(t *testing.T) {
	forEachAdmissionEngine(t, runAdmissionBudgetDenyIsNotMarkedSpendLimit)
}

func runAdmissionBudgetDenyIsNotMarkedSpendLimit(t *testing.T, cfg store.Config) {
	m, st, tenant, _ := openFinCfg(t, cfg)
	createBudget(t, st, tenant, "global-block", budgetSpec{
		Dimension: "global", Period: "monthly", LimitMicroUSD: oneUSD, Action: "block",
	})
