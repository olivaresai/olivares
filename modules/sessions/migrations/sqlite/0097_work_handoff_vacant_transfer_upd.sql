...
--
-- OT-V, UPDATE half. The accepted-Handoff lease-effect rule becomes conditional
-- on the offered generation instead of demanding a fence advance that a vacant
-- WorkLease cannot mint: an accept that found a generation (offered >= 1) must
-- still invalidate it and advance past it, and an accept sealed against the
-- item's one vacant generation (offered 0/NULL) must NOT invent one. This is the
-- exact SQL twin of handoffAcceptFenceOK in communication_state.go.
--
-- Nothing else in the trigger changes: no table, column, type, index or
-- constraint, and no data migration. The pre-check proves rather than assumes
-- that no already-accepted row is in the newly forbidden shape; it aborts the
-- migration transaction (SQLite's documented integer-overflow error) instead of
-- repairing a row, so failure preserves the v62 guard.
SELECT CASE WHEN EXISTS (
	SELECT 1
	FROM sessions_work_handoff
	WHERE state = 'accepted'
		AND COALESCE(offered_lease_fence,0) = 0
		AND COALESCE(resulting_lease_fence,0) <> 0
) THEN abs(-9223372036854775808) ELSE 0 END;
DROP TRIGGER IF EXISTS sessions_work_handoff_guard_upd;
CREATE TRIGGER sessions_work_handoff_guard_upd
