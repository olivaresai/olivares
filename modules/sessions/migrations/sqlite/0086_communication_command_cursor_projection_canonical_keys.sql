...
BEGIN
	...
	WHERE NEW.version <> 1
		...
		OR (json_type(NEW.response_projection_json,'$.inbox_cursor') IS NOT NULL
			AND json_type(NEW.response_projection_json,'$.inbox_cursor') <> 'object')
		OR (json_type(NEW.response_projection_json,'$.inbox_cursor') = 'object' AND (
			(SELECT count(*) FROM json_each(
				NEW.response_projection_json,'$.inbox_cursor')) NOT IN (1,3)
			OR EXISTS (SELECT 1 FROM json_each(
				NEW.response_projection_json,'$.inbox_cursor') entry
			...
			OR COALESCE(json_type(
				NEW.response_projection_json,'$.inbox_cursor.last_seen_seq') <> 'integer',1)
			OR typeof(json_extract(
				NEW.response_projection_json,'$.inbox_cursor.last_seen_seq')) <> 'integer'
			OR json_extract(
				NEW.response_projection_json,'$.inbox_cursor.last_seen_seq') < 0
			OR (json_type(NEW.response_projection_json,
				'$.inbox_cursor.barrier_delivery_id') IS NULL) IS NOT
				(json_type(NEW.response_projection_json,
				'$.inbox_cursor.barrier_reason') IS NULL)
			OR (json_type(NEW.response_projection_json,
				'$.inbox_cursor.barrier_delivery_id') IS NOT NULL AND (
				json_type(NEW.response_projection_json,
					'$.inbox_cursor.barrier_delivery_id') <> 'text'
				OR json_extract(NEW.response_projection_json,
					'$.inbox_cursor.barrier_delivery_id') NOT GLOB
					'????????-????-7???-[89ab]???-????????????'
				OR length(replace(json_extract(NEW.response_projection_json,
					'$.inbox_cursor.barrier_delivery_id'),'-','')) <> 32
				OR replace(json_extract(NEW.response_projection_json,
					'$.inbox_cursor.barrier_delivery_id'),'-','') GLOB '*[^0-9a-f]*'))
			OR (json_type(NEW.response_projection_json,
				'$.inbox_cursor.barrier_reason') IS NOT NULL AND (
				json_type(NEW.response_projection_json,
					'$.inbox_cursor.barrier_reason') <> 'text'
				OR json_extract(NEW.response_projection_json,
					'$.inbox_cursor.barrier_reason') NOT IN (
					'not_yet_available','temporarily_invisible')))))
		OR (NEW.result_kind = 'sessions.inbox_cursor' AND (
			(NEW.response_projection_json COLLATE BINARY) IS NOT (CASE
				...
				ELSE
					'{"inbox_cursor":{"barrier_delivery_id":"' || json_extract(
						...
						NEW.response_projection_json,'$.version') || '}'
				END) COLLATE BINARY
			OR NEW.result_id IS NULL OR NEW.http_status <> 200 OR NEW.event_id IS NOT NULL
			OR (SELECT count(*) FROM json_each(NEW.response_projection_json)) <> 2
			OR COALESCE(json_type(
				NEW.response_projection_json,'$.version') <> 'integer',1)
			OR typeof(json_extract(
				NEW.response_projection_json,'$.version')) <> 'integer'
			OR json_extract(NEW.response_projection_json,'$.version') < 1
			OR COALESCE(json_type(
				NEW.response_projection_json,'$.inbox_cursor') <> 'object',1)
			OR json_type(NEW.response_projection_json,'$.ids') IS NOT NULL
			OR json_type(NEW.response_projection_json,'$.state') IS NOT NULL
			OR json_type(NEW.response_projection_json,'$.counts') IS NOT NULL
			OR json_type(NEW.response_projection_json,'$.digests') IS NOT NULL))
		OR (NEW.result_kind <> 'sessions.inbox_cursor'
			AND json_type(NEW.response_projection_json,'$.inbox_cursor') IS NOT NULL)
