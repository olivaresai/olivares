...
CREATE TRIGGER IF NOT EXISTS sessions_work_acceptance_guard_upd
BEFORE UPDATE ON sessions_work_acceptance
FOR EACH ROW
BEGIN
	...
	SELECT RAISE(ABORT, 'olivares: acceptance identity is immutable')
	WHERE OLD.work_item_id IS NOT NEW.work_item_id OR OLD.criterion_key IS NOT NEW.criterion_key;
	...
	SELECT RAISE(ABORT, 'olivares: acceptance definition is editable only in draft')
	WHERE (OLD.statement IS NOT NEW.statement OR OLD.required IS NOT NEW.required OR OLD.ordinal IS NOT NEW.ordinal)
		AND NOT EXISTS (SELECT 1 FROM sessions_work_item w WHERE w.id = NEW.work_item_id
			AND w.tenant_id = NEW.tenant_id AND w.workspace_id = NEW.workspace_id AND w.status = 'draft');
END;
