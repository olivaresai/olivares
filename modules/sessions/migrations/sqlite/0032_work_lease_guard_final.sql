...
-- Versions 25..31 accepted any 40-byte osn_ spelling. Refuse the upgrade when
-- one of those rows carries non-canonical authority: rewriting a holder SID
-- would invent identity. SQLite's documented integer-overflow error is the
-- fail-closed assertion here. It and both trigger replacements share the
-- migration transaction, so failure preserves the v31 guards.
SELECT CASE WHEN EXISTS (
	SELECT 1
	FROM sessions_work_lease
	WHERE (state <> 'vacant' AND holder_sid IS NULL)
		OR (holder_sid IS NOT NULL AND (
			length(holder_sid) <> 40
			OR substr(holder_sid, 1, 4) <> 'osn_'
			OR substr(holder_sid, 13, 1) <> '-' OR substr(holder_sid, 18, 1) <> '-'
			OR substr(holder_sid, 23, 1) <> '-' OR substr(holder_sid, 28, 1) <> '-'
			OR length(replace(substr(holder_sid, 5), '-', '')) <> 32
			OR replace(substr(holder_sid, 5), '-', '') GLOB '*[^0-9a-f]*'
		))
) THEN abs(-9223372036854775808) ELSE 0 END;
DROP TRIGGER IF EXISTS sessions_work_lease_guard_ins;
CREATE TRIGGER sessions_work_lease_guard_ins
...
BEGIN
	...
	WHERE NEW.state <> 'vacant' AND (NEW.holder_sid IS NULL OR length(NEW.holder_sid) <> 40
		OR substr(NEW.holder_sid, 1, 4) <> 'osn_'
		OR substr(NEW.holder_sid, 13, 1) <> '-' OR substr(NEW.holder_sid, 18, 1) <> '-'
		OR substr(NEW.holder_sid, 23, 1) <> '-' OR substr(NEW.holder_sid, 28, 1) <> '-'
		OR length(replace(substr(NEW.holder_sid, 5), '-', '')) <> 32
		OR replace(substr(NEW.holder_sid, 5), '-', '') GLOB '*[^0-9a-f]*'
		OR NEW.acquired_at IS NULL OR NEW.expires_at IS NULL);
...
CREATE TRIGGER sessions_work_lease_guard_upd
...
BEGIN
	...
	WHERE OLD.state = 'active' AND NEW.state IN ('released','expired','revoked')
		AND (NEW.fence <> CASE
				WHEN OLD.fence = 9223372036854775807 THEN OLD.fence
				ELSE OLD.fence + 1
			END
			OR NEW.renewal_count <> OLD.renewal_count);
	...
	WHERE NEW.state = 'active' AND (OLD.state <> 'active' OR NEW.holder_sid IS NOT OLD.holder_sid
		...
		AND (OLD.fence = 9223372036854775807
			OR NEW.fence <> CASE
				WHEN OLD.fence = 9223372036854775807 THEN OLD.fence
				ELSE OLD.fence + 1
			END
