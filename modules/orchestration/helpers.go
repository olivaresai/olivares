...
// Recency windows for deriving liveness from last activity. They are display
// heuristics applied at READ time, never stored, so a flow/node state is always
// accurate to the moment and the module never fabricates a stored lifecycle (the
// cooperative stream carries no end/failure signal — silence is honest).
const (
...
// maxTS returns the lexicographically-greatest non-empty timestamp string
// (canonical RFC3339 timestamps sort lexically, so this is a valid max-instant).
func maxTS(ts ...string) string {
	out := ""
	for _, t := range ts {
		if t > out {
			out = t
...
// entity (docs/SECURITY-HARDENING.md,§5) — never the system actor and never a secret/payload
// (Meta carries ids and counts only).
...
// hashHex returns the hex SHA-256 of s — a one-way fingerprint of an already
// redaction-safe ref used for the on-bus FindingReport.DetailHash, the plan_hash,
// and any sensitive *_ref column. Content is never stored raw (docs/SECURITY-HARDENING.md).
func hashHex(s string) string {
	sum := sha256.Sum256([]byte(s))
	return hex.EncodeToString(sum[:])
}

// canonicalFields builds an UNAMBIGUOUS preimage from a field list by
// length-prefixing each field ("<len>:<bytes>"), so a hash over it cannot
// collide by moving a delimiter into a controllable value (a "|"-joined
// preimage let subject_ref="a|b",cadence="c" collide with subject_ref="a",
// cadence="b|c"). The lengths pin every field boundary, so the encoding is
// injective over any field list — a substitution-without-a-race is impossible.
...
// canonicalHash length-prefixes fields (domain-separated by tag) and hashes them.
func canonicalHash(tag string, fields ...string) string {
	return hashHex(canonicalFields(append([]string{tag}, fields...)...))
}
...
// A relation/schedule subject is usually an external string ref (a session id, a
// subagent type), not an engine id; those live in Metadata, and a typed core
// SubjectID stays zero rather than holding a non-id.
...
// 4-value persisted severity (core/model, no "info"): info/unknown collapse to low
// so a stray out-of-set value can never reach the store and a finding is never
// over-graded.
...
// TenantID, or false to skip (placeholder label or the system tenant). Event-driven
// ingestion uses it; API handlers use the already-pinned mc.Tenant.
...
// Derived liveness states (never stored; computed from last-activity recency).
const (
	stateActive    = "active"
	stateIdle      = "idle"
...
// deriveRecency maps a last-activity timestamp to active/idle/completed at read
// time, using the module's recency windows. An unparseable/empty timestamp is
// completed (no recent activity).
func deriveRecency(lastTS string, clock model.Clock, active, idle time.Duration) string {
