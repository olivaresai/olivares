...
**4 · Kubernetes** — the Helm chart from the tree, or a flat Helm-free manifest; the chart's OCI publication is unverified (`publication-unverified`: this repository never ran its chart publisher and has no chart tag or chart release asset, and the registry side is not observable).
